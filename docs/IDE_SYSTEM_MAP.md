...
- **Build testbench ownership**: `ScenarioBuilderPanel` now frames the left authoring zone as **Build testbench** and carries a compact summary of driven inputs, checked outputs, case/tick count, clock activity, and whether Compare checks are armed. The primary run CTA stays visually attached to this setup area instead of reading like a separate top-level shell action.
