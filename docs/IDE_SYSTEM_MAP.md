...
| Verify | `packages/rb-apps/src/apps/ide/surfaces/VerifySurface.tsx` | Deterministic verification, waveform viewer; **Phase 5:** header **Run** label from `buildVerifySessionViewModel` distinguishes **observe** (`Run · observe only` / `Update run · observe only`) vs **compare** (`Run · compare checks` / `Update run · compare checks`) so the primary action matches **Use saved checks** mode |
