...
| HQ (utility) | `packages/rb-apps/src/apps/ide/surfaces/HqSurface.tsx` | Local command-center utility surface for Marcus status/chat, control snapshot, bench evidence, memory status, tool-assisted reasoning, and approval-gated coding-plan packet generation. HQ is non-spine and does not alter required student workflow progression. |
