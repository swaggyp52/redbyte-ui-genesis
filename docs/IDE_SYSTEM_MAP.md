...
| Project | `packages/rb-apps/src/apps/ide/surfaces/ProjectSurface.tsx` | Action-first home for **Project -> Design -> Verify -> Map Pins -> Export**. First launch emphasizes starting a lab/circuit; loaded state emphasizes the next workflow action. Starter, recent, bridge/determinism, and advanced paths remain available through explicit disclosures. Import is linked as a utility, not represented as a sixth stage. |
| Design | `packages/rb-apps/src/apps/ide/surfaces/DesignSurface.tsx` | Canvas-first structural authoring and live simulation. A compact core toolbar, visible/collapsible library, and selection-driven inspector support the graph without becoming competing page authorities. Design remains editable without a local bridge agent. |
| Verify | `packages/rb-apps/src/apps/ide/surfaces/VerifySurface.tsx` | Deterministic testbench authoring, Observe-only evidence, Compare-check proof, waveform review, and repair routing. One command bar owns Run; selecting Compare after an Observe run remains reachable without changing comparison or freshness semantics. |
| Hardware / Map Pins | `packages/rb-apps/src/apps/ide/surfaces/HardwareSurface.tsx` | Table-first Basys3 binding surface. The primary loop is project signal -> board resource -> package pin through the mapping table and direct board assignment. Board reference, XDC explanation, and after-mapping tools stay secondary and consume the same mapping authority as Export. |
| Export | `packages/rb-apps/src/apps/ide/surfaces/ExportSurface.tsx` | Readiness-first Vivado handoff with one state-appropriate repair/build/download action. Generated-file browsing, diagnostics, and proof metadata are secondary; generated bytes and proof tiers are unchanged. |
| Import utility | `packages/rb-apps/src/apps/ide/surfaces/ImportSurface.tsx` | RedByte restore/recovery plus Vivado ZIP / HDL+XDC inspection. Import is reachable outside the numbered stage rail and preserves active work until explicit apply. |

### Current professional shell and surface composition (local 2026-07-13; browser-E0 validated)

- **Persistent shell:** `IdeApp.tsx`, `IdeTopBar.tsx`, `IdeLeftRail.tsx`, and `IdeSurfaceLayout.tsx` compose a compact top bar, exactly five numbered stages, and the active workbench. The former proof ribbon, bottom status footer, and injected `PageProductHeader` / product-spine strip are absent.
- **Five-stage authority:** the rail is `Project -> Design -> Verify -> Map Pins -> Export`, with current/complete/blocked states derived from existing workflow authority. Import is a separate utility action with no stage number or completion state.
- **Top bar:** product identity, editable project identity, Browser E0 boundary, and one Help entry. Surface Run/Export actions stay inside their owning surfaces.
- **Project:** action-first first-launch and loaded states; starter, recent, bridge/determinism, and advanced project paths are disclosed on request.
- **Design:** the circuit canvas is primary; Select/Wire/Undo/Redo/Fit are direct, lower-frequency controls are grouped, the library is collapsible, and the inspector opens for selection/repair context.
- **Verify:** one command-bar Run action owns execution. Observe only remains non-proof trace evidence; Compare checks owns expected-output proof, including the repaired Observe -> Compare transition.
- **Hardware / Map Pins:** the mapping table and direct row/board assignment loop are first-order. Board reference, XDC explanation, after-mapping tools, and advanced mapping data stay secondary.
- **Export:** one readiness block owns blocked/draft/ready/trusted consequence and the state-appropriate next action. Generated-file inspection, diagnostics, Vivado steps, and proof metadata stay secondary.
- **Boundary:** this local recomposition changes hierarchy and interaction wiring only. Node 20 build/tests, four-viewport professional proof, focused gates, and the full classroom aggregate pass locally. Generated bytes, mapping truth, Compare semantics, and E0/E1/E2/E3 boundaries are unchanged. Automated after evidence is complete for the declared browser states; a fresh three-loop manual replay of every local surface was not completed.

### Previous shell chrome model (superseded by the local professional recomposition)
...
### Previous Design chrome model (superseded by the local professional recomposition)
...
### Previous Hardware chrome model (superseded by the local professional recomposition)
...
### Previous Export chrome model (superseded by the local professional recomposition)
...
### Previous Verify chrome model (superseded by the local professional recomposition)
