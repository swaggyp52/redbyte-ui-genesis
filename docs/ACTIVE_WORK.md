...
**Latest product/control slice:** Workflow Orientation Integrated v1
...
5. **Workflow Orientation Integrated v1 is closed locally.** `ide:gate:interaction-affordance` now also proves a loaded Project `Flow` reopen uses an integrated callout that does not cover Project entry paths at `1440x900`; after IAB proof at `1366x768` and `1440x900` shows the compact callout above the Project paths with no root overflow.
6. **Export Artifact Direct Preview v1 is closed locally.** `ide:gate:export-artifact-direct-preview` now proves the ready-to-build Export handoff generated-file cues are real preview controls, not passive chips: `top.vhd` and `top.xdc` are button/keyboard reachable, update `ide-export-preview-path`, reveal the existing artifact workspace, expose selected state, and do not change generated bytes or make E1/E2/E3 claims at `1366x768` and `1440x900`.
