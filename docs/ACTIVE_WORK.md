...
**Latest product/control slice:** Project Command Center
...
5. **Project Command Center is closed locally.** `ide:gate:project-command-center` now proves Project as a neutral command center with blank, starter, saved/recent, Import/recovery, loaded-project peer paths, no false no-circuit mapping/export failure copy, and a guarded loaded-project Build Fresh action.
6. **Next audit-driven code slice: Export Handoff Station.** Export trust integrity is already gate-backed; the next slice should improve the first-viewport handoff hierarchy without changing generated bytes.
...
**Target:** Export Handoff Station.

Structured hardening ticket fields to start from:

- Title: Rebuild Export as a handoff station.
- Surface: Export.
- Journey segment: draft package, E0-ready package, generated artifact review, Vivado handoff, evidence boundary.
- Observed behavior: Export trust integrity is now proven, but the surface can still read dense around readiness, mapping/provenance, artifact preview, and next Vivado step.
- Expected behavior: one first-viewport Export handoff station for draft/E0-ready package trust, visible generated artifact/provenance context, and no E1/E2/E3 overclaim.
- Minimum acceptance proof: Export draft and E0-ready screenshots at `1366x768`; existing Export trust/download/artifact gates; classroom gate; no VHDL/XDC/testbench/Tcl/ZIP/golden byte changes unless generator tests prove them.
...
| Next | Export Handoff Station. | `RB-EXPORT-HANDOFF-001` in issue index after reset. |
