...
|-------------------|----------|----|----|----|----|----|----------------------|----------------|
| `golden-basys3-switch-and` | Combinational | yes | yes | **yes** (`vivado_batch_golden_and.log`) | **no** — RC1 host: `hw_probe` exit 2 | no | **RC1 = E1 only** | SW0∧SW1→LED0; E2 when `lab:vivado:hw-probe` → 0 |
| IDE `signal-tour` | Combinational / map tour | yes | yes | **yes** (`vivado_batch_signal_tour.log`) | **no** (same bench) | no | **RC1 = E1 only** | Default onboarding |
