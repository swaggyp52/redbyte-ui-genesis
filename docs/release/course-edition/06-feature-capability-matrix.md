...
| Sequential circuits | Partially implemented | Counter support present; docs fence unsupported falling-edge-triggered capture, multi-clock, active-low reset, and async sequential work while retaining authored high-to-low stimulus for the rising-edge model. | Yes, bounded | High if generalized. | Single-clock rising-edge acceptance tests. | Known limitations. | Verify/Export | P0 | Keep bounded |
