...
| RB-INV-025 | `classroom:gate` must include trust-critical lightweight browser gates. | CI/Classroom | classroom gate script | includes Project interaction affordance, active mode reload recovery, Verify saved-checks default, Hardware first viewport, design workbench, Design Workbench v1, Design no-bridge, workbench space utilization, workbench visual finish, shell layout, shell workbench hierarchy, Project command center, Export trust, Export handoff station, Export first-viewport artifact visibility, Hardware Basys3 workbench, Import recovery, and Verify evidence workbench integrity gates | runtime duration can grow; monitor CI time | P1 |
| RB-INV-026 | Broader classroom verifier must include the same invariant gates for local/nightly breadth. | CI/Classroom | `verify-gates-classroom.mjs` | includes Project interaction affordance, active mode reload recovery, Verify saved-checks default, Hardware first viewport, design workbench, Design Workbench v1, Design no-bridge, workbench space utilization, workbench visual finish, shell layout, shell workbench hierarchy, Project command center, Export trust, Export handoff station, Export first-viewport artifact visibility, Hardware Basys3 workbench, Import recovery, and Verify evidence workbench integrity gates | broad suite can be slow | P2 |
...
| RB-INV-035 | Project identity and workflow help affordances must be directly actionable from normal first-launch use. | Project / top bar / onboarding | Project command center and persistence gates | `ide:gate:interaction-affordance` intentionally failed when dismissed Workflow Orientation had no visible reopen affordance, then passed after the Project top bar gained `Flow` reopen and inline title rename. It verifies visible build hash, orientation dismiss/reopen, title edit/cancel/save, Project/top-bar identity agreement, and reload persistence at `1366x768` | broader side-dock proportions and card-heavy interaction model remain separate future visual/product coverage | P1 |
