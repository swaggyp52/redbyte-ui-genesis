...
| RB-INV-052 | Release-candidate closeout must distinguish browser E0 readiness, final deployed-SHA proof, pinned Node proof, and downstream Vivado/Basys3 proof tiers. | Release management / proof process | release docs and final-current gate | `docs/product/RED_BYTE_RELEASE_CANDIDATE_DECISION.md` records current E0 posture and not-shippable blockers; `ide:gate:node20-proof-status` records the exact Node 20 blocker when the active shell is not Node `20.19.0`; `ide:gate:release-final-sha-discipline` delegates to the clean-worktree final current-build smoke. | final deployed-SHA proof must still be collected after the release commit; Node 20 proof remains blocked until the runtime is available | P1 |
