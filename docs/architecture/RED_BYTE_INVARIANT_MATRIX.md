...
| RB-INV-054 | Rendered Verify authority must match the V2 truth model for check ownership and Project/Export readiness. | Verify truth adapter / Verify workbench / Project workflow authority | Phase 3 state and adapter tests | `verify:truth-integration-gate` now includes `projectWorkflowAuthority.test.ts`, and `ide:gate:verify-v2-authority-cutover` proves Course checks render locked, expected-output cells are disabled with duplicate guidance, Duplicate to My checks makes the same checks editable, Compare still runs, and the rendered V2 result authority exposes result status, current state, Project status, and Export readiness. | staleDesign/staleTestbench reason UI, selected-failure repair display, and sequential timing visibility remain the next Phase 3 browser-proof targets. | P1 |
...
| Verify V2 rendered truth workbench | Make the rendered Verify surface consume `verifyTruthAdapter.ts` selectors and add focused browser gates for Course/My checks, locked expected values, stale design/testbench, selected failure repair, Project readiness, Export readiness, and sequential timing visibility. |
| Classroom durability rehearsal | Add a multi-context create/edit/save/reload/recover rehearsal after the storage facade/journal slice exists. |
