...
| RB-INV-004 | Each primary surface must expose a visible first-viewport focal object without competing shell status authorities. | Shell/surfaces | visual audits and surface gates | `ide:gate:shell-workbench-hierarchy` proves compact top bar + exactly five workflow stages + active workbench, no proof ribbon/footer/injected product-spine header, and visible focal objects across Project, Design, Verify, Hardware, Export, and Import utility; `ide:gate:workbench-space-utilization` proves the primary work object owns meaningful space at `1366x768`, `1440x900`, and `1920x1080`; `ide:gate:primary-work-object-dominance` proves Design, Verify, Hardware, Export, and Import keep the main object dominant; `ide:gate:workbench-visual-finish` and `ide:gate:import-guided-recovery-workflow` protect Import first-look and active recovery hierarchy; `ide:gate:project-command-center` and `ide:gate:project-loaded-command-surface` protect one dominant Project action plus reachable disclosed alternates without boxed metric-card stacks; Export gates protect one readiness authority followed by explicit generated-file inspection; Design and Hardware gates keep the canvas and mapping table/board as focal objects. | broader Project/Verify visual maturity remains product-owner review | P1 |
...
| RB-INV-010 | Observe-only output is not equivalent to Compare PASS. | Verify | verify contract/reality tests | normal-use audit exercised Observe then Compare; `ide:gate:verify-saved-checks-default` proves saved-check starters arm Compare by default while still allowing an explicit Observe-only switch; `ide:gate:verify-evidence-workbench` now runs Observe-only first and asserts it is trace evidence, not trusted PASS/FAIL proof | no new risk for the covered starter path | P1 |
| RB-INV-011 | Expected-output edits stale old PASS, can create intentional FAIL, and can be repaired to PASS. | Verify expected grid | `ide:gate:verify-fail-edit-repair`; workflow tests | `ide:gate:verify-evidence-workbench` also proves visible first-run editing, intentional FAIL, first mismatch expected/observed evidence, and repair PASS | none current for the covered starter path | P1 |
...
| RB-INV-017 | Export generated artifact list, preview, downloaded ZIP, and generated bytes should agree. | Export artifacts | export artifact explorer/download/e2e/golden tests | `ide:gate:export-trust-integrity` proves generated preview and downloaded ZIP entry/body parity, README/provenance, XDC mapped row count, and `EXPECTED_IO.json` output evidence; the generated-file disclosure gate proves `README.txt`, `top.vhd`, `top.xdc`, `testbench.vhd`, and `vivado_import.tcl` are present after `Inspect generated files` is opened; `ide:gate:export-artifact-direct-preview` proves the revealed file controls select and show `top.vhd` and `top.xdc` without changing generated bytes | no fresh Vivado proof | P1 |
...
| RB-INV-019 | Import must not replace the active project before review/apply, and visible failure copy must match the failure layer reached. | Import | Import surface tests, zip import gates | `ide:gate:import-recovery-contract` proves corrupt manifest ZIP and non-ZIP upload failures do not replace the active Logic Gates project, archive-level failures say no files were changed without showing HDL/XDC port-reconstruction guidance, and manifest restore still requires Review Import plus Confirm Replace Project; `ide:gate:import-guided-recovery-workflow` preserves the same active taskbar review/apply contract while changing only presentation hierarchy | arbitrary unsupported user files remain parser-specific future coverage | P2 |
...
| RB-INV-034 | Export must make generated artifact inspection explicit and concrete without letting the file browser outrank readiness. | Export handoff / artifact visibility | Export handoff station and artifact explorer gates | The migrated generated-file disclosure contract proves readiness is visible first, `Inspect generated files` is reachable, and opening it reveals `README.txt`, `top.vhd`, `top.xdc`, `testbench.vhd`, and `vivado_import.tcl` plus the artifact explorer; visible build hash, browser-E0 wording, and no E1/E2/E3 overclaim remain required at `1366x768` and `1440x900`. | custom-project artifact emphasis remains separate future coverage if reopened | P1 |
...
| RB-INV-038 | Generated-file cues in Export must be direct preview controls, not passive labels, once package inspection is opened. | Export handoff / artifact interaction | export direct-manipulation browser inspection | `ide:gate:export-artifact-direct-preview` proves the explicit generated-files disclosure opens, then `top.vhd` supports click selection and `top.xdc` supports keyboard selection, the preview path and selected `aria-pressed` update, the artifact workspace is revealed, root overflow is absent, and the browser makes no E1/E2/E3 overclaim at `1366x768` and `1440x900`. | arbitrary custom artifact sets remain future coverage if reopened | P1 |
...
| RB-INV-043 | The global shell and navigation must behave like compact tools around the workbench, not permanent scaffold that creates mini-scroll traps. | Shell/navigation/workbench | shell hierarchy, side-dock, and Verify layout gates | `ide:gate:shell-navigation-overhaul` proves the compact top bar, exactly five readable workflow stages, Import utility route/reload access, no proof ribbon/footer/product-spine header, workbench directly below the top bar, no root overflow, and no console/page errors at `1366x768` and `1440x900`. `ide:gate:primary-work-object-dominance` proves Design, Verify, Hardware, Export, and Import main work regions dominate the viewport and support docks are exclusive in focused workbench modes. `ide:gate:nested-scroll-regression` rejects meaningful Verify stimulus/waveform and Hardware workbench mini-scroll traps. | broader function-depth and card-heavy composition still require future browser-first product slices | P1 |
| RB-INV-044 | Verify post-run repair must keep editable checks usable beside waveform evidence. | Verify / stimulus workbench / evidence layout | Verify Workbench Rebuild and shell/navigation gates | `ide:gate:verify-postrun-workbench-usability` intentionally failed on the old `460px` post-run checks lane, was later strengthened when browser E0 proof found waveform evidence starting too low, and was strengthened again when fresh 1440x900 proof caught clipped Observe/Compare command labels. It now proves Compare PASS, induced expected-output FAIL, repair PASS, usable checks width/share, readable visible `Observe only` / `Compare checks` command labels, waveform minimum width, waveform evidence top offset, viewport-visible chart height, no mini-scroll trap, visible first-failing-check action, build-hash identity, no root overflow, and no console/page errors at `1366x768` and `1440x900`. Updated `ide:gate:primary-work-object-dominance`, `ide:gate:nested-scroll-regression`, and `ide:gate:workbench-space-utilization` keep the balanced evidence/repair contract through wider viewports. | direct Verify no-circuit composition and broader evidence clarity remain future product-review targets | P1 |
| RB-INV-045 | Empty or loaded entry states must present the dominant task before apparatus or static metrics, while secondary paths remain explicitly reachable. | Verify / Project entry surfaces | Project command center and Verify layout gates | `ide:gate:verify-no-circuit-task-first` requires the direct Verify no-circuit recovery actions while hiding inactive waveform/testbench apparatus. The migrated Project entry contract requires first launch to lead with one start action and loaded Project to lead with Continue/direct Verify; Change Project must reveal guarded Build Fresh, Course Starter, Import / Recover, and Open Recent paths. The gates retain build-hash, navigation, destructive-confirmation, overflow, and console checks at `1366x768` and `1440x900`. | broader direct-manipulation depth remains future product-review work | P1 |
...
| RB-INV-047 | Outer workflow surfaces must expose the dominant command/recovery/readiness decision before passive cards, with secondary tools behind explicit usable disclosures. | Project / Import / Export surfaces | Project command center, Import recovery, Export handoff station | `ide:gate:project-loaded-command-surface` protects Continue/direct Verify plus reachable Change Project alternatives, `ide:gate:import-guided-recovery-wizard` protects source choice and the no-overwrite boundary, and `ide:gate:export-package-inspector` protects readiness first plus explicit generated-file inspection and selected preview. `ide:gate:outer-workflow-action-density` and `ide:gate:card-chrome-regression` reject passive card chrome outranking those controls. | deeper direct manipulation and surface-specific function depth remain future browser-first targets | P1 |
...
Protects Export ready state from losing package inspection while keeping readiness first. It reaches a ready-to-build starter export, requires the explicit generated-files disclosure, opens it, and then requires the file browser, selected artifact preview, direct build/download/copy actions, no browser E1/E2/E3 overclaim, no root overflow, and console/page cleanliness at `1366x768` and `1440x900`.
...
Protects loaded Project from losing its real action paths behind metrics/support content. It loads the Logic Gates starter at `1366x768` and `1440x900`, returns to Project, verifies the visible build hash, requires Continue and direct Verify in the useful first viewport, opens Change Project to reach Build Fresh / Course Starter / Import Recover / Open Recent, proves Continue and Import navigation, verifies the loaded Build Fresh guard, rejects root overflow, and fails on console/page errors.
...
Protects the global shell from regressing back into scaffold-heavy navigation. It verifies the visible build hash against current Git HEAD, checks the exact Project/Design/Verify/Map Pins/Export rail at `1366x768` and `1440x900`, verifies Import remains reachable as a non-stage utility route and after reload, rejects the retired proof ribbon/footer/product-spine header, requires the workbench directly below the top bar, rejects root overflow, and fails on console/page errors.
...
Protects Export generated-file direct manipulation in the ready-to-build Logic Gates path. It verifies the visible build hash against current Git HEAD, reaches Verify Compare PASS, opens Export at `1366x768` and `1440x900`, opens the explicit generated-files disclosure, requires the revealed file cues to be real buttons with preview labels and keyboard reachability, clicks `top.vhd`, keyboard-activates `top.xdc`, checks preview-path updates and selected `aria-pressed`, rejects root overflow, and rejects browser-side E1/E2/E3 or Vivado/Basys3 overclaims.
...
Protects Export artifact discoverability for the ready-to-build Logic Gates path. It verifies the visible build hash against current Git HEAD, reaches Verify Compare PASS, opens Export at `1366x768` and `1440x900`, requires readiness and the `Inspect generated files` disclosure in the first viewport, opens it, then requires `README.txt`, `top.vhd`, `top.xdc`, `testbench.vhd`, and `vivado_import.tcl` plus the artifact explorer, and rejects browser-side E1/E2/E3 or Vivado/Basys3 overclaims.
...
Protects Project against starter-first or metric-first regression. It proves the no-circuit command center is neutral and first-viewport visible with one primary start action, Build Fresh / starter / saved-recent / Import-Recover paths are reachable through visible controls/disclosures, loaded Project keeps one primary Continue action plus direct Verify, loaded alternatives start collapsed behind Change Project, and loaded Build Fresh is confirmation-guarded.
...
Protects the shell reset boundary. It loads the Logic Gates starter and checks Project, Design, Verify, Hardware, Export, and Import at `1366x768` for the compact top bar, exact five-stage rail, Import utility boundary, no proof ribbon/footer/product-spine header, workbench top at the top-bar boundary, at most one page command header and one primary action, a visible focal object, and no root overflow.

This gate is now part of `classroom:gate` and `verify:gates:classroom`.

### `ide:gate:export-trust-integrity`

Protects Export handoff trust for the mapped/verified Logic Gates path. It proves Export reaches `READY TO BUILD`, builds the current bundle, opens generated-file inspection, downloads the Vivado Project ZIP, compares preview bodies with ZIP entries, checks README/provenance and E0/E1/E2/E3 boundary wording, and validates XDC/`EXPECTED_IO.json` evidence against mapped board I/O.

This gate is now part of `classroom:gate` and `verify:gates:classroom`.

### `ide:gate:export-handoff-station`

Protects Export station hierarchy for Draft, Ready-to-build, and Trusted post-download states. It proves exactly one visible readiness authority at `1366x768`, Draft is not package-ready and has a repair path, Ready/Trusted keep one build/download primary action, the explicit generated-files disclosure reveals the artifact workspace and README preview, expected Vivado package files are listed, mapping summaries agree, Vivado next steps are downstream, E1/E2/E3 are external/manual, and Export creates no root overflow or key-region overlap.
...
Protects the Verify evidence workbench for the Logic Gates starter path. It proves the first-run stimulus editor and expected-output cells remain visible, Observe-only records waveform evidence without trusted PASS/FAIL proof, Compare PASS works from saved checks, an expected-output edit creates an intentional Compare FAIL, the first mismatch shows expected/observed values near the waveform, and repairing the expected output returns to PASS. It also checks meaningful overlap among stimulus and waveform evidence regions at the classroom viewport.
...
Protects Project, Export, and Import entry surfaces from regressing into static card stacks. It verifies one dominant first-viewport action/readiness decision, reachable Project alternatives and Export generated-file inspection through explicit disclosures, Import recovery hierarchy, build-hash identity, no root overflow, and console/page cleanliness.
...
Protects release closeout proof from using stale screenshots or stale preview output. It refuses dirty tracked worktrees by default, then verifies the visible build badge and `/os/build.json` match current Git HEAD while walking Project, Design, Verify, Hardware, Export, and Import. Use it after the closeout commit and rebuild; it is not a replacement for focused defect gates or `classroom:gate`.
