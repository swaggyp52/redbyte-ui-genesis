...
| RB-INV-057 | Browser persistence must survive reload/corrupt storage and warn on multi-tab runtime-key writes without claiming a storage-facade migration. | Project persistence / runtime storage / session metadata | Phase 3 durability docs | `ide:gate:project-durability-v2`, `ide:gate:verify-corrupt-state-recovery-v2`, and `ide:gate:verify-multitab-conflict-v2` prove snapshot/index/runtime reload restore, malformed runtime/session storage recovery without stale PASS, and visible Reload/Dismiss warning when another tab writes the runtime key. | Journaled commits, rolling snapshots, quota UX, and full conflict resolution remain future work. | P1 |
