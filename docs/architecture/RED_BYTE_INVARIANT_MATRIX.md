...
| RB-INV-061 | Design V2 must be a canvas-first authoring workspace with contextual actions, not a default right-rail/card surface. | Design surface / logic canvas / workbench CSS | V2 workspace layout contract and prior Design gates | `ide:gate:design-workspace-v2` proves a fixed tool palette, compact context property bar, usable primary canvas, no default generic inspector, active-canvas `Ctrl+A` selection, selected context delete action, no root overflow, and no browser errors at `1366x768` and `1440x900`. | broader human keyboard/screen-reader pass remains future work; no Verify/simulation/generation semantics are changed | P1 |
| RB-INV-062 | Map Pins V2 must keep table-to-board binding visible and deterministic without overlaying or hiding pin consequences. | Hardware surface / Basys3 board / mapping runtime | V2 workspace layout contract and Hardware board gates | `ide:gate:map-pins-workspace-v2` proves table/board split geometry, selected row `aria-pressed`, board resource highlight, inline selected-row resource/XDC detail, board-resource click assignment, no root overflow, and no browser errors at `1366x768` and `1440x900`. | Vivado/Basys3 proof remains external; generated XDC and mapping truth are unchanged | P1 |
| RB-INV-063 | Project V2 must be a command center with one current action, compact workflow progress, and direct tool commands, not a metric-card dashboard. | Project surface / workflow authority | Phase 5 Project workspace contract and Project loaded command surface gate | `ide:gate:project-command-center-v2` proves compact workflow progress, direct Design / Verify / Map Pins / Export commands, secondary project actions, build identity, no root overflow, and no normal-surface raw hash/E-tier command-board copy at `1366x768` and `1440x900`. | deeper human visual review remains Phase 6; project format and storage bytes are unchanged | P1 |
| RB-INV-064 | Export V2 must be an artifact workspace with selected-file inspection and plain downstream Vivado handoff, without changing generated bytes. | Export surface / package inspector | Phase 5 Export workspace contract and existing export trust gates | `ide:gate:export-artifact-workspace-v2` proves artifact workspace, file tree, selected preview, copy/download actions, immediate XDC preview update, plain Vivado handoff boundary, no root overflow, and no normal-surface bench-classifier/E-tier copy in that boundary. `ide:gate:export-workspace-density-v2` additionally rejects repeated status-pill artifact rows, oversized file rows, weak selected-preview dominance, and broken direct XDC preview at `1366x768`, `1440x900`, and `1920x1080`. Existing export package/trust gates continue to prove preview/ZIP parity. | generated README/provenance bytes still carry existing proof-tier artifact wording until an explicit generator slice changes bytes | P1 |
| RB-INV-065 | Import V2 must show a guarded transactional recovery workflow and keep active project replacement behind review/apply. | Import surface / recovery workflow | Phase 5 Import workspace contract and existing import recovery gates | `ide:gate:import-step-workflow-v2` proves first-look and active five-step workflow visibility, no-overwrite boundary, active inspection summary, no root overflow, and classroom geometry compatibility with `ide:gate:import-guided-recovery-workflow`. | parser/apply semantics are unchanged; malformed/unsupported file breadth remains parser-specific future coverage | P1 |
| RB-INV-066 | Project, Verify, and Export must agree on the same outer workflow state after a normal Compare PASS. | Project workflow authority / Verify adapter / Export readiness | Phase 5 continuity gate | `ide:gate:outer-workflow-continuity-v2` loads a starter, verifies Project shows Verify not run, runs Compare PASS, verifies Project shows Verify passed, and verifies Export artifact workspace is available after the same state transition. | broader edit/stale/import continuity waves remain Phase 6 or classroom rehearsal work | P1 |
| RB-INV-067 | A from-scratch student circuit must materialize mapping rows and exportable package evidence without relying on starter state. | Design authoring / hardware mapping / Verify / Export / proof process | Phase 6B RedByte Jury primary trial | `ide:gate:jury-half-adder-visible-trial` proves visible Project rename, from-scratch Half Adder Design authoring, undo/redo, wiring, reload persistence, My-check authoring, intentional FAIL, repair PASS, stale/rerun PASS, Map Pins row-to-board linking for `A -> SW0`, `B -> SW1`, `SUM -> LD0`, `CARRY -> LD1`, post-map Verify PASS, Export package inspection/download, ZIP manifest, and Project reload/back-forward resume. Focused mapping tests prove scalar entries are created when the canonical V2 mapping document starts empty and pins are preserved when live rows have no replacement. | secondary jury trials for Course/My checks, sequential timing, recovery, import, and accessibility remain open before non-draft | P1 |

## New Gates In This Slice

### `ide:gate:project-command-center-v2`

Protects the Phase 5 Project command-center contract. It loads the Logic Gates starter, verifies current build identity, requires compact Project workflow progress and direct route commands, requires secondary project actions, rejects root overflow, and rejects normal command-board raw hash/E-tier copy at `1366x768` and `1440x900`.

This gate is now part of `classroom:gate` and `verify:gates:classroom`.

### `ide:gate:export-artifact-workspace-v2`

Protects the Phase 5 Export artifact-workspace contract. It reaches ready Export state, requires the artifact workspace, file tree, selected preview, copy/download selected-file actions, immediate XDC preview update, plain downstream Vivado handoff boundary, no root overflow, and no normal boundary bench-classifier/E-tier copy.

This gate is now part of `classroom:gate` and `verify:gates:classroom`.

### `ide:gate:export-workspace-density-v2`

Protects the Export artifact workspace from returning to a status-card list. It reaches ready Export state, captures before/after screenshots under `.redbyte/product-immersion/export-workspace-density/YYYY-MM-DD/`, rejects repeated artifact-row `ide-status-pill` chrome, requires compact file rows and a flattened handoff strip, keeps the selected preview wider/taller than the artifact tree, checks `top.xdc` direct preview, and rejects root overflow/browser errors at `1366x768`, `1440x900`, and `1920x1080`.

This gate is now part of `classroom:gate` and `verify:gates:classroom`.

### `ide:gate:import-step-workflow-v2`

Protects the Phase 5 Import recovery workflow contract. It checks first-look and active Paste HDL five-step workflow visibility, no-overwrite boundary copy, active inspection summary, no root overflow, and current build identity.

This gate is now part of `classroom:gate` and `verify:gates:classroom`.

### `ide:gate:outer-workflow-continuity-v2`

Protects cross-surface Phase 5 continuity. It verifies Project's Verify status before Compare, runs Verify Compare PASS, checks Project's passed state afterward, and then checks Export artifact workspace availability from the same browser state.

This gate is now part of `classroom:gate` and `verify:gates:classroom`.

### `ide:gate:design-workspace-v2`

Protects the Phase 4A Design workspace contract. It loads the Logic Gates starter, verifies current build identity, requires the fixed palette and compact context property bar, rejects a default-open generic inspector, requires useful canvas geometry, proves active-canvas `Ctrl+A` updates the context bar to selected objects with delete action, rejects root overflow, and fails on browser console/page errors at `1366x768` and `1440x900`.

This gate is now part of `classroom:gate` and `verify:gates:classroom`.

### `ide:gate:map-pins-workspace-v2`

Protects the Phase 4A Map Pins workspace contract. It loads the Logic Gates starter, verifies current build identity, requires a table/board split with the board beside the mapping table, selects SW0, verifies selected-row state and board highlight, requires inline selected-row resource/XDC detail, proves board-resource click assignment, rejects root overflow, and fails on browser console/page errors at `1366x768` and `1440x900`.
