...
Phase 3E adds the source-level Verify runtime integration gate plus rendered browser authority gates:
...
Phase 4A adds visible workspace reconstruction gates:
...
- Verify V2 authority cutover protects the Phase 3C rendered contract: Course checks are locked, My checks are editable only after duplication, result status/Project status/Export readiness come from the V2 model, and legacy rail/proof-language assumptions stay out of normal Verify. Verify authority Phase 3D extends this to stale reason/recovery copy, timing labels, selected My-check failure repair authority, visible expected-output repair, and repaired PASS. Sequential authority Phase 3E proves the Counter clocked path: auto board clock is generated/read-only, manual pulses expose the clock lane, timing edits stale the prior PASS, and Project/Export consume the same stale state.
- Phase 3F acceptance gates protect the branch from being visually improved but classroom-fragile: Verify control names/dialog semantics, keyboard-only expected-output edit/rerun, 125 percent zoom/contrast, saved snapshot/runtime reload, corrupt storage recovery, multi-tab runtime-key warning, and Diagnostics support bundle must remain green.
- Design workspace V2 protects Design from reverting to a rail/card-heavy surface by requiring a fixed palette, compact context property bar, usable canvas, no default generic inspector, active-canvas select-all, and selected context actions.
- Map Pins workspace V2 protects Hardware Map Pins from hiding pin consequences by requiring table/board split geometry, selected row state, board highlight, inline XDC/detail context, and board-resource assignment.
...
| Hardware/Map Pins layout, board obstruction, resource catalog placement, row-to-board binding detail, or E0 proof wording | `ide:gate:map-pins-workspace-v2`, `ide:gate:hardware-basys3-workbench`, `ide:gate:hardware-first-viewport`, `ide:gate:hardware-board-dominance`, `ide:gate:hardware-board-unblocked`, `ide:gate:hardware-resource-catalog-not-obstructing`, `ide:gate:workbench-obstruction-usability`, hardware browser screenshots, and mapping tests only if map state changes |
...
| Import first-look wizard, active recovery presentation, selected-source persistence, or editor/review hierarchy | `ide:gate:import-guided-recovery-wizard`, `ide:gate:import-guided-recovery-workflow`, `ide:gate:import-recovery-contract`, `ide:gate:release-solidification-v1`, Import before/after screenshots, and focused Import Vitest only when selector/action contracts change |
...
`verify:truth-integration-gate` and `ide:gate:verify-v2-authority-cutover` are the current Phase 3C authority gates. The integration gate runs the truth-state, runtime adapter, and Project workflow authority tests that prove Course/My check provenance, observe-only trust limits, current compare PASS, compare FAIL repair actions, stale design versus stale testbench classification, compare-error classification, Project/Export readiness consumption, and equivalence with `deriveProjectVerifyState`. The browser gate proves the rendered Verify UI locks Course checks, duplicates them into editable My checks, preserves Compare, and exposes V2 result/Project/Export authority.
