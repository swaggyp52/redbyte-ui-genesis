...
**Local-first operation.** All computation happens in the browser. No server is required, no account is needed, and no data leaves the user's machine unless explicitly exported. A network connection is still needed while workspaces load for the first time (workspace code is fetched on demand); RedByte is not an installable/offline (PWA) application.
