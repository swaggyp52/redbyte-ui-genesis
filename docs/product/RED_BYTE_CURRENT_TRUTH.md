...
| Product Trust Reset v2 Verify statechart/runtime adapter | `docs/architecture/ADR_VERIFY_STATE_ORCHESTRATION.md`; `docs/architecture/RED_BYTE_VERIFY_RUNTIME_INTEGRATION_V2.md`; `packages/rb-apps/src/apps/ide/verifyTruthState.ts`; `packages/rb-apps/src/apps/ide/verifyTruthAdapter.ts`; `verify:truth-integration-gate`; `ide:gate:verify-v2-authority-cutover`; `ide:gate:verify-authority-phase-3d`; `ide:gate:verify-sequential-authority-v2` | Current Phase 3E authority for legal Verify states, events, revisions, check provenance, locked expected outputs, stale invalidation/recovery copy, selected-failure repair authority, sequential timing authority, runtime-to-model adaptation, rendered Course/My check authority, and Project/Export selectors. |
| Product Trust Reset v2 Phase 3F proof | `docs/release/RED_BYTE_PHASE_3F_ACCEPTANCE_CRITERIA.md`; `docs/release/RED_BYTE_VERIFY_ACCESSIBILITY_REVIEW.md`; `docs/release/RED_BYTE_CLASSROOM_REHEARSAL.md`; `.redbyte/rehearsal/phase-3f/`; `ide:gate:verify-accessibility-v2`; `ide:gate:verify-keyboard-grid-v2`; `ide:gate:verify-zoom-contrast-v2`; `ide:gate:project-durability-v2`; `ide:gate:verify-corrupt-state-recovery-v2`; `ide:gate:verify-multitab-conflict-v2`; `ide:gate:diagnostics-bundle-v2` | Current browser E0 proof for Verify accessibility/keyboard/zoom, Diagnostics support bundle, reload durability, corrupt-storage recovery, multi-tab overwrite warning, and 30-context classroom rehearsal. |
| Product Trust Reset v2 Phase 3G gate authority | `scripts/gates/gate-manifest.mjs`; `.redbyte/proof/gate-reset/phase-3g/`; `gate:manifest:validate`; `gate:no-hardcoded-redbyte-test-ports`; `verify:gates:legacy`; `docs/release/RED_BYTE_PR78_MERGE_READINESS.md` | Current authority for which classroom gates are required, focused, retired with replacement, or legacy diagnostic. `classroom:gate` and `verify:gates:classroom` must agree through this manifest. |
| Product Trust Reset v2 Phase 3H project durability | `packages/rb-apps/src/apps/ide/projectStorageFacade.ts`; `docs/architecture/RED_BYTE_PROJECT_DURABILITY_MODEL.md`; `docs/architecture/ADR_PROJECT_DURABILITY_STORAGE.md`; `ide:gate:project-storage-facade-v2`; `ide:gate:atomic-save-journal-v2`; `ide:gate:project-schema-migration-v2`; `ide:gate:project-quota-recovery-v2`; `ide:gate:project-multitab-conflict-v2`; `ide:gate:dirty-update-guard-v2`; `ide:gate:project-recovery-workflow-v2`; `ide:gate:diagnostics-storage-v2`; `ide:gate:recovery-accessibility-v2`; `.redbyte/rehearsal/phase-3h/` | Current browser-local storage facade authority. Existing raw storage bytes stay compatible while journal, last-known-good, recovery points, quota recovery UI, backup export, diagnostics fields, and dirty/multi-tab guards are proven. No backend, cloud sync, project-format change, or hardware proof is implied. |
| Product Trust Reset v2 Phase 3I branch review | `docs/release/RED_BYTE_PR78_MERGE_READINESS.md`; `docs/release/RED_BYTE_PR78_PERFORMANCE_REVIEW.md`; `docs/release/RED_BYTE_PR78_SECURITY_PRIVACY_REVIEW.md`; `gate:project-storage-authority`; `rehearsal:classroom-fault-injection`; `.redbyte/rehearsal/phase-3i/` | Current draft PR #78 review posture. Decision is `C. READY FOR REVIEW, KEEP DRAFT` after final validation/GitHub green. Phase 3I adds source storage-authority proof, rehearsal fault injection, current-build assertions, performance/security reviews, and human AT script. It does not mark the PR non-draft or claim E1-E3 proof. |
| Product Trust Reset v2 Phase 4A Design and Map Pins workspaces | `docs/architecture/RED_BYTE_DESIGN_WORKSPACE_V2.md`; `docs/architecture/RED_BYTE_MAP_PINS_WORKSPACE_V2.md`; `docs/release/RED_BYTE_DESIGN_MAP_PINS_ACCESSIBILITY_REVIEW.md`; `ide:gate:design-workspace-v2`; `ide:gate:map-pins-workspace-v2`; `.redbyte/product-immersion/product-trust-reset-v2/phase-4/` | Current Design/Map Pins reconstruction posture. Design is fixed-palette, canvas-first, and context-bar driven with compact current-I/O state instead of a default live-I/O right rail. Map Pins is a table/board binding workspace with selected-row inline resource/XDC context. This is browser E0/UI evidence only and does not change Verify, simulation, mapping truth, generated bytes, project format, goldens, or hardware proof. |
| Product Trust Reset v2 Phase 5 Project, Export, and Import workspaces | `docs/architecture/RED_BYTE_PROJECT_WORKSPACE_V2.md`; `docs/architecture/RED_BYTE_EXPORT_WORKSPACE_V2.md`; `docs/architecture/RED_BYTE_IMPORT_WORKSPACE_V2.md`; `docs/release/RED_BYTE_OUTER_WORKFLOW_ACCESSIBILITY_REVIEW.md`; `ide:gate:project-command-center-v2`; `ide:gate:export-artifact-workspace-v2`; `ide:gate:import-step-workflow-v2`; `ide:gate:outer-workflow-continuity-v2`; `.redbyte/product-immersion/product-trust-reset-v2/phase-5/` | Current outer-workflow reconstruction posture. Project is a command center with compact workflow progress and direct tool commands. Export is an artifact workspace with file list, selected preview, copy/download selected-file actions, and plain downstream Vivado handoff. Import is a guarded five-step recovery workflow. This is browser UI evidence only and does not change Verify, simulation, mapping truth, import parser/apply semantics, generated bytes, project format, goldens, or hardware proof. |
| Product Trust Reset v2 Phase 6 pilot-readiness review | `docs/release/RED_BYTE_PILOT_WALKTHROUGH_PACKET.md`; `docs/release/RED_BYTE_ASSISTIVE_TECH_HUMAN_SCRIPT.md`; `docs/release/RED_BYTE_PR78_MERGE_READINESS.md`; `.redbyte/product-immersion/product-trust-reset-v2/phase-6/current-baseline/` | Current pilot-readiness posture. Browser audit at reviewed head `6c9575b8072134e062e746fad1be9bac43bd5c1c` found no P0/P1 blocker, 0 browser problems, and 0 root overflow findings across 27 screenshots. PR #78 remains draft. Human professor/student walkthrough and actual AT review remain required before any non-draft decision. |
| Product Trust Reset v2 Phase 6B RedByte Jury | `.agents/jury/`; `.agents/skills/redbyte-jury-orchestrator/SKILL.md`; `docs/development/RED_BYTE_JURY_REVIEW_PROCESS.md`; `docs/release/RED_BYTE_JURY_REVIEW_001.md`; `docs/release/RED_BYTE_JURY_RETRIAL_001.md`; `ide:gate:jury-half-adder-visible-trial`; `.redbyte/proof/jury/2026-06-23/` | Current standing agentic browser jury posture. Jury Review 001 found the initial primary proof invalid, selected a narrow mapping/proof fix package, and Jury Retrial 001 completed the visible from-scratch Half Adder workflow through Design, Verify, Map Pins, Export, and Project recovery. Verdict is `READY WITH FIXES / KEEP DRAFT`; secondary trials plus human review and actual AT review remain required before non-draft. |
| Product Trust Reset v2 Export workspace density | `ide:gate:export-workspace-density-v2`; `.redbyte/product-immersion/export-workspace-density/2026-06-23/`; `docs/architecture/RED_BYTE_EXPORT_WORKSPACE_V2.md` | Current Export visual-density guard. The generated artifact tree must stay compact, avoid repeated status-pill bars, keep the selected-file preview dominant, preserve direct file preview behavior, and make no generated-byte or hardware-proof claims. |
...
Phase 6B branch truth: Verify has a pure truth-state foundation in `verifyTruthState.ts`, a runtime adapter in `verifyTruthAdapter.ts`, rendered Verify authority for Course checks versus My checks, and Phase 3F browser E0 proof for accessibility/durability/rehearsal. Phase 3G resets gate authority so the required `classroom:gate` and broader `verify:gates:classroom` both run from `scripts/gates/gate-manifest.mjs`; retired V1 structural checks are documented in `verify:gates:legacy` with current replacements and are not merge requirements. Phase 3H adds the browser-local project storage facade, journal, last-known-good recovery, bounded recovery points, quota recovery UI, diagnostics storage fields, dirty update guard, and storage-wave rehearsal evidence. Phase 3I adds source-level project storage authority proof, current-build assertions in the rehearsal harness, deliberate rehearsal fault injection, PR78 performance/security reviews, and a merge-readiness decision of `C. READY FOR REVIEW, KEEP DRAFT`. Phase 4A closes local Design and Map Pins V2 workspace reconstruction. Phase 5 closes local Project, Export, and Import V2 workspace reconstruction with four new outer-workflow gates; local closeout passed `classroom:gate` (`92/92`) and `verify:gates:classroom` (`111/111`) under Node `20.19.0`. Phase 6 adds a pilot-readiness browser audit, walkthrough packet, and human AT script while keeping PR #78 draft. Phase 6B adds the standing RedByte Jury and proves the primary visible from-scratch Half Adder path after a narrow mapping/proof package; secondary jury trials and human review remain open. The later Export density hardening adds `ide:gate:export-workspace-density-v2` to keep the Export artifact tree compact and the selected preview dominant without changing generated files. This is browser E0 and test-authority evidence only; backend/cloud sync, full collaborative conflict resolution, screen-reader certification, and Vivado/Basys3 E1-E3 proof remain separate.
...
- Verify post-run testbench usability is covered by `ide:gate:verify-postrun-workbench-usability`, including Compare PASS, induced expected-output FAIL, repaired PASS, usable editable checks width/share, waveform width, waveform evidence top offset, viewport-visible chart height, no mini-scroll trap, visible first-failing-check action, build-hash verification, no root overflow, and no console/page errors at `1366x768` and `1440x900`.
...
The next PR #78 step is secondary jury proof plus human review, not another automatic polish slice: run Course/My checks, sequential timing, recovery, import, and agentic accessibility jury trials, then run the professor/student pilot walkthrough and the actual AT script before deciding whether the draft PR can be marked non-draft. If a new product-polish slice is later approved, select it only after live app inspection. Do not choose the target from this queue alone.

```text
PR78 jury secondary trials, human walkthrough, actual AT session, support boundary, non-draft decision
```
