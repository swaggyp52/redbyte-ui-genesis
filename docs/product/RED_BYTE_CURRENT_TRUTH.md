...
Phase 3F branch truth: Verify now has a pure truth-state foundation in `verifyTruthState.ts`, a runtime adapter in `verifyTruthAdapter.ts`, and rendered Verify authority for Course checks versus My checks, locked expected-output cells, duplicate-to-My-checks editing, V2 result status/currentness, stale reason/recovery copy, selected-failure repair authority, sequential timing authority, Project verify status, and Export readiness. Phase 3F adds proof that the rendered model is keyboard-operable in the core expected-output loop, visible at 125 percent zoom, recoverable after malformed browser storage, and durable across reload for named saved projects. It also adds Diagnostics support-bundle proof and a visible warning when another tab writes the runtime project storage key. The three Phase 3F classroom rehearsals passed `30/30` isolated browser contexts each under `.redbyte/rehearsal/phase-3f/`. This is browser E0 evidence only; storage facade, rolling snapshots, quota-risk UX, human assistive-technology review, and Vivado/Basys3 E1-E3 proof remain separate.
