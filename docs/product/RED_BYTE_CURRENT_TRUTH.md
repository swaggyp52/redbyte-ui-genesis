...
- Current Git source posture on 2026-07-04: GitHub `origin/main` advanced to `27c61b0b81aa70ea00fc52aa0b047abb9fc55067` with the Design wiring simplification patch. Deploy passed for that SHA, but `Classroom Truth Gates` failed in `ide:gate:authoring-depth-release-safety`; local `main` was rebased normally and now adds rebased `cd6bb9f13` Vivado E1 run kit plus this docs checkpoint. The earlier `9a639a43` "latest" output was stale context, not a remote regression; `d235823a`, `fdd1abd9`, and `fdf17b77` are ancestors of current remote main.
