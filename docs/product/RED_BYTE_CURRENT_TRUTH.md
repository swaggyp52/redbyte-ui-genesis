...
| Release-candidate decision | `docs/product/RED_BYTE_RELEASE_CANDIDATE_DECISION.md` | Current E0 browser release-candidate posture, Node 20 blocker, final-SHA discipline, and not-shippable items. |
...
- Release Candidate Decision / Mode History + Node 20 Status is closed locally: full Project/Verify release-candidate audit found a normal browser-history release blocker, then `IdeApp` mode URL sync was hardened so in-app mode changes create restorable history entries and `popstate` restores the active surface. The decision report keeps the current status honest: browser E0 is approaching release-candidate quality, but final deployed-SHA proof, pinned Node `20.19.0` validation, and remaining Project/Verify visual maturity work are still required before packageable release claims.
...
- Repo-pinned Node in `.nvmrc`: `20.19.0`; pinned-runtime proof remains environment-gated because this shell exposes Node `v24.15.0` and no installed `nvm`/`fnm`/`volta`/`nvs`/`nodist` switcher.
