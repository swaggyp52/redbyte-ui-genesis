...
- Summer Rescue Sprint / Testbench Editor Simplification + Export Confidence is closed locally by `ide:gate:testbench-editor-and-export-confidence-flow`: Verify first-run now names Inputs to try, Expected outputs, Observed outputs, and Status; failed Compare repair exposes single-cell, selected-row, and all-failed-output `Use observed` scopes; stale expected-output edits remain stale until rerun; Export now has a confidence station for Verify evidence, pin mapping, package trust/ready E0 state, Vivado build, and board behavior. This is browser E0 proof at `1366x768` only, is wired into `classroom:gate` and `verify:gates:classroom`, is not pushed/deployed by this local slice, and does not prove Vivado E1, bitstream E2, or board observation E3.
