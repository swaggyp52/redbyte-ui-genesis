...
Phase 3B branch truth: Verify now has a pure truth-state foundation in `verifyTruthState.ts` and a runtime adapter in `verifyTruthAdapter.ts` that maps existing runtime/scenario/hash records into state plus Project/Export selectors. The rendered Verify UI has not yet been rebuilt on top of it. Course/My check presentation, locked expected-output controls, stale/PASS/FAIL repair hierarchy, and browser gates for that rebuilt workbench remain open.
