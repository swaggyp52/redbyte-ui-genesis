...
| 4 | Export `F-E1` / `F-E2` trust language | Export still repeats the repair story and blurs summary vs action | `docs/RED_BYTE_IDE_PRODUCT_FLOW_MODEL.md`, `docs/ide/04-export.md`, `docs/IDE_PRODUCT_DEBT_REGISTER.md` | `fix:` or `ide:` | Export shows one primary trust story, one primary fix path, and no redundant warning copy; focused validation passes |
| 5 | Map Pins `F-H2` / `F-H3` trust language | Students still see stale guidance and warning color without a clear resolution path | `docs/RED_BYTE_IDE_PRODUCT_FLOW_MODEL.md`, `docs/ide/04-export.md`, `docs/ide/01-project.md`, `docs/IDE_PRODUCT_DEBT_REGISTER.md` | `fix:` or `ide:` | Complete mappings collapse stale guide copy; `NEEDS REVIEW` explains the real fix path; focused validation passes |
