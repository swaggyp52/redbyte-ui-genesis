...
- Do not mix shell reset, Verify workbench, Verify saved-check run intent, Project command center, route/reload recovery, Export handoff, Export first-viewport artifact visibility, Hardware, Hardware first-viewport hierarchy, Design, lab profile, Import, visual finish, quickstarts, Vivado proof, or commercialization work unless a direct dependency is proven.
