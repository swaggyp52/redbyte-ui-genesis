...
| RB-IMPORT-ACCESS-001 | P2 | Project / Import | Loaded Project did not expose an obvious Import utility entry point while the manual still says the left rail includes Import. | Students or professors may not know how to recover/import once a project is already loaded. | Resolve whether Import is a Project utility, route, command, or removed rail item; update manual/product contract and add access gate. | `IdeApp.tsx`; `ProjectSurface.tsx`; `ImportSurface.tsx`; manual docs | New `ide:gate:import-utility-access` after product contract decision. | Project entry fixed / broader Import deferred |
