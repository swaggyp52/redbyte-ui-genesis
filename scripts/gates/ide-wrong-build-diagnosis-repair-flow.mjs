...
const ARTIFACT_ROOT = path.join(
  ...
  '.redbyte',
  'product-immersion',
  'wrong-build-diagnosis-repair-flow',
);
...
await runIdeGate('IDE wrong-build diagnosis repair flow satisfied', async ({ page, baseUrl }) => {
  const browserProblems = captureBrowserProblems(page);
  await installCleanStudentContext(page);

  const record = {
    gate: 'ide-wrong-build-diagnosis-repair-flow',
  ...
  await page.goto(`${baseUrl}/?mode=project&e2e=1&gate=wrong-build-diagnosis-repair-flow`, {
  ...
  await renameProject(page, 'RB Wrong Build Diagnosis Repair Flow');
  await revealDesignLibrary(page);
  await setDesignZoomPreset(page, '50');
  const nodes = await buildWrongXorCircuit(page, record);
  await capture(page, '01-wrong-or-design.png');

  await openMode(page, baseUrl, 'verify', 'wrong-build-diagnosis-repair-flow');
  await assertSurfaceSafe(page, 'Verify startup');
  const cases = await buildXorCases(page);
  await authorInputCases(page, cases);
  await authorExpectedCases(page, cases);
  await capture(page, '02-xor-expected-authored.png');

  await runCompareAndExpect(page, 'wrong OR against XOR expectations', 'fail');
  ...
  await capture(page, '03-compare-fail-design-repair-choice.png');
  ...
  await assertDesignFailureContext(page, nodes);
  await capture(page, '04-design-driver-context.png');

  await page.getByTestId('ide-design-debug-context-focus-driver').click();
  ...
  await page.waitForSelector('[data-testid="ide-design-swap-group"]', { timeout: 8000 });
  await page.getByTestId('ide-design-swap-xor').click();
  await page.waitForFunction(
    (nodeId) =>
      window.__RB_PROJECT_RUNTIME__?.getState?.()?.circuit?.nodes?.some(
        (node) => node.id === nodeId && node.type === 'XOR',
      ),
    nodes.wrongOr,
    { timeout: 8000 },
  );
  await capture(page, '05-or-swapped-to-xor.png');

  await page.getByTestId('ide-design-replay-context-return').click();
  await page.waitForSelector('[data-testid="ide-mode-verify"]', { timeout: 15000 });
  await page.waitForTimeout(250);
  await capture(page, '06-verify-after-design-repair-return.png');
  const designStaleActionVisible = await page
    .getByTestId('ide-verify-stale-keep-reference')
    .isVisible()
    .catch(() => false);
  if (!designStaleActionVisible) {
    const debugState = await page.evaluate(() => ({
      primaryStatus: document.querySelector('[data-testid="ide-verify-primary-status"]')?.textContent ?? '',
      summaryStatus: document.querySelector('[data-testid="ide-verify-summary-status"]')?.textContent ?? '',
      repairPanel: document.querySelector('[data-testid="ide-verify-repair-panel"]')?.textContent ?? '',
      runButton: document.querySelector('[data-testid="ide-vcb-run"]')?.textContent ?? '',
      mode: document.querySelector('[data-testid^="ide-mode-"]')?.getAttribute('data-testid') ?? '',
    }));
    throw new Error(`Verify must show design-stale Rerun Compare action after design repair: ${JSON.stringify(debugState)}`);
  }
  const staleText = await text(page.locator('[data-testid="ide-verify-primary-status"]'));
  assert(
    /Design changed|circuit changed|Rerun Compare|saved checks/i.test(staleText),
    `Verify must show stale evidence after design repair, got "${staleText}"`,
  );
  await capture(page, '07-verify-stale-after-design-repair.png');

  await page.getByTestId('ide-verify-stale-keep-reference').click();
  await waitForVerifyResult(page, { timeout: 20000 }).catch((error) => {
    throw new Error(`design repair rerun: Verify result did not settle: ${error instanceof Error ? error.message : String(error)}`);
  });
  const status = await text(page.locator('[data-testid="ide-verify-summary-status"]'));
  assert(isVerifyPass(status), `repaired XOR Compare should PASS, got "${status}"`);
  await capture(page, '08-repaired-compare-pass.png');

  await openMode(page, baseUrl, 'export', 'wrong-build-diagnosis-repair-flow-current-export');
  ...
  await capture(page, '09-export-current-e0-boundary.png');

  record.phases.push({ phase: 'complete', repairedNode: nodes.wrongOr });
  await writeFile(path.join(ARTIFACT_ROOT, 'wrong-build-diagnosis-repair-flow.json'), JSON.stringify(record, null, 2));
...
async function buildWrongXorCircuit(page, record) {
  const nodes = {};
  ...
  nodes.wrongOr = await placeAndLabel(page, '[data-testid="ide-design-palette-or"]', 'wrong_or_should_be_xor', { x: 0.48, y: 0.46 });
  nodes.XorOut = await placeAndLabel(page, '[data-testid="ide-design-board-output-ld0"]', 'XOR_OUT', { x: 0.84, y: 0.46 });

  const wires = [
    [nodes.A, 'out', nodes.wrongOr, 'a'],
    [nodes.B, 'out', nodes.wrongOr, 'b'],
    [nodes.wrongOr, 'out', nodes.XorOut, 'in'],
  ];
  ...
  assert(circuit.nodeCount >= 4, `wrong-build circuit must have at least 4 nodes, got ${circuit.nodeCount}`);
  assert(circuit.connectionCount === wires.length, `wrong-build circuit should have ${wires.length} wires, got ${circuit.connectionCount}`);
  record.phases.push({ phase: 'design', nodes: circuit.nodeCount, connections: circuit.connectionCount });
  await fitCenterZoom(page);
  return nodes;
}

async function buildXorCases(page) {
  const rows = await readIoRows(page);
  const ids = {
    A: requireRowIdByLabel(rows, 'A'),
    B: requireRowIdByLabel(rows, 'B'),
    XOR_OUT: requireRowIdByLabel(rows, 'XOR_OUT'),
  };
  return [
    { name: '00', inputs: { [ids.A]: 0, [ids.B]: 0 }, expected: { [ids.XOR_OUT]: 0 } },
    { name: '01', inputs: { [ids.A]: 0, [ids.B]: 1 }, expected: { [ids.XOR_OUT]: 1 } },
    { name: '10', inputs: { [ids.A]: 1, [ids.B]: 0 }, expected: { [ids.XOR_OUT]: 1 } },
    { name: '11', inputs: { [ids.A]: 1, [ids.B]: 1 }, expected: { [ids.XOR_OUT]: 0 } },
  ];
}
...
async function assertWrongBuildRepairPanel(page, options) {
  ...
  assert(/expected value is correct|circuit issue|design repair|gate or wire/i.test(panelText), `repair panel must tell students when to inspect the circuit, got "${panelText}"`);
  ...
  for (const testId of [
    'ide-verify-repair-testbench-path',
    'ide-verify-repair-design-path',
    'ide-verify-repair-edit-expected',
    'ide-verify-repair-use-observed',
    'ide-verify-repair-open-design',
    'ide-verify-repair-rerun',
  ]) {
    assert(await page.getByTestId(testId).first().isVisible().catch(() => false), `repair action/context ${testId} must be visible`);
  }
}

async function assertDesignFailureContext(page, nodes) {
  const banner = page.locator('[data-testid="ide-design-debug-context-banner"]').first();
  const bannerText = await text(banner);
  assert(/XOR_OUT/i.test(bannerText), `Design failure banner must name failed output, got "${bannerText}"`);
  assert(/expected\s*0/i.test(bannerText), `Design failure banner must show expected 0, got "${bannerText}"`);
  assert(/observed\s*1/i.test(bannerText), `Design failure banner must show observed 1, got "${bannerText}"`);
  assert(/A=1/i.test(bannerText) && /B=1/i.test(bannerText), `Design failure banner must show input vector, got "${bannerText}"`);
  assert(/Check the gate or wire/i.test(bannerText), `Design failure banner must guide circuit inspection, got "${bannerText}"`);
  assert(/Driver/i.test(bannerText) && /OR/i.test(bannerText), `Design failure banner must show direct OR driver facts, got "${bannerText}"`);
  assert(/Incoming wires/i.test(bannerText) && /Outgoing wires/i.test(bannerText), `Design failure banner must show driver wire counts, got "${bannerText}"`);

  const selection = await page.evaluate(() => {
    const state = window.__RB_LOGIC_VIEW_STORE__?.getState?.()?.selection;
    return {
      nodes: Array.from(state?.nodes ?? []),
      wires: Array.from(state?.wires ?? []),
    };
  });
  const expectedWireId = `${nodes.wrongOr}.out-${nodes.XorOut}.in`;
  assert(
    selection.nodes.includes(nodes.XorOut) || selection.wires.includes(expectedWireId),
    `Inspect Design should select failed output or its incoming wire; got ${JSON.stringify(selection)}, expected ${nodes.XorOut} or ${expectedWireId}`,
  );
}
