...
async function assertPartialBlankAuthoring(page, viewport, options = {}) {
  ...
  const metrics = await page.evaluate(() => {
    ...
    return {
      ...
      quickVerifyVisible: Boolean(document.querySelector('[data-testid="ide-design-quick-open-verify"]')),
...
async function assertProjectContinuity(page, viewport) {
  ...
  const state = await page.evaluate(() => ({
    ...
    hasDirectMap: Boolean(document.querySelector('[data-testid="ide-project-command-action-map-pins"]')),
    hasDirectExport: Boolean(document.querySelector('[data-testid="ide-project-command-action-export"]')),
    ...
    hasContinue: Boolean(document.querySelector('[data-testid="ide-project-path-continue"]')),
    nextStep: document.querySelector('[data-testid="ide-project-command-strip-next-step-copy"]')?.textContent?.replace(/\s+/g, ' ').trim() ?? '',
  }));
  assert(state.hasDirectDesign && state.hasDirectVerify, `${viewport.label}: Project lost direct Design/Verify continuity ${JSON.stringify(state)}`);
