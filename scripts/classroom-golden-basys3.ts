...
const fixturePath = join(repoRoot, 'packages/rb-apps/src/fixtures/classroom/golden-basys3-switch-and.rbproj');
const goldenShaPath = join(repoRoot, 'packages/rb-apps/src/__tests__/__goldens__/golden-basys3-switch-and.zip.sha256');
const outPath = join(repoRoot, 'out/classroom/golden-basys3-switch-and.zip');
...
function fail(message: string): never {
  console.error(`[classroom:golden-basys3] FAIL: ${message}`);
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`[classroom:golden-basys3] artifact: ${outPath}`);
  console.log(`[classroom:golden-basys3] sha256:   ${sha}`);
  console.log(`[classroom:golden-basys3] expected: ${expectedSha}`);
  console.log(`[classroom:golden-basys3] result:   ${pass ? 'PASS' : 'FAIL'}`);
