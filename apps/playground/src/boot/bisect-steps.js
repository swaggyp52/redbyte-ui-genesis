...
export async function runBisect(step) {
    const root = document.getElementById('root');
    const renderMarker = (n, children) => {
        const marker = React.createElement('div', { id: 'boot-bisect', 'data-step': n, style: { padding: 16, color: '#9ae6b4' } }, `BOOT_BISECT STEP ${n}`, children);
    ...
    try {
        switch (step) {
            ...
            case 10: {
                ...
                const TestCanvas = ({ id, color }) => {
                    return React.createElement(CanvasHost, {
                        id,
                ...
                renderMarker(10, React.createElement('div', { style: { display: 'flex', gap: 20, marginTop: 20 } }, React.createElement(TestCanvas, { id: 'left', color: '#1e293b' }), React.createElement(TestCanvas, { id: 'right', color: '#334155' })));
                break;
            }
            default:
                renderMarker(step);
        ...
        const errorDiv = React.createElement('div', { id: 'boot-bisect', 'data-step': step, style: { padding: 16, color: '#fca5a5' } }, `BOOT_BISECT STEP ${step} ERROR: ${String(e)}`);
