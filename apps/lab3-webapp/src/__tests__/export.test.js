...
import { exportAsJSON, exportAsCSV, exportAsZip, generatePDFReport, } from '../export';
// Mock LabDoc for testing
const createMockLabDoc = () => ({
    schemaVersion: 2,
    meta: {
        id: 'test-doc-1',
        name: 'Test Lab 3',
...
describe('Export Module', () => {
    ...
    describe('generatePDFReport', () => {
        ...
        it('should handle missing student name gracefully', async () => {
            ...
            delete doc.meta.studentName;
            const dataUrl = await generatePDFReport(doc);
            expect(dataUrl).toMatch(/^data:application\/pdf;base64,/);
        });
        it('should include validation status in report', async () => {
            const doc = createMockLabDoc();
            doc.results.validation.canAdvance = false;
            doc.results.validation.message = 'Errors found ❌';
            const dataUrl = await generatePDFReport(doc);
            expect(dataUrl).toMatch(/^data:application\/pdf;base64,/);
        });
    });
