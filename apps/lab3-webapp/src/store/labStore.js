...
const useLabStore = create((set, get) => ({
    ...
    fillHexDigits: () => {
        ...
        get().updateDoc((doc) => ({
            ...
            truthTable: doc.truthTable.map((row, i) => ({
                ...row,
                seg: HEX_PATTERNS[i] ?? [1, 1, 1, 1, 1, 1, 1],
                isDontCare: false,
            })),
    ...
    fillParityBit: () => {
        ...
        get().updateDoc((doc) => ({
            ...
            truthTable: doc.truthTable.map((row, i) => {
                if (i >= 8)
                    return { ...row, isDontCare: true, seg: [1, 1, 1, 1, 1, 1, 1] };
                const parity = (row.b2 ^ row.b1 ^ row.b0);
                return {
                    ...row,
                    isDontCare: false,
                    seg: [parity, 1, 1, 1, 1, 1, 1],
                };
            }),
    ...
    fillPassthrough: () => {
        ...
        get().updateDoc((doc) => ({
            ...
            truthTable: doc.truthTable.map((row) => ({
                ...row,
                isDontCare: false,
                seg: [row.b0, row.b1, row.b2, row.b3, 1, 1, 1],
            })),
    ...
    parseVerilogCase: (code) => {
        ...
        if (matchCount > 0) {
            get().updateDoc((doc) => ({ ...doc, truthTable: newTable }), 'verilog.import', { lineCount: matchCount });
            set({ implMode: 'verilogCase', verilogCode: code });
        }
        return matchCount;
    },
    parseVhdlCaseStatement: (code) => {
        ...
        if (matchCount > 0) {
            get().updateDoc((doc) => ({ ...doc, truthTable: newTable }), 'vhdl.import', { lineCount: matchCount });
        }
        return matchCount;
    },
