...
import { Lab3State, TruthTableRow, createEmptyTruthTable, DIGIT_PATTERNS, inputToNumber, segToNumber, ValidationResult, KMapState, WaveformSample } from './types';
...
interface Lab3Actions {
  ...
  regenerateKMapFromExpr: (segmentName: string) => void;
...
export const useLabStore = create<Lab3State & Lab3Actions>((set, get) => ({
  ...
  generateKMaps: () => {
    ...
    const newKMaps: KMapState = { ...state.kMaps };

    for (let i = 0; i < segmentNames.length; i++) {
      const segName = segmentNames[i];
      const kmapGrid = generateKMapGrid(state.truthTable, i as 0 | 1 | 2 | 3 | 4 | 5 | 6);
  ...
  regenerateKMapFromExpr: (segmentName) => {
    ...
    const state = get();
    const expr = state.booleanExpressions[segmentName as keyof typeof state.booleanExpressions];

    // Evaluate expression for all 16 inputs
    const newGrid = Array(16) as (0 | 1 | 'X')[];
    for (let i = 0; i < 16; i++) {
    ...
    set((state) => ({
      kMaps: {
        ...state.kMaps,
        [segmentName]: {
          ...state.kMaps[segmentName as keyof KMapState],
          grid: newGrid,
        },
      },
    }));
  ...
  runAllVectors: () => {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      waveforms.push({
        time: i,
        inputs: [b3 as 0 | 1, b2 as 0 | 1, b1 as 0 | 1, b0 as 0 | 1],
        outputs,
      });
    }
  ...
  performLiveValidation: () => {
    const state = get();
    const errors: { [segmentName: string]: string[] } = {};
    ...
    for (let segIdx = 0; segIdx < segmentNames.length; segIdx++) {
      ...
      const expr = state.booleanExpressions[segName as keyof typeof state.booleanExpressions];
...
`;
    ...
    const segmentNames = ['a', 'b', 'c', 'd', 'e', 'f', 'g'];
    for (let i = 0; i < segmentNames.length; i++) {
      const expr = state.booleanExpressions[segmentNames[i] as keyof typeof state.booleanExpressions];
