...
`;
  ...
  const generateXdcContent = (): string => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-slate-50 rounded-lg p-6">
        ...
        <button
          onClick={() => {
            const matched = parseVerilogCase(pastedVerilog);
            if (matched === 0) {
              setImportResult({
                ok: false,
                message: `No rows matched. Check format — expected: 4'b0110: seg = 7'b1010101;`,
              });
            } else {
              setImportResult({ ok: true, message: `Imported ${matched} row${matched !== 1 ? 's' : ''}. Truth table updated.` });
              setPastedVerilog('');
            }
          }}
