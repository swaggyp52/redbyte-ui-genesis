...
import { inputToNumber } from './types';
...
`;
  ...
  return (
    <div style={{ padding: '20px' }}>
      <h2>Verilog Helper & Export</h2>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', marginTop: '20px' }}>
        <div>
          <h3>Generated Verilog (from Truth Table)</h3>
          <textarea
            value={generateVerilog()}
            readOnly
            style={{
              width: '100%',
              height: '400px',
        ...
        <div>
          <h3>Paste Vivado Case Statement</h3>
          <textarea
            value={pastedVerilog}
            onChange={(e) => setPastedVerilog(e.target.value)}
            placeholder="Paste your case statement from Vivado..."
            style={{
              width: '100%',
              height: '250px',
              fontFamily: 'monospace',
      ...
      <div style={{ marginTop: '40px' }}>
        ...
        >
          Download ZIP (truth_table.json + generated_ssd_driver.v + results.json + README)
        </button>
        <p style={{ marginTop: '10px', fontSize: '12px', color: '#666' }}>
          This ZIP contains everything you need to submit for Lab 3.
        </p>
      </div>
    </div>
  );
