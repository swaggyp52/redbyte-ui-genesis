...
import type { ParsedVhdlEntity } from './circuit-designer-pro/circuitToVhdl';
...
export const VerilogExporter: React.FC = () => {
  ...
  const [parsedEntity, setParsedEntity] = useState<ParsedVhdlEntity | null>(null);
  const [detectedLang, setDetectedLang] = useState<'vhdl' | 'verilog' | 'unknown'>('unknown');
  const [showExamples, setShowExamples] = useState(false);

  // ─── HDL auto-detect and port preview ────────────────────────────────────
  function detectLang(code: string): 'vhdl' | 'verilog' | 'unknown' {
    if (/entity\s+\w+\s+is/i.test(code)) return 'vhdl';
    if (/module\s+\w+|4'b[01]{4}/.test(code)) return 'verilog';
    return 'unknown';
  }
...
`;
  ...
  const generateVerilogAssign = (): string => {
    ...
    for (let i = 0; i < segNames.length; i++) {
      const expr = booleanExpressions[segNames[i] as keyof typeof booleanExpressions];
      if (expr) {
  ...
  const generateXdcContent = (): string => generateXdcString();
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-slate-50 rounded-lg p-6">
        ...
        <div className="space-y-4 mb-6">
          <div>
            <label className="block text-sm text-slate-300 mb-2">Module Name:</label>
            <input
              type="text"
              value={modulePrefix}
              onChange={(e) => setModulePrefix(e.target.value)}
              className="w-full max-w-xs bg-slate-800 text-slate-50 border border-slate-700 rounded px-3 py-2 focus:outline-none focus:border-blue-500"
              placeholder="ssd_driver"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Case Statement */}
          <div>
            <h4 className="font-semibold text-blue-300 mb-3">Case Statement Style</h4>
            <textarea
              value={generateVerilogCaseStatement()}
              readOnly
              className="w-full h-80 bg-slate-900 text-slate-50 border border-slate-700 rounded p-3 font-mono text-xs focus:outline-none overflow-auto color-scheme-dark"
            />
            <button
              onClick={() => copyToClipboard(generateVerilogCaseStatement())}
              className="mt-3 px-4 py-2 bg-blue-700 hover:bg-blue-600 rounded flex items-center gap-2 text-sm font-medium transition-colors"
            >
              <Copy size={16} />
              Copy to Clipboard
            </button>
          ...
          {/* Boolean Assignment */}
          <div>
            <h4 className="font-semibold text-green-300 mb-3">Boolean Assignment Style</h4>
            <textarea
              value={generateVerilogAssign()}
              readOnly
              className="w-full h-80 bg-slate-900 text-slate-50 border border-slate-700 rounded p-3 font-mono text-xs focus:outline-none overflow-auto color-scheme-dark"
            />
            <button
              onClick={() => copyToClipboard(generateVerilogAssign())}
              className="mt-3 px-4 py-2 bg-green-700 hover:bg-green-600 rounded flex items-center gap-2 text-sm font-medium transition-colors"
            >
              <Copy size={16} />
              Copy to Clipboard
            </button>
      ...
      <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-slate-50 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 text-sm px-3 py-1.5 bg-slate-700 hover:bg-slate-600 rounded border border-slate-600 text-slate-300 transition-colors"
            >
              Examples
              <ChevronDown size={14} className={showExamples ? 'rotate-180 transition-transform' : 'transition-transform'} />
            </button>
            {showExamples && (
              <div className="absolute right-0 top-full mt-1 z-10 bg-slate-800 border border-slate-600 rounded-lg shadow-xl w-52 py-1">
                {IMPORT_EXAMPLES.map((ex) => (
                  <button
                    key={ex.label}
                    onClick={() => { handlePasteChange(ex.code); setShowExamples(false); }}
                    className="w-full text-left px-3 py-2 text-sm hover:bg-slate-700 text-slate-300 flex items-center justify-between gap-2"
                  >
                    <span>{ex.label}</span>
                    <span className={`text-xs px-1.5 py-0.5 rounded font-mono ${ex.lang === 'vhdl' ? 'bg-cyan-900/60 text-cyan-400' : 'bg-green-900/60 text-green-400'}`}>
                      {ex.lang}
                    </span>
                  </button>
                ))}
        ...
        <p className="text-slate-300 text-sm mb-3">
          Paste Verilog or VHDL from your Vivado implementation — auto-detected, then imported into your truth table.
        </p>

        {/* Language detection badge */}
        {detectedLang !== 'unknown' && (
          <div className={`mb-2 inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border font-mono ${
            detectedLang === 'vhdl'
        ...
        <textarea
          ...
          placeholder="Paste Verilog case/endcase or VHDL case architecture here..."
          className="w-full h-40 bg-slate-900 text-slate-50 border border-slate-700 rounded p-3 font-mono text-sm focus:outline-none focus:border-purple-500"
        />

        {/* Schematic preview — shown when VHDL entity is parsed */}
        {parsedEntity && (
          <div className="mt-3 flex items-center gap-4 p-4 bg-slate-950/60 rounded-lg border border-slate-700">
            {/* Inputs */}
            <div className="space-y-1.5 min-w-[60px]">
              {parsedEntity.inputs.map(p => (
                <div key={p} className="flex items-center gap-2">
                  <span className="font-mono text-xs text-cyan-300">{p}</span>
                  <div className="flex-1 h-px bg-cyan-500/60 min-w-[12px]" />
                </div>
              ))}
            </div>
            {/* Entity box */}
            <div className="border border-cyan-500/40 rounded px-4 py-3 text-center min-w-[120px] bg-slate-900/60">
              <div className="text-cyan-400 text-sm font-semibold">{parsedEntity.entityName}</div>
              <div className="text-slate-500 text-xs uppercase mt-0.5">{detectedLang}</div>
            </div>
            {/* Outputs */}
            <div className="space-y-1.5 min-w-[60px]">
              {parsedEntity.outputs.map(p => (
                <div key={p} className="flex items-center gap-2">
                  <div className="flex-1 h-px bg-emerald-500/60 min-w-[12px]" />
                  <span className="font-mono text-xs text-emerald-300">{p}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-3 flex items-center gap-3">
          <button
            ...
            className="px-6 py-2 bg-purple-700 hover:bg-purple-600 rounded font-medium transition-colors"
          >
            Parse & Update Table
          </button>
          {pastedVerilog && (
            <button
              onClick={() => { handlePasteChange(''); setShowExamples(false); }}
        ...
        {importResult && (
          <div className={`mt-3 p-3 rounded text-sm flex items-start gap-2 ${importResult.ok ? 'bg-emerald-900/40 border border-emerald-500/50 text-emerald-300' : 'bg-red-900/40 border border-red-500/50 text-red-300'}`}>
      ...
      <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-slate-50 rounded-lg p-6">
        ...
        <p className="text-slate-300 text-sm mb-4">
          Download a complete ZIP archive containing truth table, Verilog code, validation results, and documentation.
        </p>
        <button
          onClick={handleExportZip}
          className="px-8 py-3 bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-500 hover:to-red-500 rounded-lg font-bold text-lg transition-all flex items-center gap-2"
        >
          <Download size={20} />
