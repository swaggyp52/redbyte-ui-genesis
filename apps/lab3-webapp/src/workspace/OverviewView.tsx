...
export const OverviewView: React.FC = () => {
  ...
  const useProByDefault = (doc as LabDocV2).meta?.useProByDefault ?? false;
  ...
  return (
    <div className="space-y-4 p-4 overflow-y-auto h-full">
      ...
      <div className="space-y-2">
        <h3 className="font-tech font-bold text-emerald-400">Circuit Designer</h3>
        <label className="flex items-center gap-2 cursor-pointer p-2 rounded hover:bg-slate-800/50 transition">
          <input
            type="checkbox"
            checked={useProByDefault}
            onChange={handleTogglePro}
            className="w-4 h-4 cursor-pointer"
          />
          <span className="text-sm text-slate-300">Use Circuit Designer (Pro) by default</span>
        </label>
      </div>
