 ...
 */
export async function exportAsJSON(doc) {
 ...
 */
export async function exportAsCSV(doc) {
    const csvFiles = {};
    ...
    segments.forEach(seg => {
        const expr = doc.expressions[seg] || '';
 ...
 */
export async function exportAsZip(doc) {
    const zip = new JSZip();
    // Add JSON document
 ...
 */
export async function generatePDFReport(doc, options) {
    ...
    const addText = (text, fontSize = 12, lineHeight = 7, color = [0, 0, 0]) => {
    ...
    if (validation) {
        ...
        const color = validation.canAdvance ? [0, 150, 0] : [200, 0, 0];
 ...
 */
export async function captureCanvasImage(canvasElement) {
 ...
 */
export async function captureSegmentDisplay(displayElement) {
