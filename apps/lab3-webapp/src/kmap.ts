// Karnaugh Map utilities for 4-bit to 7-segment conversion
...
function inputToGrayPosition(input: number): number {
  const b3 = (input >> 3) & 1;
  const b2 = (input >> 2) & 1;
  const b1 = (input >> 1) & 1;
  const b0 = input & 1;

  return GRAY_CODE_ORDER.findIndex(
    (code) => code.b3 === b3 && code.b2 === b2 && code.b1 === b1 && code.b0 === b0
  );
}

// Generate K-map grid (0/1/X for don't-care) for a specific segment
export function generateKMapGrid(
  truthTable: TruthTableRow[],
  segmentIndex: 0 | 1 | 2 | 3 | 4 | 5 | 6
): (0 | 1 | 'X')[] {
  const grid: (0 | 1 | 'X')[] = Array(16).fill('X');
  ...
  for (let input = 0; input < 16; input++) {
    ...
    if (row.isDontCare) {
      ...
      grid[grayPos] = row.seg[segmentIndex] as 0 | 1;
    }
  }
...
export function minimizeBooleanExpr(kmapGrid: (0 | 1 | 'X')[]): string {
  // Collect minterms (positions where value = 1)
  const minterms: number[] = [];
  const dontCares: number[] = [];

  for (let i = 0; i < kmapGrid.length; i++) {
    if (kmapGrid[i] === 1) minterms.push(GRAY_CODE_ORDER[i] ? grayPosToInput(i) : i);
    if (kmapGrid[i] === 'X') dontCares.push(GRAY_CODE_ORDER[i] ? grayPosToInput(i) : i);
  }
...
function grayPosToInput(grayPos: number): number {
  const code = GRAY_CODE_ORDER[grayPos];
  if (!code) return grayPos;
  return (code.b3 << 3) | (code.b2 << 2) | (code.b1 << 1) | code.b0;
}

function greedyGroupKMap(
  kmapGrid: (0 | 1 | 'X')[],
  minterms: number[],
  dontCares: number[]
): { cells: number[]; mask: number }[] {
  // Simple greedy: find largest groups (8, 4, 2, 1 cells)
  const groups: { cells: number[]; mask: number }[] = [];
  ...
  for (const minterm of minterms) {
    if (covered.has(minterm)) continue;
...
function findGroupStartingFrom(
  minterm: number,
  minterms: number[],
  dontCares: number[],
  covered: Set<number>
...
function groupsToExpression(groups: { cells: number[]; mask: number }[]): string {
...
export function evaluateBoolExpr(expr: string, input: number): boolean {
  const b3 = (input >> 3) & 1;
  const b2 = (input >> 2) & 1;
  const b1 = (input >> 1) & 1;
  const b0 = input & 1;
  ...
  try {
    return Function(`"use strict"; return (${jsExpr})`)() as boolean;
  } catch {
    return false;
  }
}
