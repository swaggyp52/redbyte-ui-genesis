 ...
 */
export function validateExpressions(doc) {
    const errors = [];
        ...
        for (let input = 0; input < 10; input++) {
            ...
            try {
                ...
                errors.push({
                    ...
                    segment: segName,
                    input,
                    message: `Expression '${expr}' invalid: ${e.message}`,
 ...
 */
export function validateTruthTable(doc) {
    const errors = [];
 ...
 */
export function validateKMaps(doc) {
    const errors = [];
    for (const segName of SEGMENT_NAMES) {
        const kmap = doc.kMaps[segName];
        if (!kmap) {
            errors.push({
                severity: 'warning',
                segment: segName,
 ...
 */
export function validateLabDoc(doc) {
    const errorArrays = [
 ...
 */
export function getValidationMessage(summary) {
    if (summary.isValid) {
