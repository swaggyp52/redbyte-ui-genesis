...
export type HdlWarning =
  | { kind: 'empty'; description: string }
  | { kind: 'cycle'; description: string }
  | { kind: 'undriven'; signal: string }
  | { kind: 'multi-driver'; signal: string };

export interface GeneratedHdl {
  vhdl: string;
  verilog: string;
  xdc: string;
  warnings: HdlWarning[];
...
function hasCycle(circuit: CircuitDesignerDoc): boolean {
  ...
  const dfs = (id: string) => {
    ...
    for (const t of (adjacency.get(id) || [])) {
      ...
      else if (recStack.has(t)) { foundCycle = true; return; }
    }
    recStack.delete(id);
  };

  circuit.nodes.forEach(n => { if (!visited.has(n.id)) dfs(n.id); });
  return foundCycle;
}

/** Topological sort (post-order DFS) — returns node IDs in evaluation order */
function topoSort(circuit: CircuitDesignerDoc): string[] {
  const adjacency = new Map<string, string[]>();
  circuit.nodes.forEach(n => adjacency.set(n.id, []));
  circuit.wires.forEach(w => {
    const targets = adjacency.get(w.from.nodeId) || [];
...
function getDriverSignal(
  ...
  const wire = circuit.wires.find(
    w => w.to.nodeId === targetNodeId && w.to.port === port
  );
  if (!wire) return null;
...
function gateToVerilog(
  ...
  const v = (s: string) => s.replace(/\((\d+)\)/g, '[$1]').replace(/^'([01])'$/, "1'b$1");
...
/**
 * Parse a VHDL entity/port block and return entity name, inputs, outputs.
 * Used by roundTripCheck to verify the generated VHDL is well-formed.
 */
export interface ParsedVhdlEntity {
  entityName: string;
  inputs: string[];
  outputs: string[];
}

export function parseVhdlEntity(code: string): ParsedVhdlEntity | null {
  const entityMatch = /entity\s+(\w+)\s+is/i.exec(code);
  if (!entityMatch) return null;

  const portBlock = /port\s*\(([^)]+)\)/is.exec(code);
  if (!portBlock) return { entityName: entityMatch[1], inputs: [], outputs: [] };
 ...
 */
export function generateHdlFromCircuit(circuit: CircuitDesignerDoc): GeneratedHdl {
  const warnings: HdlWarning[] = [];
  ...
  const sortedOutputs = [...outputNodes].sort((a, b) => a.id.localeCompare(b.id));
  const vhdlOutputLines: string[] = [];
  const verilogOutputLines: string[] = [];
...
function buildVhdlEntityBlock(inputNodes: CircuitNode[], outputNodes: CircuitNode[]): string {
...
function buildVhdlStub(outputNodes: CircuitNode[]): string {
  const outCount = outputNodes.length;
  const stub =
    outCount === 0 ? '  null;' :
...
function buildVerilogStub(inputNodes: CircuitNode[], outputNodes: CircuitNode[]): string {
  const inCount = inputNodes.length;
  const outCount = outputNodes.length;
...
export function roundTripCheck(
  ...
  const fatalWarning = hdl.warnings.find(w => w.kind === 'cycle' || w.kind === 'empty') as
    | { kind: 'empty' | 'cycle'; description: string }
    | undefined;
  if (fatalWarning) {
    return {
      pass: false,
