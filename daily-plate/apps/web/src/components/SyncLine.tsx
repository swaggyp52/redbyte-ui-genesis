...
/** One honest line about where the diary stands with the Pi. */
export function SyncLine() {
  ...
  if (sync.attention > 0) return <span className="badge badge-coral">{sync.attention} need{sync.attention === 1 ? 's' : ''} attention</span>;
  if (sync.pending > 0) {
    return (
  ...
  return <span className="badge badge-teal">Saved</span>;
}
