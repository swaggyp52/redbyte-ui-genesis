 ...
 * Matches: Label -> ID
 */
export function getSignalValue(project, signalName) {
    if (!project || !signalName || !project.circuit)
        return undefined;
    // 1. Find node by Label or ID
    const node = project.circuit.nodes.find((n) => n.label === signalName || n.id === signalName);
    if (!node)
        return undefined;
    ...
    if (state.output !== undefined)
        return state.output === 1 || state.output === true;
    if (state.out !== undefined)
        return state.out === 1 || state.out === true;
    if (state.Q !== undefined)
        return state.Q === 1 || state.Q === true;
 ...
 */
export function isInputCapableSignal(node) {
 ...
 */
export function isOutputCapableSignal(node) {
 ...
 */
export function getAvailableSignals(project) {
