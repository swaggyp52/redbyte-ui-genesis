...
import type { LabActionV1, LabProjectV1, CircuitConnection } from '@redbyte/rb-utils';
...
function normalizeForGate(project: LabProjectV1): unknown {
  const { createdAt: _c, updatedAt: _u, ...rest } = project;
  return rest;
}

function invertAction(stateBefore: LabProjectV1, action: LabActionV1): LabActionV1[] {
  switch (action.t) {
    ...
    case 'circuit/deleteNode': {
      ...
      if (!prevNode) return [];
      const prevConnections = stateBefore.circuit.connections.filter(
        (c) => c.fromNodeId === prevNode.id || c.toNodeId === prevNode.id,
      );
...
describe('Undo/redo gate (core labReducer circuit edits are reversible)', () => {
  ...
  it('can undo and redo a core edit sequence without corrupting circuit references', () => {
    ...
    for (const action of actions) {
      ...
      for (const u of undoActions) inverses.push(u);
