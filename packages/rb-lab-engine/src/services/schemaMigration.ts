...
/**
 * Schema Migration Pipeline for LabProject
 *
 * Handles forward compatibility as LabProject schema evolves.
 * Philosophy: Preserve data, add sensible defaults for missing fields.
 *
 * Migration Strategy:
 * 1. Each version has a migration function (v1→v2, v2→v3, etc.)
 * 2. Unknown fields are preserved (flexible schema)
 * 3. Missing fields get defaults (non-breaking)
 * 4. Validation is strict on read, lenient on write
 */
 ...
 */
export const CURRENT_LAB_PROJECT_SCHEMA_VERSION = '1.0' as const;
...
/**
 * Migration function type: transforms project from one version to next
 */
type MigrationFn = (project: LabProjectV1) => LabProjectV1;
 ...
 */
export function migrateLabProject(
  project: LabProjectV1
): { success: true; project: LabProjectV1; alreadyCurrent?: boolean } | { success: false; error: string } {
 ...
 */
export function validateLabProject(project: unknown): { valid: true } | { valid: false; error: string } {
 ...
 */
export function isFutureVersion(version: unknown): boolean {
 ...
 */
export function getSupportedVersionsInfo(): string {
  return `Supported: ${SUPPORTED_SCHEMA_VERSIONS.join(', ')}. Current: ${CURRENT_LAB_PROJECT_SCHEMA_VERSION}`;
}

/**
 * Create migration documentation for changelog
 *
 * Called when adding new schema version to document breaking changes
 */
 ...
 */
export const exampleMigrationDoc: SchemaMigrationDoc = {
  fromVersion: '1.0',
  toVersion: '2.0',
