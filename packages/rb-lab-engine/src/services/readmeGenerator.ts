...
/**
 * README Generator — Auto-generate human-readable project summaries
 *
 * Creates markdown summaries for exported projects to make them
 * accessible without needing to parse JSON.
 */

import type { LabProjectV1 } from '@redbyte/rb-utils';

export interface ReadmeOptions {
  includeCircuitStats?: boolean;
  includeCheckpoints?: boolean;
  includeBoardMapping?: boolean;
...
export function generateReadme(
  ...
  const {
    includeCircuitStats = true,
    includeCheckpoints = true,
    includeBoardMapping = true,
    includeProbes = true,
    exportDate,
  } = options;
  ...
  const formatIso = (value: string | undefined) => {
  ...
  lines.push(`- **Created**: ${formatIso(project.createdAt)}`);
  lines.push(`- **Updated**: ${formatIso(project.updatedAt)}`);
  lines.push('');
  ...
  if (includeProbes && project.simulation.probes.length > 0) {
    ...
    project.simulation.probes.forEach((probe) => {
      lines.push(
        `- **${probe.label || probe.signal}** (${probe.signal}) — ${probe.color || 'default'}`
      );
    });
    lines.push('');
  }
  ...
  if (includeBoardMapping && project.boardMap) {
    ...
    if (project.boardMap.virtualIOState) {
      ...
      if (switches.length > 0) {
        lines.push(
          `- **Switches**: ${switches.map((s) => (s ? '1' : '0')).join(' ')}`
        );
      }
      if (buttons && buttons.length > 0) {
        lines.push(
          `- **Buttons**: ${buttons.map((b) => (b ? '1' : '0')).join(' ')}`
        );
      }
      lines.push('');
    }
  ...
  if (project.evidence.manifest) {
    lines.push(
      `- **Integrity**: ${project.evidence.manifest.files.length} files verified`
    );
  }
  lines.push('');
  ...
  lines.push(`Export date: ${formatIso(exportDate ?? project.updatedAt ?? project.createdAt)}`);
  lines.push('');
