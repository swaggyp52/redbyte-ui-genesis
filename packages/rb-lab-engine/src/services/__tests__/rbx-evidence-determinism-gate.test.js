...
const FIXTURE_PATH = 'packages/rb-lab-engine/src/services/__tests__/fixtures/rbx-evidence-determinism.fixture.project.json';
...
async function extractTextFiles(blob) {
    const zip = await JSZip.loadAsync(blob);
    const get = async (path) => {
        const file = zip.file(path);
        if (!file)
            throw new Error(`Missing ${path} in exported capsule`);
...
describe('RBX Evidence Determinism Gate', () => {
    it('exports deterministic manifest/capsule/readme contents', async () => {
        ...
        if (!goldenSha) {
            throw new Error(`Missing or invalid golden at ${GOLDEN_PATH}. Set ${UPDATE_GOLDEN_ENV}=1 to regenerate.`);
        }
