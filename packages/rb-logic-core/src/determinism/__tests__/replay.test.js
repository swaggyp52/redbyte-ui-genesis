...
import { createEventLog, appendEvent, createCircuitLoadedEvent, createInputToggledEvent, createSimulationTickEvent, createNodeStateModifiedEvent, } from '../eventLog';
import { runReplay, validateEventLog } from '../replay';
describe('replay', () => {
    ...
    describe('validateEventLog', () => {
        ...
        it('rejects log where first event is not circuit_loaded', () => {
            ...
            const result = validateEventLog(log);
            expect(result.valid).toBe(false);
            expect(result.errors.some((e) => e.includes('First event must be circuit_loaded'))).toBe(true);
        });
