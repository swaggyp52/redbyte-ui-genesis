...
describe('diffState (PR9: State Diffing)', () => {
    ...
    it('Test 3: diffState detects signal changes', () => {
        ...
        const switchAChange = diff.signalChanges.find((c) => c.nodeId === 'switchA' && c.portName === 'out');
    ...
    it('Test 4: diffState detects multiple signal changes after tick', () => {
        ...
        const andGateChange = diff.signalChanges.find((c) => c.nodeId === 'andGate' && c.portName === 'out');
    ...
    it('Test 5: diffState detects node additions', () => {
        ...
        const modifiedCircuit = JSON.parse(JSON.stringify(testCircuit));
    ...
    it('Test 8: diffState detects connection removals', () => {
        ...
        const modifiedCircuit = JSON.parse(JSON.stringify(testCircuit));
        modifiedCircuit.connections = modifiedCircuit.connections.filter((c) => !(c.from.nodeId === 'switchA' && c.to.nodeId === 'andGate'));
        let log1 = createEventLog();
