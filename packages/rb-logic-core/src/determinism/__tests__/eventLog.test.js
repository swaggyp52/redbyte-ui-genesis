...
import { encodeEventLog, decodeEventLog, createEventLog, appendEvent, createCircuitLoadedEvent, createInputToggledEvent, createSimulationTickEvent, createNodeStateModifiedEvent, } from '../eventLog';
describe('eventLog', () => {
    const simpleCircuit = {
        nodes: [
            {
    ...
    describe('appendEvent', () => {
        ...
        it('maintains event order', () => {
            ...
            expect(log.events.map((e) => e.tickIndex)).toEqual([0, 1, 2]);
        });
    });
    ...
    describe('Event log integrity', () => {
        it('preserves event order through encode/decode', () => {
            ...
            const encoded = encodeEventLog(log);
            const decoded = decodeEventLog(encoded);
            const tickIndices = decoded.events.map((e) => e.tickIndex);
        ...
        it('preserves circuit structure through encode/decode', () => {
            ...
            const encoded = encodeEventLog(log);
            const decoded = decodeEventLog(encoded);
            const decodedCircuit = decoded.events[0].circuit;
