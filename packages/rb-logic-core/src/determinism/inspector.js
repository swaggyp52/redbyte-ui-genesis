 ...
 */
export function getStateAtIndex(initialCircuit, eventLog, eventIndex, options) {
    ...
    if (eventIndex < 0 || eventIndex >= eventLog.events.length) {
        throw new Error(`Event index ${eventIndex} out of bounds (log has ${eventLog.events.length} events, valid range: 0..${eventLog.events.length - 1})`);
    }
    ...
    let circuit = JSON.parse(JSON.stringify(initialCircuit));
    ...
    for (let i = 0; i <= eventIndex; i++) {
        ...
        switch (event.type) {
            case 'circuit_loaded':
                ...
                circuit = JSON.parse(JSON.stringify(event.circuit));
    ...
    const snapshotSignals = new Map();
    for (const [nodeId, portMap] of engine.signals.entries()) {
        const clonedPortMap = new Map();
