import { validateEvidenceAgainstLabSpec, LabSpecV1 } from '../validateEvidenceAgainstLabSpec';
...
describe('validateEvidenceAgainstLabSpec', () => {
  ...
  it('is backward compatible with older evidence (missing fields)', () => {
    const legacyEvidence = { probes: [{ name: 'clk' }], ticks: undefined } as any;
    const spec: LabSpecV1 = { labId: 'lab1', requirements: { probes: ['clk'], minTicks: 1 } };
