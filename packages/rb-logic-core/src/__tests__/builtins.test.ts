...
describe('Built-in Node Behaviors', () => {
  describe('INPUT / OUTPUT', () => {
    it('should surface signals for evaluator-style nodes', () => {
      const circuit: Circuit = {
        nodes: [
          { id: 'src', type: 'INPUT', position: { x: 0, y: 0 }, rotation: 0, config: {}, state: { isOn: 0 } },
          { id: 'not', type: 'NOT', position: { x: 1, y: 0 }, rotation: 0, config: {} },
          { id: 'out', type: 'OUTPUT', position: { x: 2, y: 0 }, rotation: 0, config: {} },
        ],
        connections: [
          { from: { nodeId: 'src', portName: 'out' }, to: { nodeId: 'not', portName: 'in' } },
      ...
      expect(signals.get('src.out')).toBe(0);
      expect(signals.get('out.out')).toBe(1);

      engine.setNodeState('src', { isOn: 1 });
      engine.stabilize();

      signals = engine.getAllSignals();
      expect(signals.get('src.out')).toBe(1);
      expect(signals.get('out.out')).toBe(0);
    });
  });
  ...
  describe('Logic Gates', () => {
    ...
    it('AND gate accepts in1/in2 aliases', () => {
      const circuit: Circuit = {
        nodes: [
          { id: 'src1', type: 'PowerSource', position: { x: 0, y: 0 }, rotation: 0, config: {} },
          { id: 'src2', type: 'PowerSource', position: { x: 0, y: 1 }, rotation: 0, config: {} },
          { id: 'and1', type: 'AND', position: { x: 1, y: 0 }, rotation: 0, config: {} },
        ],
        connections: [
          { from: { nodeId: 'src1', portName: 'out' }, to: { nodeId: 'and1', portName: 'in1' } },
          { from: { nodeId: 'src2', portName: 'out' }, to: { nodeId: 'and1', portName: 'in2' } },
        ],
      };
