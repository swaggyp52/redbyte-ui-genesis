...
describe('Wire Validation', () => {
    ...
    describe('isValidConnection', () => {
        it('should allow valid output->input connections', () => {
            const result = isValidConnection({ nodeId: 'node1', portName: 'out' }, { nodeId: 'node3', portName: 'in' }, mockCircuit);
            expect(result.valid).toBe(true);
        });
        it('should allow valid input->output connections (will be normalized)', () => {
            const result = isValidConnection({ nodeId: 'node3', portName: 'in' }, { nodeId: 'node1', portName: 'out' }, mockCircuit);
            expect(result.valid).toBe(true);
        });
        it('should prevent self-loops', () => {
            const result = isValidConnection({ nodeId: 'node1', portName: 'out' }, { nodeId: 'node1', portName: 'in' }, mockCircuit);
            expect(result.valid).toBe(false);
            expect(result.reason).toBe('Cannot connect node to itself');
        });
        it('should prevent output->output connections', () => {
            const result = isValidConnection({ nodeId: 'node1', portName: 'out' }, { nodeId: 'node2', portName: 'out' }, mockCircuit);
            expect(result.valid).toBe(false);
            expect(result.reason).toBe('Cannot connect output to output');
        });
        it('should prevent input->input connections', () => {
            const result = isValidConnection({ nodeId: 'node1', portName: 'in1' }, { nodeId: 'node2', portName: 'in2' }, mockCircuit);
            expect(result.valid).toBe(false);
            expect(result.reason).toBe('Cannot connect input to input');
        });
        it('should prevent duplicate connections', () => {
            const result = isValidConnection({ nodeId: 'node1', portName: 'out' }, { nodeId: 'node2', portName: 'in1' }, mockCircuit);
            expect(result.valid).toBe(false);
            expect(result.reason).toBe('Connection already exists');
        });
        it('should prevent duplicate connections in reverse direction', () => {
            const result = isValidConnection({ nodeId: 'node2', portName: 'in1' }, { nodeId: 'node1', portName: 'out' }, mockCircuit);
            expect(result.valid).toBe(false);
            expect(result.reason).toBe('Connection already exists');
        });
    });
    describe('normalizeConnection', () => {
        it('should keep output->input connections as-is', () => {
            const connection = normalizeConnection({ nodeId: 'node1', portName: 'out' }, { nodeId: 'node2', portName: 'in' }, mockCircuit);
        ...
        it('should swap input->output connections to output->input', () => {
            const connection = normalizeConnection({ nodeId: 'node2', portName: 'in' }, { nodeId: 'node1', portName: 'out' }, mockCircuit);
            expect(connection.from.nodeId).toBe('node1');
            expect(connection.from.portName).toBe('out');
            expect(connection.to.nodeId).toBe('node2');
            expect(connection.to.portName).toBe('in');
