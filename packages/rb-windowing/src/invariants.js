 ...
 */
export function assertWindowInvariants(windows) {
    ...
    if (focusedWindows.length > 1) {
        throw new Error(`Invariant violation: Multiple focused windows detected.\n` +
    ...
    if (zIndexes.length !== uniqueZIndexes.size) {
        ...
        throw new Error(`Invariant violation: Duplicate z-index values detected.\n` +
    ...
    if (invalidFocused.length > 0) {
        throw new Error(`Invariant violation: Minimized window is focused.\n` +
    ...
    if (invalidZIndexes.length > 0) {
        throw new Error(`Invariant violation: Invalid z-index values detected.\n` +
 ...
 */
export function getFocusedWindow(windows) {
 ...
 * Useful for testing.
 */
export function checkWindowInvariants(windows) {
    const errors = [];
    const focusedWindows = windows.filter((w) => w.focused && w.mode !== 'minimized');
    if (focusedWindows.length > 1) {
