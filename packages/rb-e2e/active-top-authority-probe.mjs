...
// real Project UI: read the projected Top field, set the active top, confirm the
...
// Load the Half Adder so the Project Overview document renders.
...
// The Overview Top field must project the SAME value.
const chip = page.getByTestId('ide-project-active-top');
if (await chip.count() === 0) fail('Top field not rendered (Project Overview document missing)');
// The Top field is an editable projection of the store: read its committed value.
const topField = page.getByTestId('ide-project-fpga-top').first();
const chipValue = await topField.inputValue();
if (chipValue !== 'half_adder') fail(`Top field does not show store value: "${chipValue}"`);
console.log(`① store activeTop=half_adder projected in the Top field: "${chipValue}"`);

// Set the active top through the UI — type a valid identifier and commit it.
await topField.fill('my_ripple_top');
await topField.press('Enter'); await page.waitForTimeout(300);
...
await topField.fill('9-bad name');
await topField.press('Enter'); await page.waitForTimeout(250);
top = await storeTop();
