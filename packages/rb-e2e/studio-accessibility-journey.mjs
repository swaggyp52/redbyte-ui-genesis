...
try {
 for(const config of [{width:1440,height:900,root:16},{width:1280,height:650,root:16},{width:1280,height:650,root:32},{width:720,height:450,root:16},{width:1440,height:900,root:16,zoom:2}]) {
  let zoomContext;
  if(config.zoom){
   const profile=fs.mkdtempSync(path.join(out,'zoom-profile-'));fs.mkdirSync(path.join(profile,'Default'));
   // Chromium profile zoom scales the layout viewport while the root text remains16px.
   fs.writeFileSync(path.join(profile,'Default','Preferences'),JSON.stringify({partition:{default_zoom_level:{x:Math.log(config.zoom)/Math.log(1.2)}}}));
   zoomContext=await chromium.launchPersistentContext(profile,{channel:'chromium',headless:true,viewport:null,reducedMotion:'reduce',args:['--window-size='+config.width+','+config.height]});
  }
  const page=zoomContext?zoomContext.pages()[0]:await browser.newPage({viewport:{width:config.width,height:config.height},reducedMotion:'reduce'});const tid=id=>page.getByTestId(id);page.setDefaultTimeout(15000);const errors=[];page.on('pageerror',e=>errors.push(e.message));
  ...
  try {
   ...
   await page.evaluate(root=>document.documentElement.style.fontSize=root+'px',config.root);
   const zoomMetrics=await page.evaluate(()=>({width:innerWidth,height:innerHeight,outerWidth,outerHeight,dpr:devicePixelRatio,root:getComputedStyle(document.documentElement).fontSize,visualScale:visualViewport.scale}));
   if(config.zoom){assert.equal(zoomMetrics.dpr,2);assert.equal(zoomMetrics.root,'16px');assert.ok(zoomMetrics.width<config.width*.51,'Real browser zoom halves the CSS viewport');}await activate(tid('mode-button-design'));await activate(tid('ide-design-test-design'));
   ...
   const details=tid('ide-run-details');await activate(details.locator('summary'));
   assert.notEqual(await details.getAttribute('open'),null);
   const contrasts=[];for(const id of ['ide-vcb-run','ide-vcb-reproduce','ide-run-repetition','ide-run-check-result']){const measured=await measure(tid(id));assert.ok(measured.visibleFraction>.98 && measured.receivesClick,id+' remains reachable '+JSON.stringify(measured));assert.ok(measured.contrast>=4.5,id+' text contrast '+measured.contrast);contrasts.push({id,...measured});}
   await details.locator('summary').focus();await page.keyboard.press('Escape');assert.equal(await details.getAttribute('open'),null);assert.ok(await details.locator('summary').evaluate(el=>document.activeElement===el),'Escape returns focus to the disclosure summary');
   ...
   const boardLegends=await page.locator('svg text[data-board-alias]').evaluateAll(elements=>elements.map(element=>{
    const style=getComputedStyle(element);const rgb=style.fill.match(/[\d.]+/g)?.map(Number)??[];
    const background=[11,32,61];const foreground=rgb.slice(0,3).map((v,i)=>v*(rgb[3]??1)+background[i]*(1-(rgb[3]??1)));
    const l=c=>c.map(v=>v/255).map(v=>v<=.04045?v/12.92:((v+.055)/1.055)**2.4).reduce((sum,v,i)=>sum+v*[.2126,.7152,.0722][i],0);
    return {alias:element.textContent,fill:style.fill,contrast:(Math.max(l(foreground),l(background))+.05)/(Math.min(l(foreground),l(background))+.05)};
   }));
   assert.ok(boardLegends.length>20,'Measured mapped and inactive resource legends');assert.ok(boardLegends.every(legend=>legend.contrast>=4.5),'Board legends: '+JSON.stringify(boardLegends));
   ...
   assert.deepEqual(errors,[]);await page.screenshot({path:path.join(out,config.width+'x'+config.height+'-root'+config.root+'-zoom'+(config.zoom??1)+'.png')});results.push({config,zoomMetrics,contrasts,boardLegends,packageAction,errors});
  }catch(error){await page.screenshot({path:path.join(out,'failure-'+config.width+'-root'+config.root+'.png')});throw error;}finally{await page.close();if(zoomContext)await zoomContext.close();}
 }
