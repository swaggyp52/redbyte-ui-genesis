 ...
 */
export function loadBoardProfile(profileId: string): BoardProfile {
 ...
 */
export function listBoardProfiles(): Array<{ id: string; name: string; vendor: string }> {
  return Object.values(BUILT_IN_PROFILES).map((p: any) => ({
    id: p.id,
    name: p.name,
    vendor: p.vendor,
  }));
 ...
 */
export function validateBoardProfile(data: unknown): BoardProfile {
  ...
  return profile as BoardProfile;
}
