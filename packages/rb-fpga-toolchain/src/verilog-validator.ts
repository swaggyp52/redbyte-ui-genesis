...
/**
 * Verilog HDL syntax and semantic validation
 * Browser-safe static analysis (no Node.js dependencies)
 */

export interface VerilogValidationResult {
  valid: boolean;
  errors: VerilogError[];
  warnings: VerilogWarning[];
  moduleInfo?: {
    moduleName: string;
    inputs: string[];
    outputs: string[];
...
export interface VerilogWarning {
  line?: number;
  message: string;
  severity: 'warning';
  code: string;
 ...
 */
export function validateVerilog(verilogCode: string): VerilogValidationResult {
  const errors: VerilogError[] = [];
  const warnings: VerilogWarning[] = [];
 ...
 */
function extractPortSection(verilogCode: string): string | null {
 ...
 */
function isLiteral(signal: string): boolean {
...
/**
 * Validate XDC constraints file for Basys3 board
 */
export interface ConstraintValidationResult {
  ...
  pinInfo?: {
    ...
    clockConstraints: number;
    timingConstraints: number;
  };
}
...
export function calculateReadinessScore(
  verilogResult: VerilogValidationResult,
