...
import type { ToolchainCapabilities } from './types';
 ...
 */
const VIVADO_SEARCH_PATHS: Record<string, string[]> = {
  win32: [
    'C:\\Xilinx\\Vivado',
    'D:\\Xilinx\\Vivado',
 ...
 */
async function findVivado(): Promise<{ path: string; version: string } | null> {
 ...
 */
async function findYosys(): Promise<{ path: string; version: string } | null> {
  const platform = os.platform();
 ...
 */
async function findNextpnr(): Promise<{ path: string; version: string } | null> {
  const platform = os.platform();
 ...
 */
async function findOpenFPGALoader(): Promise<{
  path: string;
  version: string;
  boards: string[];
} | null> {
 ...
 */
export async function detectToolchain(): Promise<ToolchainCapabilities> {
 ...
 */
export function canSynthesize(capabilities: ToolchainCapabilities): boolean {
 ...
 */
export function canProgram(capabilities: ToolchainCapabilities): boolean {
 ...
 */
export function getPreferredSynthesisTool(
  capabilities: ToolchainCapabilities
): 'vivado' | 'yosys' | null {
  // Prefer Vivado for Artix-7 (most reliable)
  if (capabilities.vivado?.canSynthesize) return 'vivado';
  if (capabilities.yosys) return 'yosys';
  return null;
}
 ...
 */
export function getPreferredProgrammingTool(
  capabilities: ToolchainCapabilities
): 'vivado' | 'openFPGALoader' | null {
  // Prefer Vivado for reliability
  if (capabilities.vivado?.canProgram) return 'vivado';
  if (capabilities.openFPGALoader) return 'openFPGALoader';
  return null;
}
