 ...
 */
export function parseTimingReport(reportPath: string): TimingReport | null {
  try {
 ...
 */
export function parseUtilizationReport(reportPath: string): UtilizationReport | null {
  try {
    const content = fs.readFileSync(reportPath, 'utf-8');

    // Parse LUT utilization
    const lutMatch = content.match(
      /Slice LUTs\s*\|\s*(\d+)\s*\|\s*\d+\s*\|\s*(\d+)\s*\|\s*([\d.]+)/i
    );
    // Parse FF utilization
    const ffMatch = content.match(
      /Slice Registers\s*\|\s*(\d+)\s*\|\s*\d+\s*\|\s*(\d+)\s*\|\s*([\d.]+)/i
    );
    // Parse BRAM utilization
    const bramMatch = content.match(
      /Block RAM Tile\s*\|\s*(\d+)\s*\|\s*\d+\s*\|\s*(\d+)\s*\|\s*([\d.]+)/i
    );
    // Parse DSP utilization
    const dspMatch = content.match(
      /DSPs\s*\|\s*(\d+)\s*\|\s*\d+\s*\|\s*(\d+)\s*\|\s*([\d.]+)/i
    );
 ...
 */
export type ProgressCallback = (status: SynthesisStatus, progress: number, log: string) => void;

/**
 * Run Vivado synthesis
 */
export async function runVivadoSynthesis(
  vivadoPath: string,
  request: {
    verilog: string;
 ...
 */
export function generateVivadoProgramTcl(bitstreamPath: string): string {
 ...
 */
export async function programFpgaWithVivado(
  vivadoPath: string,
  bitstreamPath: string,
  onProgress?: (status: string, progress: number, log: string) => void
): Promise<{
  success: boolean;
  logs: string[];
  error?: string;
