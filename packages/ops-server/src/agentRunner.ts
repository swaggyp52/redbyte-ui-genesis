...
export async function runAgentIngest(
  ...
  options: { strictHash?: boolean; golden?: string } = {}
): Promise<IngestResult> {
  return new Promise((resolve, reject) => {
    // Build pnpm agent:lab command
    const args = ['agent:lab', '--', '--submission', submissionPath];
    if (options.strictHash) args.push('--strict-hash');
    if (options.golden) args.push('--golden', options.golden);
    ...
    proc.on('close', async (code) => {
      try {
        ...
        const verdict = (verdictMatch?.[1] || 'INVALID') as 'PASS' | 'FAIL' | 'INVALID';
 ...
 */
export async function loadRun(runId: string): Promise<IngestResult | null> {
  try {
    const runDir = path.join(repoRoot, 'packages/ops/labs/runs', runId);
    const gradeJsonPath = path.join(runDir, 'grade.json');
    const gradeMdPath = path.join(runDir, 'grade.md');
    ...
    return {
      run_id: runId,
      verdict: (gradeJson.verdict || 'INVALID') as 'PASS' | 'FAIL' | 'INVALID',
 ...
 */
export async function listRuns(): Promise<Array<{ run_id: string; timestamp: string; verdict: string }>> {
  try {
