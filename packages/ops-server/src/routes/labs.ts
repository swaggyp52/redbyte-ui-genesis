 ...
 */
router.post('/api/labs/ingest', upload.single('submission'), async (req: Request, res: Response) => {
  try {
    if (!req.file) {
    ...
    const golden = req.query.golden as string | undefined;
 ...
 */
router.get('/api/labs/runs', async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/api/labs/runs/:run_id', async (req: Request, res: Response) => {
  try {
