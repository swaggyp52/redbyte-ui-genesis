...
import type { LabEvent, LabGraph, LabTimeline, LabNode, LabWire } from '../types';
...
const createRng = (seed: number): Rng => {
    ...
    return {
        ...
        pick: <T>(items: T[]) => (items.length ? items[Math.floor(next() * items.length)] : undefined)
    };
};

const cloneGraph = (graph: LabGraph): LabGraph => JSON.parse(JSON.stringify(graph));

const normalizeQuaternion = (rng: Rng) => {
    let x = rng.next() * 2 - 1;
    let y = rng.next() * 2 - 1;
    let z = rng.next() * 2 - 1;
    let w = rng.next() * 2 - 1;
...
const buildNode = (rng: Rng, type: string, idSuffix: number): LabNode => ({
    id: `${type}-${idSuffix}`,
    type,
    pose: {
...
const buildWire = (rng: Rng, sourceId: string, sourceType: string, targetId: string, targetType: string, idSuffix: number): LabWire => {
...
const applyEvent = (graph: LabGraph, pinStates: Record<string, number>, event: LabEvent) => {
    switch (event.type) {
        case 'PLACE_PART':
...
const deriveFromScratch = (timeline: LabTimeline, targetTick: number) => {
...
const runFuzz = (seed: number, iterations: number, mode: 'basic' | 'mode-switch') => {
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        try {
            if (action < 15) {
                ...
                if (id) {
                    state.updateNodePose(id,
                        { x: (rng.next() - 0.5) * 20, y: (rng.next() - 0.5) * 20, z: 0 },
                        normalizeQuaternion(rng)
                    );
                }
            } else if (action < 55) {
                if (nodeIds.length >= 2) {
                ...
                if (wireId) {
                    ...
                    if (index >= 0) wireIds.splice(index, 1);
                }
            } else if (action < 90) {
