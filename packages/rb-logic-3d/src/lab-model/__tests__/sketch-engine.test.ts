...
import type { LabGraph, LabNode } from '../types';

const buildGraph = (): LabGraph => {
  const nano: LabNode = {
    id: 'nano-1',
    type: 'arduino-nano',
...
const runSketch = (
  source: string,
  ticks: number,
  inputs?: (tick: number) => Record<string, number>
) => {
  ...
  const serial: string[] = [];
  const writes: Array<{ tick: number; pinKey: string; value: number }> = [];
...
void loop() {
  ...
  delay(50);
    ...
    const pattern = (tick: number) => ({ 'nano-1:D2': tick % 2 === 1 ? 1 : 0 });
