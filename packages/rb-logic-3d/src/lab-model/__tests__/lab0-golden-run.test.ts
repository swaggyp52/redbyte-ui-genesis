...
import type { LabGraph, LabTimeline, LabEvent, LabNode } from '../types';
...
const LAB0_TEMPLATE: LabTemplate = {
...
function makeNode(id: string, type: string): LabNode {
  return { id, type, pose: DEFAULT_POSE, properties: {} };
}

function makeGraph(): LabGraph {
  return {
    nodes: [
 ...
 */
function makePassingTimeline(): LabTimeline {
  let seq = 0;
  const events: LabEvent[] = [];

  const pin = (tick: number, diffs: Record<string, number>): LabEvent => ({
    type: 'SIM_PIN_DIFF',
    tick,
    seq: seq++,
    source: 'engine',
...
function makeFailingTimeline(): LabTimeline {
...
describe('Lab 0 Golden Run', () => {
  ...
  it('fails parts check when graph is missing a required part', () => {
    const incompleteGraph: LabGraph = {
  ...
  it('detects SW0 held for exactly min_ticks threshold', () => {
    ...
    const timeline: LabTimeline = { events, snapshots: [] };
