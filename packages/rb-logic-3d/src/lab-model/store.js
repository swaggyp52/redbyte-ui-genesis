...
const createEmptyGraph = () => ({ nodes: [], wires: [], net: {} });
...
const playbackModeFromState = (state) => state.startsWith('live') ? 'live' : 'replay';
const isRunningFromState = (state) => state === 'live:running' || state === 'replay:playing';
const applyPlaybackState = (simulation, nextState) => {
...
const pauseLiveSimulationForEdit = (state) => {
    if (state.simulation.playbackState !== 'live:running')
        return;
...
const resyncEventSeq = (events) => events.map((event, index) => ({ ...event, seq: index }));
...
const buildWireAdjacency = (graph) => {
...
const propagatePinDiffs = (graph, pinStates, diffs) => {
    const adjacency = buildWireAdjacency(graph);
    const expanded = {};
    const queue = Object.entries(diffs).map(([key, value]) => ({ key, value }));
...
const scheduleSnapshotFingerprint = (setState, snapshotSource) => {
...
const applyIntegrityRecovery = (state, reason, options) => {
...
const assertInvariants = (state, context) => {
    if (!DEBUG_INVARIANTS)
        return;
...
export const useLabStore = create()(subscribeWithSelector((set, get) => {
    ...
    return {
        ...
        integrityError: null,
        lastGoodSnapshot,
        setInteractionMode: (mode) => set(produce((state) => { state.interaction.mode = mode; })),
        setHighlightedPins: (pins) => set(produce((state) => { state.interaction.highlightedPins = pins; })),
        setSelectedNetId: (netId) => set(produce((state) => { state.interaction.selectedNetId = netId; })),
        setSelectedNodeId: (nodeId) => set(produce((state) => { state.interaction.selectedNodeId = nodeId; })),
        toggleSimulation: (running) => set(produce((state) => {
            if (state.integrityError)
                return;
            ...
            if (nextRunning === state.simulation.isRunning)
                return;
        ...
        setPlaybackMode: (mode) => set(produce((state) => {
            if (state.integrityError)
                return;
        ...
        runSimulationStep: () => set(produce((state) => {
            if (state.integrityError)
                return;
            if (state.simulation.playbackState === 'replay:playing') {
                const maxTick = state.simulation.tick;
            ...
            const hardwareNodes = state.graph.nodes.filter(n => n.type === 'fpga-basys3' || n.hardware_target === 'arduino-uno');
        ...
        setHoveredPin: (pin) => set(produce((state) => { state.interaction.hoveredPin = pin; })),
        startWire: (nodeId, pinId) => set(produce((state) => {
            if (state.simulation.playbackMode === 'replay')
                return;
            if (state.integrityError)
                return;
        ...
        cancelWire: () => set(produce((state) => {
            state.interaction.wireStartPin = null;
            state.interaction.mode = 'orbit';
            state.interaction.dragPosition = null;
        })),
        updateDragPosition: (pos) => set(produce((state) => { state.interaction.dragPosition = pos; })),
        ...
        updateNodePose: (id, position, rotation) => set(produce((state) => {
            if (state.simulation.playbackMode === 'replay')
                return;
            if (state.integrityError)
                return;
        ...
        addWire: (wire) => set(produce((state) => {
            if (state.simulation.playbackMode === 'replay')
                return;
            if (state.integrityError)
                return;
        ...
        removeWire: (id) => set(produce((state) => {
            if (state.simulation.playbackMode === 'replay')
                return;
            if (state.integrityError)
                return;
        ...
        setSketchSource: (source) => set(produce((state) => {
            if (state.simulation.playbackMode === 'replay')
                return;
            if (state.integrityError)
                return;
        ...
        clearSerial: () => set(produce((state) => {
            state.sketch.serial = [];
        })),
        startLabSession: (template) => set(produce((state) => {
        ...
        clearLabSession: () => set(produce((state) => { state.labSession = null; })),
        recover: () => set(produce((state) => {
            const recoveryMsg = state.integrityError || 'Manual Recovery';
            applyIntegrityRecovery(state, recoveryMsg, { clearError: true });
        })),
        loadFpgaPreset: (nodeId, presetId) => set(produce((state) => {
            if (state.simulation.playbackMode === 'replay')
                return;
            if (state.integrityError)
                return;
        ...
        setUserPinState: (nodeId, pinId, value) => {
            ...
            if (transportToNotify) {
                transportToNotify.pushInteraction(nodeId, pinId, value);
            }
        },
        setTransport: (type) => {
            set(produce((state) => {
                if (state.activeTransport.getStatus().type === type)
                    return;
                // Disconnect old
        ...
        resetLayout: () => {
            set(produce((state) => {
                ...
                state.graph.nodes = [];
                state.graph.wires = [];
                state.timeline.events = state.timeline.events.filter((e) => e.type !== 'PLACE_PART' &&
                    e.type !== 'MOVE_PART' &&
                    e.type !== 'ADD_WIRE' &&
                    e.type !== 'REMOVE_WIRE');
...
function deriveStateAtTick(timeline, targetTick) {
    ...
    try {
        ...
        const graph = JSON.parse(JSON.stringify(bestSnap.graph));
        const pinStates = { ...bestSnap.pinStates };
