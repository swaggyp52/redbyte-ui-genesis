...
// --- VALIDATORS ---
export function validateLabGraph(graph) {
    const errors = [];
...
// --- REPAIR ---
export function repairLabGraph(graph) {
    const warnings = [];
    const safeNodes = [];
    const validNodeIds = new Set();
    const validPinsByNodeId = new Map();
    // 1. Repair Nodes
    for (const node of graph.nodes) {
...
export function repairTimeline(timeline) {
    const warnings = [];
    const safeEvents = [];
    const safeSnapshots = [];
    let lastSeq = -1;
...
export function fingerprintStateSync(state) {
    const canonical = {
        tick: state.tick,
...
export async function fingerprintCapsuleContent(content) {
    // Canonicalize
    ...
    const canonical = {
        meta: Object.keys(content.meta).sort().reduce((acc, key) => {
        ...
        history: {
            ...
            events: content.history.events.map(e => ({
                ...
                ...('pinDiffs' in e ? {
                    pinDiffs: Object.keys(e.pinDiffs).sort().reduce((acc, k) => {
                        acc[k] = e.pinDiffs[k];
                        return acc;
                    }, {})
                } : {})
