...
export const hashSketchSource = (source) => {
...
class Parser {
    ...
    parseBlock() {
        if (!this.consumeValue('{', 'Expected "{" to start block'))
            return null;
        const statements = [];
...
const asBlock = (statement) => {
    if (statement.type === 'block')
        return statement;
    return { type: 'block', statements: [statement] };
};
const compileStatements = (block) => {
    const instructions = [];
...
const compileStatement = (stmt, out) => {
    switch (stmt.type) {
        case 'block':
...
const compileExpression = (expr, out) => {
    switch (expr.type) {
        case 'literal':
...
export class SketchRuntime {
    ...
    step(host) {
        if (!this.program || this.state.halted)
            return;
        if (host.tick < this.state.sleepUntilTick)
            return;
        let steps = 0;
...
const applyBinary = (op, left, right) => {
