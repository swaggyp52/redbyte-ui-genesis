...
export const Rb3DSceneCircuit = ({ nodes, wires, signals, pulseMap, currentTime, animateSignalFlow, selectedNodeIds = new Set(), onNodeSelect, onNodeHover, onNodeMove, probeWireHighlights, netHighlightWireIds, mismatchWireHighlights, mismatchNodeIds, enterAnimation = false, onEnterAnimationComplete, }) => {
    ...
    return (_jsxs(_Fragment, { children: [activeTransformTarget && (_jsx(TransformControls, { object: activeTransformTarget, mode: "translate", showY: false, translationSnap: 0.5, onMouseUp: handleTransformEnd })), nodes.map((node, index) => {
                ...
                const position = [node.view.x / 20, animatedY, node.view.y / 20];
                ...
                const from = [wire.from.x / 20, wireY, wire.from.y / 20];
                const to = [wire.to.x / 20, wireY, wire.to.y / 20];
                ...
                return (_jsxs(React.Fragment, { children: [_jsx(WireMesh, { from: from, to: to, isActive: isActive, pulse: pulse, probeColors: mergedProbeColors, mismatchColors: mismatchColors }), animProgress >= 1 && isActive && animateSignalFlow && (_jsx(SignalParticleSystem, { from: from, to: to, isActive: isActive, wireId: wire.id, currentTime: currentTime }))] }, wire.id));
            })] }));
};
