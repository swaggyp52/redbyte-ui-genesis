...
export const SignalParticleSystem = ({ from, to, isActive, wireId, currentTime, }) => {
    ...
    return (_jsx(_Fragment, { children: particles.map(({ k, progress }) => {
            ...
            return (_jsxs("mesh", { position: [position.x, position.y + yOffset, position.z], children: [_jsx("sphereGeometry", { args: [0.07, 8, 8] }), _jsx("meshStandardMaterial", { color: "#D4930D", emissive: "#D4930D", emissiveIntensity: 1.2, transparent: true, opacity: fade })] }, `${wireId}-p-${k}`));
        }) }));
};
