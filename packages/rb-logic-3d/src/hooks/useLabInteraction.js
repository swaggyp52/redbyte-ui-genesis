...
export const useLabInteraction = () => {
    ...
    const handlePointerMove = useCallback((_event) => {
    ...
    const handlePinHover = useCallback((nodeId, pinId, e) => {
        e.stopPropagation(); // Pin hover blocks other hovers
        setHoveredPin({ nodeId, pinId });
    }, [setHoveredPin]);
    const handlePinUnhover = useCallback((nodeId, pinId, e) => {
        setHoveredPin(null);
    }, [setHoveredPin]);
    const handlePinClick = useCallback((nodeId, pinId, e) => {
        e.stopPropagation();
