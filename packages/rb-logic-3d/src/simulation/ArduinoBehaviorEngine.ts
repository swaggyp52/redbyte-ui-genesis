import { LabNet, LabGraph } from '../lab-model/types';
...
export class ArduinoBehaviorEngine {
    ...
    private pins: Map<string, 'INPUT' | 'OUTPUT'> = new Map();
    private pinValues: Map<string, number> = new Map();
    private millisCounter: number = 0;
    private TICK_MS = 50; // Simulation step size (20Hz) - deterministic unit

    constructor(graph: LabGraph) {
        this.graph = graph;
        this.setup();
    }

    // Mapping: "D13" -> NetId
    private getNetForPin(nodeId: string, pinId: string): LabNet | undefined {
    ...
    digitalWrite(pin: number, val: 'HIGH' | 'LOW') {
