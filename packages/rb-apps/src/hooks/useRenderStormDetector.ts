...
type RenderStormGlobal = {
  ...
  maxTotalPerSecond: number;
  samples: Array<{ tMs: number; offenders: Array<{ label: string; renders: number }> }>;
  steps: Array<{ name: string; tMs: number }>;
  warnings: Array<{ label: string; renders: number; tMs: number }>;
  leaks: {
    patched: boolean;
    baseline: { intervals: number; timeouts: number; rafs: number };
    active: { intervals: number; timeouts: number; rafs: number };
    handleTypes: Map<number, 'interval' | 'timeout' | 'raf'>;
  };
};
...
function ensureTimerLeakPatchOnce() {
  ...
  if (global.leaks.patched) return;
  ...
  if (w.__RB_TIMER_LEAK_PATCHED__) return;
  ...
  const markCreated = (id: number, type: 'interval' | 'timeout' | 'raf') => {
    handleTypes.set(id, type);
    if (type === 'interval') active.intervals += 1;
    if (type === 'timeout') active.timeouts += 1;
    if (type === 'raf') active.rafs += 1;
  };
  ...
  const markCleared = (id: number) => {
    ...
    if (type === 'interval') active.intervals = Math.max(0, active.intervals - 1);
    if (type === 'timeout') active.timeouts = Math.max(0, active.timeouts - 1);
    if (type === 'raf') active.rafs = Math.max(0, active.rafs - 1);
  };
  ...
  window.clearInterval = ((id?: number) => {
    if (typeof id === 'number') markCleared(id);
  ...
  if (originalRaf && originalCancelRaf) {
    window.requestAnimationFrame = ((cb: FrameRequestCallback) => {
      const id = originalRaf(cb) as unknown as number;
      markCreated(id, 'raf');
      return id as any;
    }) as any;
    ...
    window.cancelAnimationFrame = ((id: number) => {
      markCleared(id);
      return originalCancelRaf(id as any);
    }) as any;
  }
}
...
function ensureRenderStormApiOnce() {
  ...
  const w = window as any;
  if (w.__RB_RENDER_STORM_API__) return;
  ...
  w.__RB_RENDER_STORM_API__ = {
    ...
    getReport: () => {
      ...
      const report = {
        ...
        maxRendersPerSecondByLabel: Object.fromEntries(
          Object.entries(global.maxPerSecond).sort((a, b) => a[0].localeCompare(b[0]))
        ),
        lastSampleTopOffenders:
          global.samples.length > 0 ? global.samples[global.samples.length - 1].offenders : [],
        leaks: {
...
function startTopOffendersReporterOnce() {
  ...
  setInterval(() => {
    ...
    if (global.samples.length > 180) global.samples.shift();
