...
export const HardwareStatusOverlay = () => {
    ...
    return (_jsx("div", { className: "absolute top-3 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 z-40 scale-90 origin-top", children: _jsxs("div", { className: "flex items-center gap-3 bg-black/90 backdrop-blur-2xl border border-white/10 rounded-full px-4 py-1.5 shadow-2xl", children: [_jsxs("div", { className: "flex bg-black/40 rounded-full p-0.5 border border-white/5 mr-1", children: [_jsx("button", { onClick: () => setTransport('sim'), className: `px-3 py-1 rounded-full text-[8px] font-black transition-all ${!isBridge ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`, children: "SIM" }), _jsx("button", { onClick: () => setTransport('bridge'), className: `px-3 py-1 rounded-full text-[8px] font-black transition-all ${isBridge ? (verified ? 'bg-green-600' : 'bg-red-600') + ' text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`, children: "HARDWARE" })] }), _jsx("div", { className: "h-4 w-px bg-white/10" }), _jsx("div", { className: "flex items-center gap-6", children: Object.entries(detailedStatuses).map(([id, s]) => {
                        ...
                        return (_jsxs("div", { className: "flex flex-col min-w-[80px]", children: [_jsx("span", { className: "text-gray-500 text-[7px] uppercase tracking-widest leading-none", children: label }), _jsxs("div", { className: "flex items-center gap-1.5 mt-0.5", children: [_jsx("div", { className: `w-1 h-1 rounded-full ${dotClass}` }), _jsx("span", { className: `text-[8px] font-bold tracking-tighter ${statusClass}`, children: statusLabel })] })] }, id));
                    }) })] }) }));
};
