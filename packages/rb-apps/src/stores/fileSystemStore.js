 ...
 */
function serializeState(envelope) {
    ...
    return create((set, get) => {
        ...
        return {
            ...
            getAllFiles: () => {
                ...
                function collectFilesFromFolder(folderId) {
            ...
            updateFileContent: (fileId, content) => {
                ...
                const updatedEntries = parentFolder.entries.map((e) => e.id === fileId ? updatedEntry : e);
                const newFs = {
                    ...fs,
                    folders: {
                        ...fs.folders,
 ...
 */
export const useFileSystemStore = ((...args) => {
    if (!_store)
        _store = createFileSystemStore();
    return _store(...args);
});
...
useFileSystemStore.setState = (...a) => {
    if (!_store)
        _store = createFileSystemStore();
    return _store.setState(...a);
};
useFileSystemStore.subscribe = (...a) => {
    if (!_store)
        _store = createFileSystemStore();
