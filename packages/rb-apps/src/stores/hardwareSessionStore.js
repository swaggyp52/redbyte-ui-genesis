...
let booted = false;
const emitHardwareEvent = (detail) => {
...
export const useHardwareSessionStore = create()(devtools(immer((set, get) => {
    ...
    const syncState = (clientState) => {
        set((state) => {
            ...
            if (clientState.status !== 'connected' && wasOnline) {
                Object.keys(state.sessions).forEach((target) => {
    ...
    return {
        ...
        boot: () => {
            ...
            if (typeof window !== 'undefined') {
                window.addEventListener('rb:safe-mode', (event) => {
        ...
        ensureSession: async (target, forcedPort) => {
            ...
            if (target === 'basys3') {
                ...
                candidate = devices.find(d => d.deviceId === 'basys3' ||
                ...
                candidate = devices.find(d => {
                    ...
                    if (modelLower.includes('uno') || modelLower.includes('arduino'))
                        return true;
                    // Check manufacturer
