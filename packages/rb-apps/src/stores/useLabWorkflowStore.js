...
const STEP_ORDER = ['selection', 'specification', 'design', 'simulation', 'hardware', 'verification', 'report'];
export const useLabWorkflowStore = create()(devtools(immer((set, get) => ({
    ...
    getMaxUnlockedStepIndex: () => {
        ...
        const lastCompletedIndex = state.completedSteps.length === 0
            ? -1
            : Math.max(...state.completedSteps.map((s) => STEP_ORDER.indexOf(s)));
