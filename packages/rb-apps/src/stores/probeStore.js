...
function createProbeStore() {
    return create((set, get) => ({
        ...
        addProbe: ({ nodeId, portName, label }) => {
            const existing = get().probes.find((probe) => probe.nodeId === nodeId && probe.portName === portName);
            if (existing) {
        ...
        renameProbe: (id, label) => {
            ...
            if (!nextLabel)
                return;
            const before = get().probes;
            const nextProbes = before.map((probe) => probe.id === id ? { ...probe, label: nextLabel } : probe);
        ...
        toggleProbe: (id) => {
            const before = get().probes;
            const nextProbes = before.map((probe) => probe.id === id ? { ...probe, enabled: !probe.enabled } : probe);
            set({ probes: nextProbes });
            recordAuditTransition({
                scope: 'probe_store',
                action: 'toggle',
        ...
        hasProbe: (nodeId, portName) => {
            return get().probes.some((probe) => probe.nodeId === nodeId && probe.portName === portName);
        },
...
export const useProbeStore = ((...args) => {
    if (!_store)
        _store = createProbeStore();
    return _store(...args);
});
useProbeStore.getState = () => {
    if (!_store)
        _store = createProbeStore();
    return _store.getState();
};
useProbeStore.setState = (...a) => {
    if (!_store)
        _store = createProbeStore();
    return _store.setState(...a);
};
useProbeStore.subscribe = (...a) => {
    if (!_store)
        _store = createProbeStore();
