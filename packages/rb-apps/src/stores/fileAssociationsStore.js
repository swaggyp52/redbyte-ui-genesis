...
export function loadFileAssociations() {
    if (typeof localStorage === 'undefined')
        return null;
    try {
...
export const useFileAssociationsStore = ((...args) => {
    if (!_store)
        _store = createFileAssociationsStore();
    return _store(...args);
});
useFileAssociationsStore.getState = () => {
    if (!_store)
        _store = createFileAssociationsStore();
    return _store.getState();
};
useFileAssociationsStore.setState = (...a) => {
    if (!_store)
        _store = createFileAssociationsStore();
    return _store.setState(...a);
};
useFileAssociationsStore.subscribe = (...a) => {
    if (!_store)
        _store = createFileAssociationsStore();
 ...
 */
export function resolveDefaultTarget(resourceType, extension, eligibleTargets) {
