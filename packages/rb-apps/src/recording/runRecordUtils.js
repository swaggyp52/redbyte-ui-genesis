...
export const tickFromPosition = (x, width, maxTick) => {
    if (width <= 0 || maxTick <= 0)
        return 0;
    const ratio = clamp(x / width, 0, 1);
    return Math.round(ratio * maxTick);
};
export const positionFromTick = (tick, width, maxTick) => {
    if (width <= 0 || maxTick <= 0)
        return 0;
    const ratio = clamp(tick / maxTick, 0, 1);
    return ratio * width;
};
export const normalizeStimulusEvents = (events) => {
    return events
...
export const buildCircuitSummary = (circuit) => {
...
export const compareCircuitSummary = (current, record) => {
    const countsMatch = current.nodeCount === record.nodeCount && current.connectionCount === record.connectionCount;
    if (!countsMatch)
        return 'mismatch';
    const idsMatch = current.nodeIds.length === record.nodeIds.length &&
        current.nodeIds.every((id, index) => id === record.nodeIds[index]);
    return idsMatch ? 'exact' : 'close';
};
export const normalizeCircuit = (circuit) => {
...
export const normalizeStimulus = (events) => {
...
export const normalizeTrace = (trace) => {
    return trace.map((sample) => ({
        tick: sample.tick,
        values: sample.values,
    }));
};
export const digestCircuit = (circuit) => digestValue(normalizeCircuit(circuit));
export const digestStimulus = (events) => digestValue(normalizeStimulus(events));
export const digestTrace = (trace) => digestValue(normalizeTrace(trace));
export const buildDebugOverlayFromSignals = (signals, tick, tickRate) => {
    const nodeSignals = {};
    const portKeySignals = {};
...
export const buildMismatchEntries = (mismatch, probes) => {
...
export const buildMismatchReport = (expectedTrace, actualTrace, stimulus, recentCount) => {
