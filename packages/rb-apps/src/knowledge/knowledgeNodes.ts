...
const _byId = new Map<string, KnowledgeNode>();
const _byGateType = new Map<string, KnowledgeNode[]>();
const _byExampleId = new Map<string, KnowledgeNode[]>();
const _byLabId = new Map<string, KnowledgeNode[]>();
const _byErrorCode = new Map<string, KnowledgeNode[]>();
const _byTag = new Map<string, KnowledgeNode[]>();
const _byHelpTopicId = new Map<string, KnowledgeNode[]>();

function _pushToMap<K>(map: Map<K, KnowledgeNode[]>, key: K, node: KnowledgeNode) {
...
for (const node of KNOWLEDGE_NODES) {
  ...
  for (const tag of node.tags) _pushToMap(_byTag, tag, node);
  for (const gt of node.maps_to.gateTypes ?? []) _pushToMap(_byGateType, gt, node);
  for (const eid of node.maps_to.exampleIds ?? []) _pushToMap(_byExampleId, eid, node);
  for (const lid of node.maps_to.labIds ?? []) _pushToMap(_byLabId, lid, node);
  for (const ec of node.maps_to.errorCodes ?? []) _pushToMap(_byErrorCode, ec, node);
  for (const ht of node.maps_to.helpTopicIds ?? []) _pushToMap(_byHelpTopicId, ht, node);
}
...
export function getNodesByGateType(gateType: string): KnowledgeNode[] {
  return _byGateType.get(gateType) ?? [];
}

export function getNodesByExampleId(exampleId: string): KnowledgeNode[] {
  return _byExampleId.get(exampleId) ?? [];
}

export function getNodesByLabId(labId: string): KnowledgeNode[] {
  return _byLabId.get(labId) ?? [];
}

export function getNodesByErrorCode(errorCode: string): KnowledgeNode[] {
...
export function getNodesByHelpTopicId(topicId: string): KnowledgeNode[] {
...
export function searchKnowledge(query: string): KnowledgeNode[] {
  ...
  for (const node of KNOWLEDGE_NODES) {
    ...
    if (titleLower === lower) score += 10;
    else if (titleLower.startsWith(lower)) score += 6;
    else if (titleLower.includes(lower)) score += 3;

    // Summary match
    if (node.summary.toLowerCase().includes(lower)) score += 2;

    // Tag exact match
    for (const tag of node.tags) {
      if (tag === lower) { score += 4; break; }
      if (tag.includes(lower)) { score += 1; break; }
    }

    // Gate type match
    for (const gt of node.maps_to.gateTypes ?? []) {
      if (gt.toLowerCase() === lower) { score += 5; break; }
    }
