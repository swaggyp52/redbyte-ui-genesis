 ...
 */
export const verilogFromNetlist = (netlist) => {
    ...
    sortedNodes.forEach((node) => {
        ...
        const portList = node.ports
            .map((port) => {
            const net = sortedNets.find((candidate) => (candidate.from.nodeId === node.id && candidate.from.port === port.name) ||
 ...
 */
export const synthesizableVerilogFromNetlist = (netlist, options = {}) => {
    ...
    const nodeIdToPortName = new Map();
    ...
    const internalWires = sortedNets.filter((net) => {
        ...
        const fromIsIO = INPUT_NODE_TYPES.includes(sortedNodes.find((n) => n.id === net.from.nodeId)?.type || '');
        const toIsIO = OUTPUT_NODE_TYPES.includes(sortedNodes.find((n) => n.id === net.to.nodeId)?.type || '');
    ...
    const constraintsXdc = generateConstraintsXdc(board, inputPorts, outputPorts, includeClock || clockNodes.length > 0);
    return {
 ...
 */
function generatePrimitivesLibrary(nodes) {
 ...
 */
function generateConstraintsXdc(board, inputPorts, outputPorts, includeClock) {
    if (board !== 'basys3') {
