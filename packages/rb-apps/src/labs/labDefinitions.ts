...
Opcode table:
  ...
  },
  {
    ...
    learningGoal: 'Build and test the four core storage elements while distinguishing level-sensitive and edge-triggered behavior.',
    whatToDo: 'Use the blank scaffold to build four separate test circuits: D latch, rising-edge D flip-flop, T flip-flop with CLR tied low, and JK flip-flop with CLR tied low. Verify each device before taking it to Basys3.',
    starterId: 'flipflop-seq',
    starterExampleId: '21_lab6-flipflop-starter',
    basys3Required: true,
    buildSteps: [
      'Build the D latch test path from the DL_D / DL_G scaffold so Q follows D only while G is asserted.',
      'Build the D flip-flop test path from the DFF_D / DFF_CLK scaffold so Q updates only on the rising edge.',
      'Build the T flip-flop test path from the TFF_T / TFF_CLK scaffold and keep CLR tied low with the provided Ground node.',
      'Build the JK flip-flop test path from the JK_J / JK_K / JK_CLK scaffold and keep CLR tied low with the provided Ground node.',
    ],
    simulateChecks: [
      'D latch: prove Q follows D when G=1 and holds when G=0.',
      'D flip-flop: prove Q changes only on a rising clock edge and holds between edges.',
      'T flip-flop: prove T=0 holds, T=1 toggles, and CLR=0 is held inactive.',
      'JK flip-flop: prove 00 hold, 01 reset, 10 set, 11 toggle, with data stable while CLK changes.',
    ],
    hardwareSteps: [
      'Map each section to Basys3 switches/buttons and LEDs so you can demo the four behaviors one device at a time.',
      'Keep CLR wired to Ground for TFF and JKFF; do not leave clear floating.',
      'Use Verify as the proof surface first, then use slow manual toggles on Basys3 as the classroom demo of the same behavior.',
    ],
    submitEvidence: [
      'Bundle includes readable Verify traces for latch transparency, DFF edge capture, TFF toggle/hold, and JKFF hold/reset/set/toggle.',
      'Bundle includes the Basys3 mapping used for the hardware demo.',
    ],
    commonMistakes: [
      'Dropping finished storage elements into the canvas without wiring the required test path from the scaffold.',
      'Leaving CLR floating instead of tying it to Ground for TFF and JKFF.',
      'Changing D, T, J, or K at the same time as the clock edge and then blaming the flip-flop.',
    ],
    rubric: [
      'All four device test paths are completed from the unsolved scaffold.',
      'Verify evidence clearly proves level-sensitive vs rising-edge behavior for the correct device.',
      'Basys3 demo wiring matches the verified circuit and keeps clear inputs deterministic.',
    ],
    requiredTop: 'top',
    requiredBoardPreset: 'basys3',
    requireSimEvidence: true,
  ...
  },
  {
    ...
    learningGoal: 'Design a Moore FSM that detects valid 3-bit groups in a serial bit stream and unlocks after 4 consecutive valid groups.',
    whatToDo: 'Build the sequence detector using DFlipFlops. ENTER (SW5) acts as the manual clock — toggle it once per input bit to advance the FSM. IN0 (SW6) is the serial data bit; IN1/IN2 are unused in the starter checkpoints. RESET (SW4) clears all state. LOCK (LED1) goes high only after 4 valid groups in 12 bits.',
    starterId: 'security-lock-fsm-basys3',
    starterExampleId: '23_lab8-fsm-lock-starter-basys3',
    basys3Required: true,
    buildSteps: [
      'Use DFlipFlops as the state-storage elements. Every DFlipFlop CLK pin must connect to ENTER (SW5) — this is the manual clock that advances the FSM one step per toggle.',
      'Connect RESET (SW4) to the RST pin of every DFlipFlop. Toggling RESET clears all state so you can re-run sequences.',
      'Implement a 2-bit position counter (pos1, pos0) that cycles through positions 0→1→2→0 on each ENTER pulse. This tracks where you are within the current 3-bit group.',
      'Capture IN0 at position 0 (bit_at_p0) and position 1 (bit_at_p1) using EN-gated DFlipFlops. EN = high only at the correct position.',
      'At position 2 (the boundary), compute group_valid = NOT(IN0) AND XOR(bit_at_p0, bit_at_p1). Valid groups are 010 and 100.',
      'Use a 4-stage milestone chain (m1→m2→m3→m4). Each milestone advances only when group_valid is true at position 2. m4=1 means 4 consecutive valid groups.',
      'Connect m4.Q → LOCK (LED1). LOCK stays high once reached.',
    ],
    simulateChecks: [
      'Run the invalid-path checkpoint: bitstream 110010010100 on IN0 must never unlock (LOCK stays 0 for all 12 pulses).',
      'Run the valid-path checkpoint: bitstream 010100010100 on IN0 must unlock exactly at clock pulse 12 (LOCK goes 1).',
      'Both checkpoints must pass before submitting.',
    ],
    hardwareSteps: [
      'Pin assignments are pre-mapped: SW4=RESET, SW5=ENTER (manual clock), SW6=IN0, SW7=IN1 (unused), SW8=IN2 (unused), LED1=LOCK.',
      'Export the project from the Export tab. All pins should map cleanly with no errors.',
      'Load into Vivado: unzip, run vivado_import.tcl, then Synthesis → Implementation → Generate Bitstream.',
      'Program the Basys3. Use SW4 to RESET first (toggle it on then off).',
      'To test: set IN0 (SW6) to 0 or 1, then briefly toggle ENTER (SW5) once to clock that bit in. Repeat for all 12 bits.',
      'For the valid sequence 010100010100: alternately set IN0=0/1/0/1/0/0/0/1/0/1/0/0 and toggle ENTER after each. After bit 12, LOCK (LED1) should light up.',
      'Use RESET (SW4) to restart between test sequences.',
    ],
    submitEvidence: [
      'Bundle includes passing invalid-path and valid-path checkpoint evidence.',
      'Both FSM paths (unlock and no-unlock) must be demonstrated in simulation before export.',
    ],
    commonMistakes: [
      'Not connecting ENTER (SW5) to ALL DFlipFlop CLK pins. Every flip-flop in the design must share the same clock — any DFF with no CLK connection is permanently stuck.',
      'Not connecting RESET (SW4) to the RST pin of every DFlipFlop. Without this, you cannot clear state and re-test sequences on the board.',
      'Treating ENTER as a data input instead of a clock. ENTER must only connect to CLK ports of DFlipFlops — never to AND/OR/XOR logic gates.',
    ],
    rubric: [
      'LOCK never goes high for the invalid bitstream 110010010100.',
      'LOCK goes high exactly at clock 12 for the valid bitstream 010100010100.',
      'Both simulation checkpoints pass.',
      'Basys3 export is clean (no pin errors, all DFFs clocked by ENTER).',
    ],
    requiredTop: 'top',
    requireSimEvidence: true,
    submitGates: [
      { id: 'fsm-paths', severity: 'block', message: 'Both the invalid-path and valid-path simulation checkpoints must pass before submitting.', stage: 'simulate' },
    ],
  },
  {
    id: 'freeplay',
...
const LAB_STAGE_TEACHING_BY_ID: Record<string, Partial<LabStageTeachingMap>> = {
  ...
  'lab-6': {
    simulate: {
      concept: 'Sequential correctness comes from proving each device updates at the right time and holds at the right time.',
      commonMistake: 'Treating one passing edge case as enough without proving hold, set, reset, and toggle behavior separately.',
      goodLooksLike: 'The waveform shows one device at a time, with inputs stable around the active edge and the expected hold/update behavior visible.',
    },
  },
  'lab-7': {
    hardware: {
...
const LAB_EXPECTED_BEHAVIOR_BY_ID: Record<string, LabExpectedBehaviorVisual> = {
  ...
  'lab-6': {
    kind: 'truth-table',
    title: 'Lab 6 proof checklist',
    columns: ['Device', 'Condition', 'Clock/G', 'Expected Q'],
    rows: [
      ['D latch', 'D=1', 'G=1', 'Q=1'],
      ['D latch', 'D=0 after hold', 'G=0', 'Q holds'],
      ['DFF', 'D=1', '↑', 'Q=1'],
      ['TFF', 'T=1, CLR=0', '↑', 'Q toggles'],
      ['JKFF', 'J=1, K=0, CLR=0', '↑', 'Q=1'],
    ],
    note: 'Tie CLR low with Ground for TFF/JKFF, and keep data stable while the clock changes.',
  },
  'lab-7': {
    kind: 'waveform',
