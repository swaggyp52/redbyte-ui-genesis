...
import { LAB_2_ADDER_CONTENT } from './lab2-adder';
export const LAB_1_CONTENT = [
    {
        id: 'intro',
...
export function getLabMetadata(labId) {
...
export function getLabStepTitles(labId) {
    const content = LABS[labId];
    if (!content)
        return [];
    if (Array.isArray(content))
        return content.map((s) => s.title);
    return content.steps.map((s) => s.title ?? s.id ?? 'Step');
}
