...
export interface Experiment {
    ...
    name: string;
    description: string;
    compute: (inputs: { SW: number; BTN: number }, tick: number, state?: any) => {
...
export const EXPERIMENTS: Record<string, Experiment> = {
    ...
    'counter': {
        ...
        compute: (inputs, tick, state) => {
            ...
            if (btnC) nextCount = 0;
            else if (enable) nextCount = (state.count + 1) & 0xFF; // 8-bit wrap
    ...
    'traffic': {
        ...
        compute: (inputs, tick, state) => {
            ...
            let led = 0;
            if (nextFsm === 0) led = 0b100; // Red
            if (nextFsm === 1) led = 0b001; // Green
            if (nextFsm === 2) led = 0b010; // Yellow
