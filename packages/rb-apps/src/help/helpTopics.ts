...
export interface HelpTopic {
  id: string;
  title: string;
  errorCodes?: string[]; // Related error codes for search
  steps: string[]; // 2-8 actionable steps
}
 ...
 */
export function getAllReferencedErrorCodes(): Set<string> {
...
export function searchHelpTopics(query: string): HelpTopic[] {
  ...
  return HELP_TOPICS.filter((topic) => {
    ...
    // Match error codes
    if (topic.errorCodes?.some((code) => code.toLowerCase().includes(lowerQuery))) return true;
    
    // Match steps
    if (topic.steps.some((step) => step.toLowerCase().includes(lowerQuery))) return true;
 ...
 */
export function getTopicsByErrorCode(errorCode: string): HelpTopic[] {
  return HELP_TOPICS.filter((topic) =>
    topic.errorCodes?.some((code) => code === errorCode)
  );
}
