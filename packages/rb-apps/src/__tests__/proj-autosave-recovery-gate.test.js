...
import { buildRbprojAutosaveRecord, getCanonicalProjectAutosaveKey, loadRbprojAutosave, saveRbprojAutosave, } from '../utils/rbprojAutosave';
...
function normalizeCircuitForGate(circuit) {
...
describe('Project autosave recovery gate (RBProject codec, canonical key)', () => {
    ...
    it('writes autosave (rb:autosave:<projectId>) and restores identical canonical circuit state', () => {
        ...
        const edited = actions.reduce((state, action) => labReducer(state, action), base);
