...
const FIXTURE_PROJECT_PATH = join(
  process.cwd(),
...
async function loadLabFixture(projectPath: string): Promise<LabProjectV1> {
  const projectText = readFileSync(projectPath, 'utf-8');
  return JSON.parse(projectText) as LabProjectV1;
}

function normalizeForComparison(project: RBProject): unknown {
  ...
  const metaSafe = meta
    ? Object.fromEntries(
        Object.entries({ ...meta, appVersion: undefined, gitCommit: undefined }).filter(([, value]) => value !== undefined)
      )
    : undefined;
