...
vi.mock('@redbyte/rb-utils', async (importOriginal) => {
    ...
    return {
        ...actual,
        useSettingsStore: (selector) => selector ? selector(mockSettingsState) : mockSettingsState,
...
vi.mock('@redbyte/rb-logic-view', async (importOriginal) => {
    ...
    return {
        ...actual,
        useLogicViewStore: Object.assign((selector) => {
            const mockState = {
...
describe.skip('LogicPlaygroundApp - Circuit Persistence', () => {
    ...
    describe('Autosave with 5s debounce', () => {
        it('should not autosave immediately after circuit change', async () => {
            ...
            const { container } = render(_jsx(Component, { initialFileId: fileId }));
        ...
        it('should autosave after 5 seconds of idle', async () => {
            ...
            const fileId = createFile('documents', 'test.rblogic', JSON.stringify(initialCircuit));
            const Component = LogicPlaygroundApp.component;
            render(_jsx(Component, { initialFileId: fileId }));
    ...
    describe('Ctrl+O Open Flow', () => {
        it('should open file picker modal on Ctrl+O', async () => {
            ...
            const { createFile } = useFileSystemStore.getState();
            // Create a test file
            const circuit = { version: '1', nodes: [], connections: [] };
    ...
    describe('Open With Association', () => {
        it('should load circuit when opened via resourceId', async () => {
            ...
            const Component = LogicPlaygroundApp.component;
            render(_jsx(Component, { resourceId: fileId, resourceType: "file" }));
            await waitFor(() => {
        ...
        it('should create new file if resourceId does not exist', async () => {
            ...
            render(_jsx(LogicPlaygroundApp, { manifest: LogicPlaygroundApp.manifest, resourceId: nonExistentId, resourceType: "file" }));
            await waitFor(() => {
        ...
        it('should update window title with filename', async () => {
            ...
            const Component = LogicPlaygroundApp.component;
            render(_jsx(Component, { windowId: "test-window", resourceId: fileId, resourceType: "file" }));
            await waitFor(() => {
        ...
        it('should show dirty indicator in window title when unsaved', async () => {
            ...
            const Component = LogicPlaygroundApp.component;
            render(_jsx(Component, { windowId: "test-window", initialFileId: fileId }));
    ...
    describe('Share Polish Features', () => {
        ...
        describe('Decode Error Recovery', () => {
            it('should show error modal when URL has invalid circuit param', async () => {
                ...
                window.location = { search: '?circuit=invalid-garbage' };
                const Component = LogicPlaygroundApp.component;
                render(_jsx(Component, {}));
            ...
            it('should have link to report issue', async () => {
                delete window.location;
                window.location = { search: '?circuit=corrupt' };
                const Component = LogicPlaygroundApp.component;
                render(_jsx(Component, {}));
                await waitFor(() => {
                    expect(screen.getByText(/invalid share link/i)).toBeInTheDocument();
            ...
            it('should close error modal on Close button', async () => {
                const user = userEvent.setup();
                delete window.location;
                window.location = { search: '?circuit=bad' };
                const Component = LogicPlaygroundApp.component;
                render(_jsx(Component, {}));
                await waitFor(() => {
                    expect(screen.getByText(/invalid share link/i)).toBeInTheDocument();
