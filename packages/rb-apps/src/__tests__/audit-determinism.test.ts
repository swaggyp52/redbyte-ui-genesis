...
const runDeterministicSequence = async (): Promise<string> => {
  localStorage.clear();
  ...
  try {
    ...
    const baseCircuit: SerializedCircuitV1 = { version: 'v1', nodes: [], connections: [] };
    const updatedCircuit: SerializedCircuitV1 = {
      version: 'v1',
      nodes: [
        {
          id: 'node-1',
    ...
    vi.useRealTimers();
    delete (window as any).__RB_AUDIT__;
  }
};
