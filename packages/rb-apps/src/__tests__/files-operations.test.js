...
describe('PHASE_W: Files operations', () => {
    ...
    describe('Create folder operation', () => {
        it('opens modal with Cmd/Ctrl+Shift+N and creates folder', () => {
            const { container } = render(_jsx(FilesComponent, {}));
    ...
    describe('Create file operation', () => {
        it('opens modal with Cmd/Ctrl+N and creates file', () => {
            ...
            // Change name
            const input = screen.getByDisplayValue('New File.txt');
            act(() => {
    ...
    describe('Rename operation', () => {
        it('opens modal with F2 and renames selected entry', () => {
            ...
            const mainContainer = container.querySelector('[tabIndex="0"]');
            // Navigate to Desktop
            const desktopButton = screen.getAllByRole('button', { name: 'Desktop' }).find((el) => el.className.includes('w-full'));
            act(() => {
            ...
            // Change name
            const input = screen.getByDisplayValue('Project Files');
            act(() => {
    ...
    describe('Delete operation', () => {
        it('opens modal with Delete key and deletes selected entry', () => {
            ...
            const documentsButton = screen.getAllByRole('button', { name: 'Documents' }).find((el) => el.className.includes('w-full'));
            act(() => {
    ...
    describe('Edge cases', () => {
        ...
        it('operations are shared across window instances (global store)', () => {
            const { container: container1 } = render(_jsx(FilesComponent, {}));
            const { container: container2 } = render(_jsx(FilesComponent, {}));
            ...
            const input1 = screen.getAllByDisplayValue('New Folder')[0];
            act(() => {
    ...
    describe('PHASE_W Stage 3: Keyboard priority and modal guards', () => {
        ...
        it('Escape priority: closes modal first, then window', () => {
            const onClose = vi.fn();
            const { container } = render(_jsx(FilesComponent, { onClose: onClose }));
            const mainContainer = container.querySelector('[tabIndex="0"]');
            // Open modal
            act(() => {
                fireEvent.keyDown(mainContainer, { key: 'N', metaKey: true, shiftKey: true });
        ...
        it('blocks "Open in Playground" (Cmd+Enter) when modal is open', () => {
            ...
            const { container } = render(_jsx(FilesComponent, { onDispatchIntent: onDispatchIntent }));
            const mainContainer = container.querySelector('[tabIndex="0"]');
            // Navigate to Desktop
            const desktopButton = screen.getAllByRole('button', { name: 'Desktop' }).find((el) => el.className.includes('w-full'));
            act(() => {
    ...
    describe('PHASE_X: Cross-App File Actions', () => {
        it('Cmd/Ctrl+Shift+Enter opens "Open With..." modal for selected file', () => {
            ...
            const mainContainer = container.querySelector('[tabIndex="0"]');
            // Navigate to Home
            const homeButton = screen.getAllByRole('button', { name: 'Home' }).find((el) => el.className.includes('w-full'));
            act(() => {
        ...
        it('"Open With..." modal supports keyboard navigation', () => {
            ...
            const modal = screen.getByText('Open With...').closest('div[tabIndex="0"]');
    ...
    describe('PHASE_Y: Open-With Payload + Target Consumption', () => {
        ...
        it('Files dispatches different resourceIds for different files', () => {
            ...
            expect(onDispatchIntent).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('PHASE_Z: Eligibility Predicates', () => {
        it('Logic Playground eligible only for .rblogic files', async () => {
            ...
            const playgroundTarget = FILE_ACTION_TARGETS.find((t) => t.id === 'logic-playground');
        ...
        it('Text Viewer eligible for .txt and .md files', async () => {
            const { FILE_ACTION_TARGETS } = await import('../apps/files/fileActionTargets');
            const textViewerTarget = FILE_ACTION_TARGETS.find((t) => t.id === 'text-viewer');
        ...
        it('getFileActionTargets filters by eligibility predicate', async () => {
            ...
            const rblogicFile = { id: 'f1', name: 'circuit.rblogic', type: 'file' };
            const txtFile = { id: 'f2', name: 'notes.txt', type: 'file' };
            const mdFile = { id: 'f3', name: 'readme.md', type: 'file' };
            const folder = { id: 'd1', name: 'MyFolder', type: 'folder' };
            const unknownFile = { id: 'f4', name: 'data.json', type: 'file' };
    ...
    describe('PHASE_Z: FILE_ACTION_TARGETS Registry', () => {
        ...
        it('All targets have deterministic isEligible predicates', async () => {
            const { FILE_ACTION_TARGETS } = await import('../apps/files/fileActionTargets');
            FILE_ACTION_TARGETS.forEach((target) => {
