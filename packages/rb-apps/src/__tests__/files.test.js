...
function getSidebarButton(name) {
    return screen.getAllByRole('button', { name }).find((el) => el.className.includes('w-full'));
}
describe('Files app lifecycle', () => {
    it('renders sidebar with fixed roots', () => {
        render(_jsx(FilesComponent, {}));
    ...
    it('maintains independent state per instance', () => {
        const { container: container1, rerender: rerender1 } = render(_jsx(FilesComponent, {}));
...
describe('PHASE_V: Breadcrumb navigation', () => {
    it('renders breadcrumb for root folder (Home)', () => {
        ...
        const activeBreadcrumb = breadcrumbs.find((el) => el.className.includes('text-cyan-400') && el.className.includes('font-semibold'));
    ...
    it('clicking breadcrumb segment navigates to that folder', () => {
        ...
        const clickableHomeBreadcrumb = homeBreadcrumbs.find((el) => el.className.includes('hover:bg-slate-800'));
    ...
    it('breadcrumb shows deep path (Home / Desktop / Project Files)', () => {
        ...
        const homeBreadcrumb = screen.getAllByText('Home').find((el) => el.className.includes('text-slate-300'));
        const desktopBreadcrumb = screen.getAllByText('Desktop').find((el) => el.className.includes('text-slate-300'));
        const projectBreadcrumbs = screen.getAllByText('Project Files');
        const activeProjectBreadcrumb = projectBreadcrumbs.find((el) => el.className.includes('text-cyan-400') && el.className.includes('font-semibold'));
        expect(homeBreadcrumb).toBeTruthy();
...
describe('PHASE_V: Open With workflow', () => {
    ...
    it('Open in Logic Playground button dispatches correct intent', () => {
        ...
        render(_jsx(FilesComponent, { onDispatchIntent: onDispatchIntent }));
