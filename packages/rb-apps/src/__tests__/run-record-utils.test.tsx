...
import type { RunStimulusEvent, RunTraceSample } from '../recording/runRecord';
import {
  tickFromPosition,
  positionFromTick,
...
describe('run record utils', () => {
  ...
  it('builds mismatch report with expected/actual diff', () => {
    const expected: RunTraceSample[] = [
      { tick: 1, values: { p1: 1, p2: 0 } },
      { tick: 2, values: { p1: 1, p2: 1 } },
    ];
    const actual: RunTraceSample[] = [
      { tick: 1, values: { p1: 1, p2: 0 } },
      { tick: 2, values: { p1: 0, p2: 1 } },
    ];
    const stimulus: RunStimulusEvent[] = [
  ...
  it('produces stable circuit digests across reorderings', () => {
    ...
    expect(digestCircuit(circuitA as any)).toBe(digestCircuit(circuitB as any));
  });
