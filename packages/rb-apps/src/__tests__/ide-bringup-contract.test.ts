...
function buildFixtureRuntimeVerifyRun(project: RBProject): RuntimeVerifyRun {
  ...
  return {
    ...
    meta: {
      ...
      samplePoint: scheduleContract.schedule === 'clocked_macro' ? 'post-rising-edge' : 'steady-state',
      tick0Meaning: scheduleContract.schedule === 'clocked_macro' ? 'initial-state' : null,
