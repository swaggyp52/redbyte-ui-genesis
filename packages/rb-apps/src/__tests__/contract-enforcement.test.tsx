...
const BASE_CIRCUIT: SerializedCircuitV1 = { version: 'v1', nodes: [], connections: [] };
...
describe('contract enforcement', () => {
  it('rejects file writes without metadata', () => {
    expect(() => createFile('NoMeta', BASE_CIRCUIT, undefined as any)).toThrow(
  ...
  it('rejects file updates without derived_from', () => {
    ...
    expect(() =>
      updateFile(file.id, BASE_CIRCUIT, {
        kind: 'source',
        schema_version: 'v1',
        created_by: 'test-suite',
      })
  ...
  it('rejects disallowed app outputs', () => {
    expect(() => assertAppOutput('submission-inspector', 'rb-lab.zip')).toThrow(
      'not allowed'
    );
  });
