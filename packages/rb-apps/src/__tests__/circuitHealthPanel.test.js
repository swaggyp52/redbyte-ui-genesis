...
describe('CircuitHealthPanel', () => {
    ...
    it('supports ignoring issues for the session', async () => {
        ...
        render(_jsx(CircuitHealthPanel, { circuit: TEST_CIRCUIT }));
    ...
    it('focuses issues via the provided callback', async () => {
        const user = userEvent.setup();
        render(_jsx(CircuitHealthPanel, { circuit: TEST_CIRCUIT, onFocusNode: (nodeId) => useViewStateStore.getState().setHighlightedNode(nodeId, 0) }));
