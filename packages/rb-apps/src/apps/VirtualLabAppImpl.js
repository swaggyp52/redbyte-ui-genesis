...
const VirtualLabAppComponent = ({ resourceId, resourceType, windowId, onOpenApp }) => {
    ...
    const open2DEditor = useCallback(() => {
        ...
        const shellApi = window.__RB_SHELL_E2E__;
    ...
    const promptEditIn2D = useCallback((reason) => {
        const now = Date.now();
        if (now - lastEditToastRef.current < 1500)
            return;
    ...
    const windowMinimized = useWindowStore(useMemo(() => (state) => {
        if (!windowId)
            return false;
    ...
    const openGuardrail = (config) => setGuardrail(config);
    // Selectors
    ...
    return (_jsxs("div", { className: "flex h-full w-full bg-[#1e1e1e] text-gray-200 overflow-hidden relative", children: [guardrail && (_jsx(GuardrailConfirmModal, { isOpen: Boolean(guardrail), title: guardrail.title, message: guardrail.message, lossItems: guardrail.lossItems, confirmLabel: guardrail.confirmLabel, confirmTone: guardrail.confirmTone, onConfirm: () => {
                    ...
                    closeGuardrail();
                                        ...
                                        }) }), _jsx("button", { onClick: handleStartLab, disabled: !activeTemplate || !!labSession, className: `w-full px-3 py-2 rounded text-xs ${labSession ? 'bg-gray-700 text-gray-400 cursor-not-allowed' : 'bg-green-700/70 hover:bg-green-600 text-white'}`, children: "Start Lab" }), labSession && activeTemplate && (_jsx("button", { onClick: () => openGuardrail({
                                ...
                                ${playbackMode === 'replay' ? 'bg-amber-900 accent-amber-500' : 'bg-gray-700 accent-blue-500'}` }), _jsx("span", { className: "text-xs text-gray-500 font-mono", children: tick })] })] }), bookmarks.length > 0 && (_jsxs("div", { className: "absolute bottom-6 right-4 bg-black/60 border border-gray-700 rounded p-3 text-[10px] text-gray-300 w-48", children: [_jsx("div", { className: "text-[10px] uppercase text-gray-500 mb-2", children: "Bookmarks" }), _jsx("div", { className: "space-y-1", children: bookmarks.map((mark, index) => (_jsxs("button", { onClick: () => {
                                        ...
                                        scrub(mark.tick);
                                                            ...
                                                            onExport: handleExportCapsule,
                                                        }), className: "text-[10px] text-red-400 hover:text-red-300", children: "Reset Layout" })] }), gradeReport.checks
                                        ...
                                        })() })] })] })] }), _jsx(GuidedLabSidebar, {}), !is3DViewOnly && _jsx(HardwareAutoAdopt, {})] }));
};
...
export default VirtualLabAppWithBoundary;
