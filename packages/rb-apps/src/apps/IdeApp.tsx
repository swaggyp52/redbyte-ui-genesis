...
import {
  ...
  saveLegacyAutosaveRaw,
} from './ide/projectStorageFacade';
...
const IDE_RUNTIME_STORAGE_KEY = PROJECT_RUNTIME_STORAGE_KEY;
...
export const IdeApp: React.FC = () => {
  ...
  const [projectStorageWarning, setProjectStorageWarning] = useState<string | null>(null);
  ...
  const savedProjectHashRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    const handleFailure = (event: Event) => {
      const detail = (event as CustomEvent<{ status?: string; key?: string }>).detail;
      if (!detail) return;
      const health = getProjectStorageHealth();
      const support = ` (${health.recoveryPointCount} recovery point${health.recoveryPointCount === 1 ? '' : 's'} available)`;
      if (detail.status === 'quota') {
        setProjectStorageWarning(`Browser storage is full. Download a backup, free some browser storage, then retry saving.${support}`);
        return;
      }
      if (detail.status === 'conflict') {
        setProjectStorageWarning('This project changed in another tab. Download a backup or reload before saving over the newer copy.');
        return;
      }
      setProjectStorageWarning('Project storage could not save cleanly. Download a backup before continuing.');
    };
    window.addEventListener('redbyte:project-storage-failure', handleFailure);
    return () => window.removeEventListener('redbyte:project-storage-failure', handleFailure);
  }, []);
  ...
  const handleRestoreAutosave = useCallback(() => {
    try {
      const saved = loadLegacyAutosaveRaw();
      if (!saved) {
  ...
  savedProjectHashRef.current = savedProjectHash;
  ...
  const handleSaveProject = useCallback(() => {
    ...
    if (!snapshot) {
      setProjectStorageWarning('Save did not complete. Download a backup, then retry saving.');
      setLastSavedAt('Save failed - backup recommended');
      return;
    }
    setProjectStorageWarning(null);
  ...
  const handleDownloadStorageBackup = useCallback(() => {
    const backupText = exportProjectBackup();
    const blob = new Blob([backupText], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `${sanitizeDownloadStem(projectName || projectId || 'redbyte-project')}-storage-backup.json`;
  ...
  const handleRetryProjectSave = useCallback(() => {
    setProjectStorageWarning(null);
    handleSaveProject();
  }, [handleSaveProject]);
  ...
  const handleRenameProject = useCallback(
    (nextName: string) => {
      ...
      setProjectStorageWarning('Rename changed the open project but did not save to browser storage. Download a backup before refreshing.');
    },
    [
      activeExampleId,
      activeMode,
  ...
  const handleSaveAsProject = useCallback(() => {
    ...
    if (!snapshot) {
      setProjectStorageWarning('Save As did not complete. Download a backup, then retry saving.');
      setLastSavedAt('Save As failed - backup recommended');
      return;
    }
    setProjectStorageWarning(null);
    setProjectIdentity({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (snapshot) {
        ...
        setProjectStorageWarning('Autosave did not complete. Download a backup before refreshing.');
      }
  ...
  useEffect(() => {
    if (!hasCircuit) return;
    try {
      const saved = saveLegacyAutosaveRaw(JSON.stringify(exportProject));
      if (saved.status !== 'ok') {
        setProjectStorageWarning('Project autosave did not complete. Download a backup before refreshing.');
      }
    } catch {
      // ignore quota errors
    }
  ...
  useEffect(() => {
    const handleBeforeUnload = (event: BeforeUnloadEvent) => {
      ...
      if (projectHashRef.current && projectHashRef.current !== savedProjectHashRef.current) {
        event.preventDefault();
        event.returnValue = '';
      }
    };
    ...
    >
      {autosaveAvailable && projectKind === 'home' && !hasCircuit && (
        <div className="ide-autosave-banner" data-testid="ide-autosave-banner">
          ...
          <button onClick={() => { setAutosaveAvailable(false); clearLegacyAutosave(); }}>Dismiss</button>
        </div>
      )}
      ...
      {projectStorageWarning && (
        <div
          className="ide-autosave-banner ide-storage-recovery-banner"
          data-testid="ide-storage-recovery-banner"
          role="alert"
        >
          <span><strong>Project save needs attention.</strong> {projectStorageWarning}</span>
          <button onClick={handleDownloadStorageBackup}>Download backup</button>
          <button onClick={handleRetryProjectSave}>Retry save</button>
          <button onClick={() => setProjectStorageWarning(null)}>Dismiss</button>
        </div>
      )}
      <IdeTopBar
...
function sanitizeDownloadStem(value: string): string {
  const normalized = value
    ...
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
  return normalized || 'redbyte-project';
}
