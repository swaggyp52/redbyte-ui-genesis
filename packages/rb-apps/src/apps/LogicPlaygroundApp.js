...
import { saveSnapshot, loadSnapshot, initSnapshotSystem, clearAllSnapshots } from '../utils/snapshotSystem';
...
const LogicPlaygroundInner = ({ windowId, initialFileId, initialExampleId, resourceId, resourceType, recoveredData, onOpenApp, dockTab, dockSubview, showExamples, registerStateAccessor, unregisterStateAccessor, determinismRecorder, debugFlags, }) => {
    ...
    const applyLayoutSnapshot = useCallback((layout) => {
        ...
        if (layout.perspective)
            storeSetPerspective(layout.perspective);
        if (typeof layout.splitRatio === 'number')
            setSplitRatio(layout.splitRatio);
        if (layout.rightDockState)
            setRightDockState(layout.rightDockState);
        if (layout.rightDockTab)
            setRightDockTab(layout.rightDockTab);
        if (typeof layout.showHelpDock === 'boolean')
            setShowHelpDock(layout.showHelpDock);
        if (layout.helpDockSection)
            setHelpDockSection(layout.helpDockSection);
        ...
        try {
            ...
            if (projectRef?.kind === 'rbproj' && typeof projectRef.projectId === 'string') {
                ...
                if (projectPayload && applyProjectRef.current) {
                    ...
                    if (payload.circuit) {
                        useCircuitStore.getState().updateCircuit(payload.circuit, { skipHistory: true, enforceLimits: true });
                        setCircuit(payload.circuit);
                        engineRef.current.setCircuit(payload.circuit);
                    }
                    if (payload.layout) {
    ...
    const legacyRbprojAutosaveKey = useMemo(() => getRbprojAutosaveKey('logic-playground', windowId), [windowId]);
    ...
    useEffect(() => {
        if (dockTab) {
            ...
            store.openDock(dockTab, dockSubview || undefined);
        }
        if (showExamples) {
    ...
    useEffect(() => {
        ...
        const undoHandler = (event) => {
            ...
            if (detail?.windowId && windowId && detail.windowId !== windowId)
                return;
            handleUndo();
        };
        ...
        window.addEventListener('rb:history-undo', undoHandler);
        window.addEventListener('rb:history-redo', redoHandler);
        return () => {
            window.removeEventListener('rb:history-undo', undoHandler);
            window.removeEventListener('rb:history-redo', redoHandler);
        };
    ...
    const openGuardrail = useCallback((config) => {
        setGuardrail(config);
    }, []);
    const confirmReplacement = useCallback((actionDescription, onProceed) => {
        openGuardrail({
    ...
    useEffect(() => {
        if (!recoveredData || hasAppliedRecoveryRef.current)
            return;
        recoveredDataRef.current = recoveredData;
        try {
            const snapshot = recoveredData;
            const payload = snapshot?.payload ?? {};
            if (payload.project) {
    ...
    useEffect(() => {
        ...
        window.addEventListener('rb:export-request', handler);
        return () => window.removeEventListener('rb:export-request', handler);
    ...
    useEffect(() => {
        ...
        window.addEventListener('rb:hardware-session', handler);
        return () => window.removeEventListener('rb:hardware-session', handler);
    }, [circuit, getLayoutSnapshot, safeMode]);
    // rb:open-dock event bridge — lets Shell (and other packages) open a dock tab+subview
    useEffect(() => {
        if (typeof window === 'undefined')
            return;
        const VALID_TABS = new Set(['inspector', 'health', 'learn', 'probes', 'record', 'chips', 'io']);
        const VALID_SUBVIEWS = new Set(['lessons', 'help', 'manual']);
        const handler = (e) => {
            const detail = e.detail;
            if (!detail || typeof detail.tab !== 'string' || !VALID_TABS.has(detail.tab))
                return;
            ...
            useLayoutStore.getState().openDock(detail.tab, subview, errorCode);
        };
    ...
    return (_jsx(ErrorBoundary, { children: _jsxs("div", { className: "h-full flex flex-col min-h-0 min-w-0 bg-gray-900 text-white", "data-testid": "logic-playground-root", children: [guardrail && (_jsx(GuardrailConfirmModal, { isOpen: Boolean(guardrail), title: guardrail.title, message: guardrail.message, lossItems: guardrail.lossItems, confirmLabel: guardrail.confirmLabel, confirmTone: guardrail.confirmTone, onConfirm: () => {
                        guardrail.onConfirm();
                        closeGuardrail();
                    }, onCancel: closeGuardrail, onExport: guardrail.onExport })), unifiedRecovery.mode === 'autosave' && (_jsx(GuardrailConfirmModal, { isOpen: true, title: "Restore autosave?", message: `An autosave is available from ${new Date(unifiedRecovery.autosaveMeta?.savedAtMs ?? Date.now()).toLocaleString()}. Restore it now?`, lossItems: ['Discarding autosave will permanently delete the autosaved copy.'], confirmLabel: "Restore", cancelLabel: "Discard", confirmTone: "warning", onConfirm: unifiedRecovery.restoreAutosave, onCancel: unifiedRecovery.discardAutosave })), unifiedRecovery.mode === 'workspace' && (_jsx(RecoveryBanner, { onRecover: unifiedRecovery.restoreWorkspace, onStartFresh: unifiedRecovery.dismissWorkspace })), syncWarning && (_jsxs("div", { className: "bg-amber-900/80 border-b border-amber-600 px-4 py-2 text-xs text-amber-100 flex items-center justify-between", children: [_jsx("span", { children: syncWarning }), _jsx("button", { type: "button", onClick: () => setSyncWarning(null), className: "px-2 py-1 rounded bg-amber-700 hover:bg-amber-600 text-white text-[10px] font-semibold", children: "Dismiss" })] })), resourceId && (_jsxs("div", { className: "bg-cyan-900/30 border-b border-cyan-700 p-2 text-xs", children: ["Opened from Files: ", _jsx("span", { className: "font-semibold", children: resourceId }), " (", resourceType, ")"] })), _jsx(HierarchyBreadcrumbs, {}), _jsx(ClassroomModeBanner, {}), _jsx(TopCommandBar, { onExamples: ceMode ? () => setShowCEExamplesModal(true) : () => setShowExamplesModal(true), projectName: projectName, onNew: handleNew, onNewProject: handleNewProject, onSaveProject: handleSaveProject, onOpenProject: handleOpenProject, onExportProject: ceMode ? () => setShowCEExportModal(true) : handleExportProject, onSave: handleSave, onSaveAs: handleSaveAs, onShare: handleShare, isDirty: isDirty, onUndo: handleUndo, onRedo: handleRedo, canUndo: canUndo, canRedo: canRedo, isRunning: isRunning, onRun: handleRun, onPause: handlePause, onStep: handleStep, tickCount: tickCount, tickRate: currentHz, onTickRateChange: handleHzChange, onResetTickCount: handleResetTickCount, onReset: ceMode ? () => setShowCEResetModal(true) : undefined, perspective: perspective, onPerspectiveChange: setPerspective, schematicMiniEnabled: schematicMiniEnabled, onToggleSchematicMini: toggleSchematicMini, onManual: () => useLayoutStore.getState().openDock('learn', 'manual'), onHelp: () => setShowKeyboardHelp(true), onStartHere: () => setShowStartHere(true), onExportEvidence: handleExportEvidence, onOpenEvidence: () => onOpenApp?.('submission-inspector'), onResetWorkspace: handleResetWorkspace, onResetLayout: handleResetLayout }), _jsx("input", { ref: projectFileInputRef, type: "file", accept: "application/json,.json", onChange: handleProjectFileChange, className: "hidden", "aria-label": "Open project file" }), _jsxs("div", { className: "flex-1 flex min-h-0 min-w-0 overflow-hidden", children: [_jsx(EnhancedPalette, { primitiveNodes: PRIMITIVE_NODES, compositeNodes: COMPOSITE_NODES, chips: allChips, onNodeDragStart: handleNodeDragStart, onAddNode: handleAddNode, onChipLibraryOpen: () => setShowChipLibrary(true), getChipMetadata: getChipMetadataForNode, getNodeDescription: getNodeDescription, isReplayMode: isReplayMode }), _jsxs("div", { ref: canvasAreaRef, tabIndex: -1, className: "flex-1 min-h-0 min-w-0 relative outline-none", "data-testid": "logic-canvas", onDragOver: handleNodeDragOver, onDrop: handleNodeDrop, onDragEnd: () => {
