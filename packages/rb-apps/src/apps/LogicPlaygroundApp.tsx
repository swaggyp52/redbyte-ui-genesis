import { exportGradingNotes } from '../utils/gradingExport';
import { useEvidenceViewerStore } from '../stores/evidenceViewerStore';
import { useGradingNotesStore } from '../stores/gradingNotesStore';
import { useLabSpecStore } from '../stores/labSpecStore';
import { validateEvidenceAgainstLabSpec } from '@redbyte/rb-logic-core';
// Evidence Viewer Panel with Grading and Lab Spec Validation
const EvidenceViewerPanel: React.FC = () => {
  const { evidenceBundle, verificationStatus, errorMessage } = useEvidenceViewerStore();
  const { grading, setGrading, resetGrading } = useGradingNotesStore();
  const { labSpec, setLabSpec, clearLabSpec } = useLabSpecStore();
  const [validationResult, setValidationResult] = React.useState<any>(null);
  const fileInputRef = React.useRef<HTMLInputElement>(null);

  React.useEffect(() => {
    if (evidenceBundle && labSpec) {
      setValidationResult(validateEvidenceAgainstLabSpec(evidenceBundle, labSpec));
    } else {
      setValidationResult(null);
    }
  }, [evidenceBundle, labSpec]);

  if (!evidenceBundle) return null;
  const { exportedAtIso, integrity, app, context, circuitSnapshot, simulationSnapshot, probesSnapshot, oscilloscopeSnapshot } = evidenceBundle;

  // Handler for loading a .labspec.json file
  const handleLoadLabSpec = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (evt) => {
      try {
        const spec = JSON.parse(evt.target?.result as string);
        setLabSpec(spec);
      } catch (err) {
        window.alert('Invalid lab spec file.');
      }
    };
    ...
    <div className="flex flex-col h-full w-full bg-gray-900 text-gray-100">
      <div className="p-4 border-b border-gray-700 flex items-center gap-4">
        ...
        <span className={verificationStatus === 'PASS' ? 'text-green-400' : 'text-red-400'}>
          {verificationStatus === 'PASS' ? '✅ Verified' : verificationStatus === 'FAIL' ? '❌ Integrity Check Failed' : '⚠️ Unverified'}
        </span>
        <span className="ml-4 text-xs text-gray-400">Hash: {integrity?.integrityHash}</span>
        <span className="ml-4 text-xs text-gray-400">Exported: {exportedAtIso}</span>
        {errorMessage && <span className="ml-4 text-xs text-red-400">{errorMessage}</span>}
        <div className="ml-auto flex gap-2">
          <input
            type="file"
            accept=".labspec.json,application/json"
            ref={fileInputRef}
            style={{ display: 'none' }}
            onChange={handleLoadLabSpec}
            title="Select a .labspec.json file to load lab requirements"
          />
          <button
            className="px-3 py-1.5 bg-blue-700 hover:bg-blue-600 rounded text-white text-xs"
            onClick={() => fileInputRef.current?.click()}
            title="Load Lab Spec (.labspec.json)"
          >
            Load Lab Spec
          </button>
          {labSpec && (
            <button
              ...
              onClick={clearLabSpec}
              title="Clear loaded lab spec"
            >
              Clear Spec
            </button>
          )}
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Center: Circuit snapshot (read-only) */}
        <div className="flex-1 flex items-center justify-center bg-gray-950">
          {/* TODO: Render circuitSnapshot using LogicCanvas or similar, read-only */}
          <pre className="text-xs text-gray-400 bg-gray-950 p-4 rounded max-w-xl overflow-x-auto">{JSON.stringify(circuitSnapshot, null, 2)}</pre>
        </div>
        {/* Right: Details + Grading + Lab Requirements */}
        <div className="w-96 border-l border-gray-800 p-4 flex flex-col gap-4 bg-gray-900 overflow-y-auto">
          {/* Lab Requirements Panel */}
          <div className="mb-4 border-b border-gray-700 pb-4">
            <div className="font-bold text-base mb-2">Lab Requirements</div>
            {labSpec ? (
              <div>
                <div className="text-sm font-semibold mb-1">{labSpec.title || labSpec.labId}</div>
                {labSpec.notes && <div className="text-xs text-gray-400 mb-2">{labSpec.notes}</div>}
                {validationResult ? (
                  <div className="space-y-2">
                    {/* Probes validation */}
                    {validationResult.probes && (
                      <div>
                        <div className="font-semibold text-xs mb-1">Probes</div>
                        <ul className="ml-2">
                          {Object.entries(validationResult.probes).map(([probe, status]) => (
                            <li key={probe} className={status === 'present' ? 'text-green-400' : 'text-red-400'}>
                              {status === 'present' ? '✅' : '❌'} {probe}
                            </li>
                          ))}
                        </ul>
                      </div>
                    ...
                    {/* Ticks validation */}
                    {validationResult.ticks && validationResult.ticks.status !== 'not-checked' && (
                      <div>
                        <div className="font-semibold text-xs mb-1">Ticks</div>
                        <span className={validationResult.ticks.status === 'sufficient' ? 'text-green-400' : 'text-red-400'}>
                          {validationResult.ticks.status === 'sufficient' ? '✅' : '❌'} Required: {validationResult.ticks.required}, Observed: {validationResult.ticks.observed}
                        </span>
                      </div>
                    )}
                    {/* Example ID validation */}
                    {validationResult.exampleMatch && validationResult.exampleMatch !== 'not-checked' && (
                      <div>
                        <div className="font-semibold text-xs mb-1">Example ID</div>
                        <span className={validationResult.exampleMatch === 'match' ? 'text-green-400' : 'text-red-400'}>
                          {validationResult.exampleMatch === 'match' ? '✅ Match' : '❌ Mismatch'}
                        </span>
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="text-xs text-gray-400">No validation results.</div>
                )}
              </div>
            ) : (
              <div className="text-xs text-gray-400">No lab spec loaded. Load a .labspec.json to see requirements.</div>
            )}
          </div>
          {/* ...existing code... Grading Panel and other details ... */}
          <div>
            <div className="font-semibold text-sm mb-1">Context</div>
            <pre className="text-xs bg-gray-950 p-2 rounded">{JSON.stringify(context, null, 2)}</pre>
            <div className="flex gap-2 mt-4">
              <button
                className="px-3 py-1.5 bg-green-600 hover:bg-green-500 rounded text-white font-semibold disabled:bg-gray-700 disabled:cursor-not-allowed"
                disabled={
                  !grading.initials.trim() ||
                  (!grading.score && !grading.passFail && !grading.notes)
                }
                onClick={() => {
                  try {
                    exportGradingNotes({ grading, evidenceHash: integrity?.integrityHash || 'unknown' });
                  } catch (e: any) {
                    window.alert(e.message || 'Failed to export grading');
                  }
                }}
                title="Export grading notes as JSON"
              >
                Export Grading Notes
              </button>
              <button
                className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 rounded text-white"
                onClick={resetGrading}
                title="Clear grading fields"
              >
                Clear
              </button>
            </div>
          ...
          <div>
            <div className="font-semibold text-sm mb-1">Simulation</div>
            <pre className="text-xs bg-gray-950 p-2 rounded">{JSON.stringify(simulationSnapshot, null, 2)}</pre>
          </div>
          <div>
            <div className="font-semibold text-sm mb-1">Probes</div>
            <pre className="text-xs bg-gray-950 p-2 rounded">{JSON.stringify(probesSnapshot, null, 2)}</pre>
          </div>
          <div>
            <div className="font-semibold text-sm mb-1">Oscilloscope</div>
            <pre className="text-xs bg-gray-950 p-2 rounded">{JSON.stringify(oscilloscopeSnapshot, null, 2)}</pre>
          </div>
          {/* Grading Panel */}
          <div className="mt-4 border-t border-gray-700 pt-4">
            <div className="font-bold text-base mb-2">Grading Notes</div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <input
                ...
                min={0}
                max={100}
                onChange={e => setGrading({ score: e.target.value === '' ? undefined : Number(e.target.value) })}
                title="Enter score (0-100)"
                placeholder="Score"
              />
              <label className="ml-4 text-sm">Pass:</label>
              <input
                type="checkbox"
                checked={!!grading.passFail}
                onChange={e => setGrading({ passFail: e.target.checked })}
                title="Check if pass"
              />
            </div>
            <div className="mb-2">
              ...
              <textarea
                className="w-full min-h-[60px] px-2 py-1 rounded bg-gray-800 text-white border border-gray-700"
                value={grading.notes ?? ''}
                onChange={e => setGrading({ notes: e.target.value })}
                title="Enter grading notes"
                placeholder="Notes"
              />
            </div>
            <div className="mb-2">
              <label className="text-sm block mb-1">TA Initials <span className="text-red-400">*</span>:</label>
              <input
                type="text"
                className="w-24 px-2 py-1 rounded bg-gray-800 text-white border border-gray-700"
                value={grading.initials}
                maxLength={8}
                onChange={e => setGrading({ initials: e.target.value })}
                title="Enter TA initials"
