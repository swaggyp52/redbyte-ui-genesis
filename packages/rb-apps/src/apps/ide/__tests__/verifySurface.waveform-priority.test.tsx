...
describe('VerifySurface waveform lane priority', () => {
  ...
  it('keeps mapped input stimulus lanes visible on FAIL runs for repair context', () => {
    const { container } = render(
      <VerifySurface
  ...
  it('keeps mapped inputs stable across PASS rerenders without group toggles', () => {
    ...
    expect(queryByTestId('ide-verify-group-toggle-inputs')).toBeNull();

    // Re-rendering the same run keeps the stable lane browser intact.
