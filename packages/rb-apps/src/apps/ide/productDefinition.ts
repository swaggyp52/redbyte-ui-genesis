...
export const PRODUCT_SPINE_DEFINITIONS: Record<ProductSpinePageKey, ProductSpinePageDefinition> = {
  ...
  design: {
    ...
    keyStates: ['Empty', 'Editing', 'Structurally blocked', 'Ready to simulate'],
    primaryAction: 'Edit the circuit',
    secondaryActions: ['Open Simulate', 'Open Diagnostics'],
    emptyState: 'The stable library, empty canvas, and circuit overview remain visible with a direct first placement path.',
    nextAction: 'What do I do next? Build the circuit, resolve structural diagnostics, then open Simulate.',
    ...
    proofBoundary: 'Design reports structural readiness and live propagation only. Compare proof belongs in Simulate.',
  },
  verify: {
    key: 'verify',
    kind: 'workflow-stage',
    ...
    proofBoundary: 'Simulation proves browser E0 behavior only. It does not prove Vivado or board behavior.',
  },
  hardware: {
    key: 'hardware',
    kind: 'workflow-stage',
    order: 4,
    label: 'Board & Constraints',
    shortLabel: 'Board',
    job: 'Plan project I/O, clocks, electrical constraints, and Basys3 resource assignments.',
    primaryObject: 'Board and constraints workspace',
    stableRegions: ['Progress header', 'Mapping table', 'Selected-signal editor', 'Board reference', 'XDC consequences'],
    keyStates: ['Unmapped', 'Partially mapped', 'Conflicted', 'Ready to build and export'],
    ...
    nextAction: 'What do I do next? Assign required signals, resolve conflicts, then open Build & Export.',
    ...
    excludes: ['Simulation repair controls', 'After-mapping disclosures', 'Dominant board artwork', 'Hidden conflict diagnostics'],
    proofBoundary: 'Board & Constraints records browser package intent. Vivado and board proof remain external.',
  },
  export: {
    key: 'export',
    kind: 'workflow-stage',
    ...
    recovery: 'Follow the named blocker to Design, Simulate, or Board & Constraints, then return and rebuild.',
