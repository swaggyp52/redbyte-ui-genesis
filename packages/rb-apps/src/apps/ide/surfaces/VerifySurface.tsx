...
export interface VerifySurfaceProps {
  ...
  onDebugTickSelected?: (tick: number, signals: Record<string, 0 | 1>) => void;
...
export const VerifySurface: React.FC<VerifySurfaceProps> = ({
  ...
  const [verifyTab, setVerifyTab] = useState<VerifyDrawerTab>('mismatches');
  ...
  const [tickWidth, setTickWidth] = useState(48);
  ...
  const [autoVectorBannerDismissed, setAutoVectorBannerDismissed] = useState(false);
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem('rb.verify-ui.v1');
      if (!raw) return;
  ...
  // Reset dismiss state when a fresh set of auto-generated vectors arrive
  useEffect(() => {
    if (vectorsAreAutoGenerated) setAutoVectorBannerDismissed(false);
  ...
  const handleDebugInDesign = useCallback(() => {
    ...
    onDebugTickSelected(selectedTick, signals);
  }, [selectedTick, onDebugTickSelected, stepSnapshotRows]);
            ...
            {vectorsAreAutoGenerated && !autoVectorBannerDismissed && (
              <IdeCallout tone="info" testId="ide-verify-auto-vector-notice">
                ...
                <div className="ide-inline-actions">
                  <IdeButton
                    tone="ghost"
                    onClick={() => setAutoVectorBannerDismissed(true)}
              ...
              <div className="ide-verify-side-panel-tabs" role="tablist">
                {(['mismatches', 'vectors', 'details'] as const).map((tab) => (
                  <button
                    ...
                    className={`ide-verify-side-panel-tab${sidePanelTab === tab ? ' is-active' : ''}`}
                    onClick={() => setSidePanelTab(tab)}
                    data-testid={`ide-verify-side-tab-${tab}`}
                  >
                    {tab === 'mismatches' && <>Mismatches{failingRows.length > 0 && <span className="ide-verify-tab-badge">{failingRows.length}</span>}</>}
                    {tab === 'vectors' && `Vectors (${authoredVectors.length})`}
                    {tab === 'details' && 'Details'}
                  </button>
                ))}
              </div>
              <div className="ide-verify-side-panel-body">
                {sidePanelTab === 'mismatches' && (
                  <section className="ide-verify-mismatch-panel" data-testid="ide-verify-mismatch-table">
                    ...
                    {displayStatus === 'FAIL' && selectedFailureCase && (
                      <div className="ide-verify-failure-explainer" data-testid="ide-verify-failure-explainer">
                        ...
                        <div className="ide-inline-actions">
                          <IdeButton tone="primary" onClick={() => applyFailureSelection(selectedFailureCase)} testId="ide-verify-explainer-jump">
                            Jump to tick
                          </IdeButton>
                          <IdeButton tone="secondary" onClick={focusMismatchLanes} testId="ide-verify-explainer-show-mismatches">
                            Show only mismatches
                          </IdeButton>
                          {showMismatchOnlySignals && (
                            <IdeButton tone="ghost" onClick={clearMismatchLaneFilter} testId="ide-verify-explainer-show-all-lanes">
                              Show all lanes
                            </IdeButton>
                          )}
                        </div>
                          ...
                          {failingRows.map((row) => (
                            ...
                            >
                              <td className="ide-verify-mismatch-tick">
                                <button type="button" className="ide-verify-mismatch-tick-btn" onClick={() => applyFailureSelection(row)}>
                                  t{row.tick}
                                </button>
                              </td>
                  ...
                  <>
                    ...
                    <section data-testid="ide-verify-signal-table">
                      <IdeDataTable
                        ...
                        rows={selectedTickRows.map((row) => [row.signal, String(row.tick), row.expected, row.actual])}
                      />
                    </section>
                    {status === 'fail' && (
                      <section data-testid="ide-verify-diff-table">
                        <IdeCallout tone="error" title="Failure Diff">
                          ...
                          <div className="ide-inline-actions">
                            <IdeButton
                              tone="secondary"
                              onClick={() => { if (!selectedFailureCase) return; onFixPath?.(selectedFailureCase); }}
            ...
            <div className="ide-verify-drawer-body">
              ...
              {drawerTabs.map((tab) => (
                <button
                  key={tab}
                  type="button"
                  className={`ide-verify-tab-btn ${verifyTab === tab ? 'is-active' : ''}`}
