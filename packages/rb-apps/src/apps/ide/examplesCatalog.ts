...
export const IDE_EXAMPLES: IdeExampleDefinition[] = [
  ...
  },
  {
    ...
    // Synchronous 2-bit counter. With clocked_macro schedule [CLK=0→1→0] per vector:
    // Q0_next = Q0 XOR EN  (Q0 toggles each tick when EN=1)
    // Q1_next = Q1 XOR (Q0 AND EN)  (Q1 toggles when Q0=1 and EN=1)
    // Trace from Q0=0, Q1=0:
    //   tick0 EN=1: Q0→1 Q1→0  (01)
    //   tick1 EN=1: Q0→0 Q1→1  (10)
    //   tick2 EN=1: Q0→1 Q1→1  (11)
    //   tick3 EN=1: Q0→0 Q1→0  (00 rollover)
    //   tick4 EN=0: Q0→0 Q1→0  (held)
    //   tick5 EN=0: Q0→0 Q1→0  (still held)
    vectors: [
      { tick: 0, inputs: { clk_node: 0, en_node: 1 }, expected: { q0_out: 1, q1_out: 0 } },
      { tick: 1, inputs: { clk_node: 0, en_node: 1 }, expected: { q0_out: 0, q1_out: 1 } },
      { tick: 2, inputs: { clk_node: 0, en_node: 1 }, expected: { q0_out: 1, q1_out: 1 } },
      { tick: 3, inputs: { clk_node: 0, en_node: 1 }, expected: { q0_out: 0, q1_out: 0 } },
      { tick: 4, inputs: { clk_node: 0, en_node: 0 }, expected: { q0_out: 0, q1_out: 0 } },
    ...
    circuit: {
      nodes: [
        ...
        // Output sinks
        { id: 'q0_out',   type: 'OUTPUT',    x: 620, y: 100, label: 'LD0', config: {}, state: {} },
        { id: 'q1_out',   type: 'OUTPUT',    x: 620, y: 260, label: 'LD1', config: {}, state: {} },
      ],
      connections: [
        // Q0_next = Q0 XOR EN
        { from: { nodeId: 'q0_ff',  portName: 'Q'   }, to: { nodeId: 'xor0',   portName: 'a'   } },
        { from: { nodeId: 'en_node',portName: 'out'  }, to: { nodeId: 'xor0',   portName: 'b'   } },
        { from: { nodeId: 'xor0',   portName: 'out'  }, to: { nodeId: 'q0_ff',  portName: 'D'   } },
        ...
        { from: { nodeId: 'xor1',   portName: 'out'  }, to: { nodeId: 'q1_ff',  portName: 'D'   } },
