...
import { CircuitEngine, TickEngine, toCircuitV1, fromCircuitV1 } from '@redbyte/rb-logic-core';
...
export const ECELabAppComponent = ({ windowId, labId }) => {
    ...
    useEffect(() => {
        ...
        if (!saveStatusText)
            return;
        if (lastSaveToastRef.current === saveStatusText)
            return;
...
`);
    ...
    return (_jsxs("div", { className: "flex flex-col h-full bg-gradient-to-b from-gray-950 to-black font-mono", children: [unifiedRecovery.mode === 'autosave' && (_jsx(GuardrailConfirmModal, { isOpen: true, title: "Restore autosave?", message: `An autosave is available from ${new Date(unifiedRecovery.autosaveMeta?.savedAtMs ?? Date.now()).toLocaleString()}. Restore it now?`, lossItems: ['Discarding autosave will permanently delete the autosaved copy.'], confirmLabel: "Restore", cancelLabel: "Discard", confirmTone: "warning", onConfirm: unifiedRecovery.restoreAutosave, onCancel: unifiedRecovery.discardAutosave })), _jsxs("div", { className: "flex items-center justify-between px-4 py-2 bg-gradient-to-b from-gray-900 to-black border-b border-gray-800", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: "w-8 h-8 rounded flex items-center justify-center", style: {
