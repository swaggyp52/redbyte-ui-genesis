...
import type { RBProjectDoc } from '@redbyte/rb-logic-core';
import {
  normalizeProjectDoc,
  serializeProjectDoc,
 ...
 */
let autosaveScheduled: ReturnType<typeof setTimeout> | null = null;
 ...
 */
export async function loadProjectFromAutosave(): Promise<RBProjectDoc | null> {
  try {
 ...
 */
export async function saveProjectToAutosave(doc: RBProjectDoc): Promise<void> {
  if (typeof localStorage === 'undefined') {
    return;
  }
 ...
 */
export async function loadProjectFromFile(fileContent: string): Promise<RBProjectDoc> {
  try {
 ...
 */
export async function saveProjectToFile(doc: RBProjectDoc, filename?: string): Promise<void> {
  try {
    const normalized = normalizeProjectDoc(doc);
    const json = serializeProjectDoc(normalized);
 ...
 */
export async function loadProject(source: 'autosave' | 'file' = 'autosave'): Promise<RBProjectDoc> {
  if (source === 'autosave') {
 ...
 */
export async function saveProject(doc: RBProjectDoc, target: 'autosave' | 'file' = 'autosave', filename?: string): Promise<void> {
  if (target === 'autosave') {
