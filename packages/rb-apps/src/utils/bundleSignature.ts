...
export async function verifyBundleSignature(zipBytes: Uint8Array): Promise<SignatureStatus> {
  const zip = await JSZip.loadAsync(zipBytes);
  const signatureFile = zip.file('integrity/signature.sig');
  if (!signatureFile) return 'Unsigned';

  const capsuleFile = zip.file('integrity/capsule.json');
  if (!capsuleFile) return 'Invalid';
  ...
  const fileEntries: Array<{ path: string; bytes: Uint8Array }> = [];
