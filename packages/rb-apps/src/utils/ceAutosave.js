...
import { getCEConfig, loadCEState, saveCEState } from '@redbyte/rb-utils';
...
export function loadSavedCircuit() {
    ...
    try {
        ...
        if (!saved.lastCircuit)
            return null;
        const parsed = JSON.parse(saved.lastCircuit);
        // Validate schema version (for future upgrades)
        if (parsed.version !== CE_CIRCUIT_SCHEMA_VERSION) {
            console.warn(`[Autosave] Saved circuit has version ${parsed.version}, ` +
 ...
 */
export function saveCircuitToStorage(circuit) {
    const config = getCEConfig();
    if (!config.enabled)
        return;
    try {
 ...
 */
export function useAutosaveCircuit() {
    const config = getCEConfig();
 ...
 */
export function useRestoreCircuit() {
    const circuit = useCircuitStore((state) => state.circuit);
 ...
 */
export function clearAutosaveStorage() {
    try {
 ...
 */
export function clearSavedCircuit() {
