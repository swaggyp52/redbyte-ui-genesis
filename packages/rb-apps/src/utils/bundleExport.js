...
export async function exportV2Bundle(options) {
    ...
    try {
        ...
        const safeAddFile = async (path, data, step) => {
            try {
        ...
        if (warnings.length > 0) {
            await safeAddFile('warnings.json', JSON.stringify({ schemaVersion: 1, createdAt: new Date().toISOString(), warnings }, null, 2), 'warnings');
        }
