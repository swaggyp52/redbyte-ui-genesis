...
// ---------------------------------------------------------------------------
// Worker pool (single worker, message-multiplexed)
// ---------------------------------------------------------------------------
...
function postToWorker(op: string, payload: unknown, transfer?: Transferable[]): Promise<unknown> {
  const w = getWorker();
 ...
 */
export async function hashBytesOffThread(data: ArrayBuffer): Promise<string> {
  if (useWorker()) {
    try {
      // Transfer the buffer to avoid copy (caller must not reuse)
      return (await postToWorker('hashBytes', data, [data])) as string;
    } catch {
 ...
 */
export async function stableHashOffThread(value: unknown): Promise<string> {
  if (useWorker()) {
    try {
      return (await postToWorker('stableHash', value)) as string;
    } catch {
      // Fall through
    }
  }
 ...
 */
export async function stableSerializeOffThread(value: unknown): Promise<string> {
  if (useWorker()) {
    try {
      return (await postToWorker('stableSerialize', value)) as string;
    } catch {
      // Fall through
    }
  }
