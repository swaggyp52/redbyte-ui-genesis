...
export function isCEMode(): boolean {
 ...
 */
export function isHeavyCircuit(nodeCount: number, connectionCount: number): boolean {
  const config = getCEConfig();
  return (
    nodeCount >= config.heavyCircuitThreshold.nodes ||
