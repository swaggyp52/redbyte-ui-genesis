...
const READABLE_TOOL_VERSION = 'Vivado 2024.2+';
...
export function buildVivadoXpr(input: BuildVivadoXprInput): string {
  ...
  const simulationFileBlock = includeSimulation
    ? [
        ...
        '          <Attr Name="VHDLVersion" Val="vhdl_2k8"/>',
        '        </FileInfo>',
        '      </File>',
      ]
    : [];
...
function buildVivadoProjectFolderReadme(input: BuildVivadoProjectFolderReadmeInput): string {
  const lines = [
    ...
    'This package does not prove Vivado build/bitstream success, board programming, or observed hardware behavior.',
    'Collect E1/E2/E3 evidence separately after running Vivado and checking the physical board.',
    '',
    'IMPORTANT — Extract to a short path:',
    '  Vivado warns when the project path exceeds ~80 characters.',
    '  Extract this ZIP to a short location, e.g.:',
    '    Windows: C:\\rb\\  or  C:\\Users\\YourName\\Desktop\\',
    '    macOS/Linux: ~/rb/',
    '  Avoid deeply nested Downloads folders.',
    '',
    'IMPORTANT — Launch Vivado from a writable folder:',
    '  Vivado writes vivado.jou and vivado.log to its working directory.',
    '  If Vivado is launched from C:\\Windows\\System32 or another read-only location,',
    '  it will print "Failed to open vivado.jou / vivado.log" — this is a launch issue,',
    '  not a project issue. To avoid it:',
    '    - Open Vivado from the Start menu (not by double-clicking vivado.exe directly).',
    '    - Or: cd to this project folder in a terminal, then run: vivado .',
    '',
    'Recommended student flow:',
    `1. Extract this ZIP so the folder "${input.projectSlug}" is at a short path.`,
    '2. Open Vivado 2024.2 from the Start menu.',
    `3. Click "Open Project" and select "${input.projectSlug}.xpr".`,
    '4. Dismiss any "project file moved" notice — this is expected and harmless.',
    '5. Confirm the part, top module, design sources, and constraints are loaded.',
    '6. Run Synthesis → Run Implementation → Generate Bitstream.',
    '7. Open Hardware Manager and program the Basys3.',
    '',
    'Fallback (batch mode):',
    'If Open Project is unavailable, run from this project folder:',
    '  vivado -mode batch -source vivado_import.tcl -notrace -nojournal -log vivado_import.log',
    '',
    'Notes:',
    `- Constraints file: "${REFERENCE_CONSTRAINTS_FILE_NAME}" — top-level port names match the generated XDC.`,
    '- VHDL source is set to VHDL 2008 via file properties (not the obsolete project-level flag).',
    '- RedByte generated this folder deterministically from the project manifest.',
    '',
    'Harmless Vivado messages you can safely ignore:',
    '  "Project file moved from $PPRDIR"       — expected; this is a portable ZIP layout.',
    '  "GeneratedRun file not found"            — expected on first open of a fresh project.',
    '  "No supported debug core(s) found"       — expected; no ILA/VIO in this export.',
    '  "No reference checkpoint was found"      — expected on initial synthesis.',
    '  "No user specified timing constraints"   — expected for combinational designs.',
    '  "WARNING: vivado.jou cannot be opened"  — launch Vivado from a writable folder (see above).',
  ];
