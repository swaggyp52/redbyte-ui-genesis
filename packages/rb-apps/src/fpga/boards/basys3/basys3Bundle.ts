...
function buildReadme(
  ...
  const lines: string[] = [];
  lines.push('# RedByte Basys3 Export — Vivado Project Folder');
  lines.push('');
  lines.push('## Contents');
  lines.push('- `<project>.xpr`: Vivado project file (pre-configured — just open it)');
  lines.push('- `top.vhd`: deterministic synthesizable VHDL top module');
  lines.push('- `basys3.xdc`: Basys3 pin constraints for mapped ports only');
  lines.push('- `vivado_import.tcl`: batch-mode fallback import script');
  ...
  lines.push('2. Open Vivado → File → Open Project → select the .xpr file inside the folder.');
  lines.push('3. Dismiss any "project file moved" notice — this is expected and harmless.');
  ...
  lines.push('');
  lines.push('Batch fallback (from the extracted folder):');
