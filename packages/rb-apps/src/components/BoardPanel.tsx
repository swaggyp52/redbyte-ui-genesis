...
const Basys3Layout: React.FC<{
  ioSnapshot: ReturnType<typeof useHardwareStore>['ioSnapshot'];
...
export const BoardPanel: React.FC<BoardPanelProps> = ({ className = '' }) => {
  ...
  const renderBoardLayout = () => {
    ...
    // Fall back to generic grid for other boards
    return (
      <GenericIOGrid
        ...
        <div className="p-3 border-b border-gray-800">
          ...
          <div className="flex flex-col gap-2">
            {availableDevices.map((device) => (
              ...
              >
                <div className="text-gray-200">{device.boardModel}</div>
                <div className="text-xs text-gray-500">
                  {device.serial || device.deviceId}
                </div>
              </button>
            ))}
          </div>
