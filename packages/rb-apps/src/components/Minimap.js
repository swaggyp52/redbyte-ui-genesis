...
export const Minimap = ({ nodes, viewport, containerWidth, containerHeight }) => {
    ...
    return (_jsxs("div", { className: "w-[200px] h-[150px] bg-gray-900 border border-gray-700 rounded overflow-hidden relative cursor-pointer shadow-lg", onClick: handleMapClick, children: [_jsx("div", { className: "absolute inset-0 opacity-20 bg-gray-800" }), nodes.map(node => {
                const pos = worldToMap(node.position.x, node.position.y);
                return (_jsx("div", { className: "absolute w-1 h-1 bg-gray-400 rounded-full", style: { left: pos.x, top: pos.y } }, node.id));
            }), _jsx("div", { className: "absolute border border-cyan-500 bg-cyan-500/10 z-10 box-border pointer-events-none", style: {
                    left: vpRect.x,
                    top: vpRect.y,
                    width: vpW,
                    height: vpH,
