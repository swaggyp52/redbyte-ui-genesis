...
export interface ClassroomEditionConfig {
  enabled: boolean;
  autosaveEnabled: boolean;
  tickRateCapped: boolean;
  defaultPerspective: 'build' | 'analyze' | 'explain' | 'explore' | 'quad';
  examplesVisible: boolean;
  exportVisible: boolean;
  resetVisible: boolean;
  helpOverlayAccessible: boolean;
  autosaveIntervalMs: number;
}

const DEFAULT_CE_CONFIG: ClassroomEditionConfig = {
  enabled: false,
 ...
 */
export const detectCEMode = (): boolean => {
  if (typeof window === 'undefined') return false;

  // Check query param
  const params = new URLSearchParams(window.location.search);
  if (params.get('ce') === '1') return true;

  // Check Vite env var (injected at build time)
  if (import.meta.env.VITE_CLASSROOM === 'true') return true;
 ...
 */
export const getCEConfig = (): ClassroomEditionConfig => {
...
export interface CEStorageState {
  lastCircuit?: string; // JSON-stringified circuit state
 ...
 */
export const saveCEState = (state: CEStorageState) => {
 ...
 */
export const loadCEState = (): CEStorageState => {
  if (typeof localStorage === 'undefined') return {};
  try {
