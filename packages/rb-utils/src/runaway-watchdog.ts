...
export function enableWatchdog() {
  ...
  if (typeof window !== 'undefined') {
    (window as any).__RB_WATCHDOG_CLEANUP__ = () => {
      ...
      const microtaskInterval = (window as any).__RB_WATCHDOG_MICROTASK_INTERVAL__;
...
function monitorMicrotasks() {
  // Sample microtask throughput by measuring how many microtasks execute
  // in a short burst, triggered periodically via setInterval (macro task).
  const SAMPLE_INTERVAL_MS = 200;
  const BURST_SIZE = 20; // queue this many microtasks per sample
  ...
  const intervalId = setInterval(() => {
    ...
    // Queue a fixed burst of microtasks and measure throughput
    function countOne() {
      burstCount++;
      if (burstCount < BURST_SIZE) {
        queueMicrotask(countOne);
      } else {
        // All burst microtasks completed — estimate rate
        const elapsed = Math.max(1, Date.now() - burstStart);
        // Extrapolate from burst to per-second rate
  ...
  if (typeof window !== 'undefined') {
    (window as any).__RB_WATCHDOG_MICROTASK_INTERVAL__ = intervalId;
  }
}
