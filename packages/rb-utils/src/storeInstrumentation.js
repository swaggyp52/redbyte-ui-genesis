...
let writeCount = 0;
let writeCountWindow = [];
let lastWriteValue = Symbol('initial');
 ...
 */
export function trackStoreWrite(value) {
    writeCount++;
    const now = Date.now();
 ...
 */
export function trackSubscriberCount(count) {
 ...
 */
export function assertNoRunawayLoop(options) {
    const m = getMetrics();
    const maxWrites = options?.maxWritesPerSec ?? 5;
    const maxChurn = options?.maxChurnPercent ?? 50;
    if (m.stateWritesPerSecond > maxWrites) {
        throw new Error(`Potential runaway store loop: ${m.stateWritesPerSecond} writes/sec (limit: ${maxWrites}). ` +
    ...
    if (m.churnPercentage > maxChurn && m.stateWritesPerSecond > 2) {
        throw new Error(`High store churn detected: ${m.churnPercentage}% redundant writes (limit: ${maxChurn}%). ` +
