...
function classifyByMessage(message) {
    ...
    if (m.includes('port busy') ||
        m.includes('permission denied') ||
        m.includes('failed to open session')) {
        return 'SESSION_CONNECT_FAILED';
    }
    if (m.includes('invalid trace') ||
        m.includes('evidence file is invalid') ||
        m.includes('invalid evidence')) {
        return 'EVIDENCE_INVALID';
    }
    return null;
}
export function getStudentFacingMessage(code, opts) {
...
export function toStudentFacingError(err) {
    if (err &&
        typeof err === 'object' &&
        'code' in err &&
        ...
        const shouldUseRawMessage = !!rawMessage &&
            rawMessage !== String(code) &&
            rawMessage !== String(e.code);
        return {
            code,
