...
export type ResolvedThemeVariant = Exclude<ThemeVariant, 'system'>;

export function resolveThemeVariant(variant: ThemeVariant): ResolvedThemeVariant {
  if (variant === 'system') {
...
export function getActiveTheme(): ThemeVariant | null {
  ...
  try {
    ...
    if (stored === 'redbyte-dark') return 'dark';
    if (stored === 'instrument') return 'light';
  } catch {}
