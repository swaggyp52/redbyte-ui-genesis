...
describe('ThemeProvider', () => {
  ...
  it('applies theme CSS variables to document.documentElement', async () => {
    ...
    await waitFor(() => {
      ...
      expect(root.style.getPropertyValue('--rb-color-accent-500')).toBe(
        '#f43f5e'
      );
  ...
  it('allows switching themes via setVariant', async () => {
    ...
    await waitFor(() => {
      ...
      expect(document.documentElement.getAttribute('data-rb-theme')).toBe(
        'light-frost'
      );
    });
  });
  ...
  it('persists theme changes to localStorage', async () => {
    ...
    await waitFor(() => {
      expect(window.localStorage.getItem('rb-theme-variant')).toBe(
        'light-frost'
      );
    });
  });
