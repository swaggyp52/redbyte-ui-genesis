 ...
 */
export function normalizePortRef(ref, fallbackPort = 'out') {
 ...
 */
export function calculatePortPosition(node, portName, nodeWidth, nodeHeight) {
    const { x, y } = pos(node);
 ...
 */
export function manhattanRoute(from, to) {
    const points = [];
    points.push(from);
