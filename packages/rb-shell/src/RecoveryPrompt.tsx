...
interface RecoveryPromptProps {
  onRecover: (entries: RecoveryAction[]) => void;
  onDiscard: () => void;
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 13, color: 'var(--rb-text-2, #a1a1aa)', marginBottom: 16 }}>
          {entries.length === 1
            ? 'An unsaved session was found from a previous session that was interrupted.'
            : `${entries.length} unsaved sessions were found from a previous session that was interrupted.`}
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          {entries.map((entry) => (
            ...
            >
              <span style={{ color: 'var(--rb-text, #e4e4e7)' }}>
                {entry.appId ?? entry.windowId}
              </span>
              <span style={{ color: 'var(--rb-text-3, #71717a)', fontSize: 11 }}>
                {age(entry.timestamp)}
                {entry.reason === 'interrupted' ? ' · interrupted' : ' · unsaved'}
              </span>
            </div>
          ))}
        </div>
