...
describe('WindowSwitcher', () => {
    ...
    describe('MRU Ordering', () => {
        it('sorts windows by lastFocusedAt DESC (most recent first)', () => {
            ...
            render(_jsx(WindowSwitcher, { windows: windows, onSelect: onSelect, onCancel: onCancel }));
    ...
    describe('Keyboard Navigation', () => {
        it('Tab cycles forward through windows', () => {
            ...
            const onSelect = vi.fn();
            const onCancel = vi.fn();
            const { container } = render(_jsx(WindowSwitcher, { windows: windows, onSelect: onSelect, onCancel: onCancel }));
            const overlay = container.querySelector('div[tabindex="0"]');
    ...
    describe('Cancel Behavior', () => {
        ...
        it('clicking inside modal does NOT call onCancel', () => {
            ...
            const modal = screen.getByText('Switch Window').closest('div.bg-slate-900');
    ...
    describe('Edge Cases', () => {
        it('displays empty state when no windows', () => {
            const onSelect = vi.fn();
            const onCancel = vi.fn();
            render(_jsx(WindowSwitcher, { windows: [], onSelect: onSelect, onCancel: onCancel }));
            expect(screen.getByText('No windows open')).toBeInTheDocument();
        });
        it('does not call onSelect when Enter pressed with no windows', () => {
            const onSelect = vi.fn();
            const onCancel = vi.fn();
            const { container } = render(_jsx(WindowSwitcher, { windows: [], onSelect: onSelect, onCancel: onCancel }));
            const overlay = container.querySelector('div[tabindex="0"]');
            fireEvent.keyDown(overlay, { key: 'Enter' });
            expect(onSelect).not.toHaveBeenCalled();
        });
        it('resets selection to 0 when windows list changes', () => {
            ...
            const onSelect = vi.fn();
            const onCancel = vi.fn();
            const { container, rerender } = render(_jsx(WindowSwitcher, { windows: windows1, onSelect: onSelect, onCancel: onCancel }));
            const overlay = container.querySelector('div[tabindex="0"]');
            // Navigate to second window
            fireEvent.keyDown(overlay, { key: 'Tab' });
            // Change windows list
            rerender(_jsx(WindowSwitcher, { windows: windows2, onSelect: onSelect, onCancel: onCancel }));
