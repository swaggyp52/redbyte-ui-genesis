...
export const DeadZoneScanner: React.FC = () => {
    const [status, setStatus] = useState<string>('Ready to scan (Click Button)');
    const [missedCount, setMissedCount] = useState<number>(0);

    const getClassNameSafe = (el: Element): string => {
        const anyEl = el as any;
        const cn = anyEl?.className;
        if (typeof cn === 'string') return cn;
        if (cn && typeof cn === 'object' && typeof cn.baseVal === 'string') return cn.baseVal;
        return '';
    };
    ...
    return (
        <div id="dead-zone-scanner-ui" className="fixed bottom-20 left-1/2 -translate-x-1/2 z-[100000] bg-slate-900 border border-cyan-500 p-4 rounded-lg shadow-2xl font-mono text-xs text-cyan-400">
            <div className="flex items-center gap-2 mb-2">
                <span className="animate-pulse">●</span>
                <span className="font-bold">DEAD-ZONE SCANNER (v3 DETECT)</span>
            </div>
            <div className="mb-2">Status: {status}</div>
            <div className="mb-2">Grid: 25x25</div>
            <button
                onClick={runScan}
                onPointerDown={(e) => e.stopPropagation()} // Prevent self-block
                className="bg-cyan-600 hover:bg-cyan-500 text-white px-3 py-2 rounded w-full font-bold transition-colors cursor-pointer"
