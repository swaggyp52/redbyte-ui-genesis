...
FEATURE BRANCH PUSHED / PR #85 DRAFT / NO MERGE / NO PRODUCTION.**

Connor's correction opening this session: "inherited" is not a reason to ignore a defect, but a
failing historical test is not by itself a specification for what RedByte should become. And:
stop saying "it failed before my edit, so it's fine" - this codebase was written by many agents in
sequence and everything has to be reconciled.

### The validation claim itself was the first thing to fix

`packages/rb-apps` has no `typecheck` script and declares no TypeScript. Root `typecheck` is
`pnpm -r --if-present run typecheck` = four small packages. `.github/workflows/pr-fast-checks.yml:48`
runs it. **CI has never type-checked the workbench.** Three compilers installed through other
packages; 5.3.3 -> 762 diagnostics, 5.9.3 -> 778. `scripts/rb-typecheck-baseline.mjs` pins the
compiler and the project, records exit code, per-code counts and a file+code fingerprint
(`8f5e51e15f8ececd`), fails on an increase, refuses to compare across compilers, and runs first in
`verify:gates`. `byFile` records every file, not a top-25 slice - a truncated list cannot attribute
a regression, which is the only question the record answers.

### Data safety

- **Import collision.** `handleProjectFileSelected` -> `handleSafeLoadIntoIde` -> `loadFromProject`
  adopts `project.meta.projectId` verbatim (projectRuntime.ts:975-978), and the autosave effect then
  writes under that id. Exporting a backup and importing it after more work replaced the newer saved
  record in place. An imported file colliding with a saved id becomes its own project.
  `project-persistence-journey` section I proves both survive, including cross-writes and evidence
  provenance. Red without the fix with a message that names the defect.
- **Recents.** `isPristineProjectHome` existed at IdeApp.tsx:1518 and was consumed by one unrelated
  line; the autosave effect now returns early on it. Recovery checkpoints (`backup-` ids) are
  filtered from Recent only - the Start Center's Recover section still lists them from its own
  source.

### Observe-first, restored without restoring the old direction

`ide-vcb-author-expected` existed in `simulation-studio-v3.css:414` and one test, and in no
component: `VerifyCommandBar` declared `needsExpectedOutputs` and `onAuthorExpectedOutputs` and
rendered neither. Restoring the control alone would have restored the rejected priority with it, so
four remnants went at the same time - the prop comment, the mode explainer showing the Compare
blocker while Observe was selected, the NEEDS CHECKS badge and warn tone, and a Run pulse keyed to
expected values. Comparison states what it needs beside Compare, only when the offer is not already
stating it, with `aria-describedby`.

Read off the running product with every expected value stripped: Observe selected and enabled,
"Run - observe only", explainer "Record observed outputs without grading expected values", offer
"Add expected outputs", and NEEDS CHECKS nowhere on the surface.

### The inherited reds

`docs/validation/test-debt.md` - 531 files, 454 pass / 53 fail; 3418 tests pass / 111 fail; 1 chunk
produced no report, recorded as crashed. Explicitly a **mixed-revision diagnostic**, since the sweep
ran while this session edited source. Closed here: the seven callers of one Board-readiness helper
that pinned prose after the Verify -> Simulate rename (it reads the callout tone now), the Export
assertion that reached for a file row on first entry when Build & Export opens on the dossier, the
Case Lab cycle test that pinned 0 -> 1 when the documented cycle is unset -> 1 -> 0 -> unset, the
observe-first test that pinned a signal id where the surface publishes a label for a display
breadcrumb, and the missing authoring control - which is the one that turned out to be a product
defect rather than a stale assertion. **51 files remain open.**

### Two artefacts of the tooling, caught

- The Edit tool rewrote mixed-EOL files uniformly: a 7-line change produced a 153-line diff.
  `.redbyte/tools/capture/restore-eol.mjs` restores per-line endings against a revision; the whole
  test-file diff for this session is 226 insertions / 5 deletions against `50223c14e`.
- vitest rewrote `packages/rb-fpga-toolchain/__tests__/__snapshots__/protocol.test.ts.snap` with
  different line endings during the sweep. Content identical; reverted rather than committed - a
  golden regenerated by accident is still a golden regenerated.

### Validation at `632089057`

`pnpm verify:gates` exit 0. 27/27 journeys against one build. Typecheck 778, shape identical. Six
touched suites 109 passed / 1 skipped.

### Not started

Board Guided/Expert over the existing planner. The Project close/resume lifecycle: **there is no
Close operation in the product at all** - grep returns zero hits and `projectRepository`'s interface
is list/open/save/autosave/checkpoint/recover with no delete and no rename. The 51 open debt entries.
